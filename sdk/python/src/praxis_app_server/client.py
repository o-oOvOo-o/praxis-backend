...
RUNTIME_PKG_NAME = "praxis-cli-bin"
...
def _installed_praxis_path() -> Path:
    try:
        from praxis_cli_bin import bundled_praxis_path
    except ImportError as exc:
        raise FileNotFoundError(
            "Unable to locate the pinned Codex runtime. Install the published SDK build "
            f"with its {RUNTIME_PKG_NAME} dependency, or set AppServerConfig.praxis_bin "
            "explicitly."
        ) from exc

    return bundled_praxis_path()


@dataclass(frozen=True)
class CodexBinResolverOps:
    installed_praxis_path: Callable[[], Path]
    path_exists: Callable[[Path], bool]


def _default_praxis_bin_resolver_ops() -> CodexBinResolverOps:
    return CodexBinResolverOps(
        installed_praxis_path=_installed_praxis_path,
        path_exists=lambda path: path.exists(),
    )


def resolve_praxis_bin(config: "AppServerConfig", ops: CodexBinResolverOps) -> Path:
    if config.praxis_bin is not None:
        praxis_bin = Path(config.praxis_bin)
        if not ops.path_exists(praxis_bin):
            raise FileNotFoundError(
                f"Codex binary not found at {praxis_bin}. Set AppServerConfig.praxis_bin "
                "to a valid binary path."
            )
        return praxis_bin

    return ops.installed_praxis_path()


def _resolve_praxis_bin(config: "AppServerConfig") -> Path:
    return resolve_praxis_bin(config, _default_praxis_bin_resolver_ops())


@dataclass(slots=True)
class AppServerConfig:
    praxis_bin: str | None = None
    launch_args_override: tuple[str, ...] | None = None
    config_overrides: tuple[str, ...] = ()
    cwd: str | None = None
    env: dict[str, str] | None = None
    client_name: str = "praxis_python_sdk"
...
class AppServerClient:
    ...
    def start(self) -> None:
        ...
        if self.config.launch_args_override is not None:
            ...
            praxis_bin = _resolve_praxis_bin(self.config)
            args = [str(praxis_bin)]
