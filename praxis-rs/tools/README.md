...
`praxis-tools` is intended to become the home for tool-related code that is
...
- MCP resource, `list_directory`, and `test_sync_tool` spec builders
...
compatibility-sensitive orchestration in `praxis-core` until the surrounding
...
- tool metadata and compatibility shims that do not depend on `praxis-core`
- other narrowly scoped utility code that multiple crates need

The corresponding non-goals are just as important:

- do not move `praxis-core` orchestration here prematurely
...
2. Switch non-core consumers to depend on `praxis-tools` directly.
3. Leave compatibility-sensitive adapters in `praxis-core` while downstream
   call sites are updated.
4. Only extract higher-level tool infrastructure after the crate boundaries are
   clear and independently testable.

That means it is normal for `praxis-core` to temporarily re-export types or
helpers from `praxis-tools` during the transition.
...
`praxis-core`, that is a sign to revisit the extraction boundary before adding
more here.
