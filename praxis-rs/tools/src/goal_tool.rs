...
pub fn create_update_goal_tool() -> ToolSpec {
    let properties = BTreeMap::from([(
        ...
        JsonSchema::String {
            description: Some(
                "Required. One of: complete, blocked. Set complete only when the objective is achieved. Set blocked only after the same blocking condition repeats for at least three consecutive goal turns and progress is impossible without user input."
                    .to_string(),
            ),
        },
    )]);
