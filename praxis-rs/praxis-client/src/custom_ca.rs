//! Custom CA handling for Praxis outbound HTTP and websocket clients.
//!
//! Praxis constructs outbound reqwest clients and secure websocket connections in a few crates, but
...
//!   because Praxis itself commonly runs spawned test processes under seatbelt, so this is not just
...
/// easy mistake to make when adding a new outbound Praxis HTTP path, and the resulting bug only
...
fn maybe_build_rustls_client_config_with_env(
    ...
    // they would get from tungstenite's default rustls connector, then layer in the Praxis custom
...
fn build_reqwest_client_with_env(
    ...
    info!(
        praxis_ca_certificate_configured = false,
...
impl ConfiguredCaBundle {
    ...
    /// Loads every certificate block from a PEM file intended for Praxis CA overrides.
    ///
    /// This accepts a few common real-world variants so Praxis behaves like other CA-aware tooling:
...
impl NormalizedPem {
    ...
    ///
    /// Praxis only needs certificate DER bytes to seed `reqwest`'s root store, but operators may
