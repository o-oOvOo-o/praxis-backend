...
#[derive(Clone, Debug)]
pub struct PraxisHttpClient {
    inner: reqwest::Client,
}

impl PraxisHttpClient {
    pub fn new(inner: reqwest::Client) -> Self {
        Self { inner }
    }

    pub fn get<U>(&self, url: U) -> PraxisRequestBuilder
    where
        U: IntoUrl,
    {
    ...
    pub fn post<U>(&self, url: U) -> PraxisRequestBuilder
    where
        U: IntoUrl,
    {
    ...
    pub fn request<U>(&self, method: Method, url: U) -> PraxisRequestBuilder
    where
        U: IntoUrl,
    {
        let url_str = url.as_str().to_string();
        PraxisRequestBuilder::new(self.inner.request(method.clone(), url), method, url_str)
    }
}

#[must_use = "requests are not sent unless `send` is awaited"]
#[derive(Debug)]
pub struct PraxisRequestBuilder {
    builder: reqwest::RequestBuilder,
    method: Method,
    url: String,
}

impl PraxisRequestBuilder {
