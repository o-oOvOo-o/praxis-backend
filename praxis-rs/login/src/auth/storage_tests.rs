...
async fn file_storage_load_returns_auth_dot_json() -> anyhow::Result<()> {
    ...
    let storage = FileAuthStorage::new(praxis_home.path().to_path_buf());
...
async fn file_storage_save_persists_auth_dot_json() -> anyhow::Result<()> {
    ...
    let file = get_auth_file(praxis_home.path());
    storage
        .save(&auth_dot_json)
        .context("failed to save auth file")?;
...
#[test]
fn create_auth_storage_with_resolved_home_uses_shared_praxis_home() -> anyhow::Result<()> {
    let praxis_home = tempdir()?;
    let shared_praxis_home = tempdir()?;

    let storage = create_auth_storage_with_resolved_home(
        shared_praxis_home.path().to_path_buf(),
        AuthCredentialsStoreMode::File,
        Arc::new(MockKeyringStore::default()),
    );
    let auth_dot_json = AuthDotJson {
        auth_mode: Some(AuthMode::ApiKey),
        openai_api_key: Some("shared-key".to_string()),
        tokens: None,
        last_refresh: Some(Utc::now()),
    };
    storage.save(&auth_dot_json)?;
    ...
    assert!(
        ...
        "original praxis home should remain untouched when auth storage is bridged"
    );
    let shared_auth_file = shared_praxis_home.path().join("auth.json");
    assert!(
        shared_auth_file.exists(),
        "shared codex home should receive the auth.json"
    );

    let persisted = FileAuthStorage::new(shared_praxis_home.path().to_path_buf()).load()?;
    assert_eq!(persisted, Some(auth_dot_json));
    Ok(())
}
    ...
    F: FnOnce() -> std::io::Result<String>,
{
    ...
    let auth_file = get_auth_file(praxis_home);
...
fn keyring_auth_storage_load_returns_deserialized_auth() -> anyhow::Result<()> {
    ...
    seed_keyring_with_auth(
        &mock_keyring,
        || compute_store_key(praxis_home.path()),
        &expected,
    )?;
...
fn keyring_auth_storage_compute_store_key_for_home_directory() -> anyhow::Result<()> {
    let praxis_home = PathBuf::from("~/.praxis");

    let key = compute_store_key(praxis_home.as_path())?;
...
fn keyring_auth_storage_save_persists_and_removes_fallback_file() -> anyhow::Result<()> {
    ...
    let auth_file = get_auth_file(praxis_home.path());
    std::fs::write(&auth_file, "stale")?;
    ...
    let key = compute_store_key(praxis_home.path())?;
    assert_keyring_saved_auth_and_removed_fallback(&mock_keyring, &key, praxis_home.path(), &auth);
    Ok(())
}
...
fn keyring_auth_storage_delete_removes_keyring_and_file() -> anyhow::Result<()> {
    ...
    let (key, auth_file) =
        seed_keyring_and_fallback_auth_file_for_delete(&mock_keyring, praxis_home.path(), || {
            compute_store_key(praxis_home.path())
        })?;
...
fn auto_auth_storage_load_uses_file_when_keyring_empty() -> anyhow::Result<()> {
    let praxis_home = tempdir()?;
    let mock_keyring = MockKeyringStore::default();
    let storage = AutoAuthStorage::new(praxis_home.path().to_path_buf(), Arc::new(mock_keyring));
