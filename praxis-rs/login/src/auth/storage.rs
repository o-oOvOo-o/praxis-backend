...
pub(super) fn get_auth_file(praxis_home: &Path) -> PathBuf {
    praxis_home.join("auth.json")
}

pub(super) fn delete_file_if_exists(praxis_home: &Path) -> std::io::Result<bool> {
...
impl FileAuthStorage {
    pub(super) fn new(praxis_home: PathBuf) -> Self {
        Self { praxis_home }
    }
...
impl AuthStorageBackend for FileAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let auth_file = get_auth_file(&self.praxis_home);
    ...
    fn delete(&self) -> std::io::Result<bool> {
        delete_file_if_exists(&self.praxis_home)
    }
}

const KEYRING_SERVICE: &str = "Praxis Auth";

// turns praxis_home path into a stable, short key string
fn compute_store_key(praxis_home: &Path) -> std::io::Result<String> {
    let canonical = praxis_home
        .canonicalize()
        .unwrap_or_else(|_| praxis_home.to_path_buf());
...
impl KeyringAuthStorage {
    fn new(praxis_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            praxis_home,
            keyring_store,
        }
...
impl AuthStorageBackend for KeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let key = compute_store_key(&self.praxis_home)?;
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        if let Err(err) = delete_file_if_exists(&self.praxis_home) {
    ...
    fn delete(&self) -> std::io::Result<bool> {
        ...
        let file_removed = delete_file_if_exists(&self.praxis_home)?;
...
impl AutoAuthStorage {
    fn new(praxis_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            keyring_storage: Arc::new(KeyringAuthStorage::new(praxis_home.clone(), keyring_store)),
            file_storage: Arc::new(FileAuthStorage::new(praxis_home)),
        }
    }
}
...
impl EphemeralAuthStorage {
    fn new(praxis_home: PathBuf) -> Self {
        Self { praxis_home }
    }
...
fn create_auth_storage_with_resolved_home(
    ...
    match mode {
        AuthCredentialsStoreMode::File => Arc::new(FileAuthStorage::new(praxis_home)),
        AuthCredentialsStoreMode::Keyring => {
            Arc::new(KeyringAuthStorage::new(praxis_home, keyring_store))
        }
