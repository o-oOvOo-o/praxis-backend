...
pub fn is_first_party_originator(originator_value: &str) -> bool {
    originator_value == DEFAULT_ORIGINATOR
        || originator_value == "praxis-tui"
        || originator_value == "praxis_vscode"
        || originator_value.starts_with("Praxis ")
}

pub fn is_first_party_chat_originator(originator_value: &str) -> bool {
    originator_value == "praxis_atlas" || originator_value == "praxis_chatgpt_desktop"
}

pub fn get_praxis_user_agent() -> String {
...
fn sanitize_user_agent(candidate: String, fallback: &str) -> String {
    ...
    if !sanitized.is_empty() && HeaderValue::from_str(sanitized.as_str()).is_ok() {
        tracing::warn!(
            "Sanitized Praxis user agent because provided suffix contained invalid header characters"
        );
        sanitized
    } else if HeaderValue::from_str(fallback).is_ok() {
        tracing::warn!(
            "Falling back to base Praxis user agent because provided suffix could not be sanitized"
        );
        fallback.to_string()
    } else {
        tracing::warn!(
            "Falling back to default Praxis originator because base user agent string is invalid"
        );
        originator().value
    }
}
