...
fn app_mentioned_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::AppMentioned(PraxisAppMentionedEventRequest {
        event_type: "praxis_app_mentioned",
        event_params: praxis_app_metadata(
            &tracking,
            AppInvocation {
...
fn app_used_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::AppUsed(PraxisAppUsedEventRequest {
    ...
    let event = TrackEventRequest::ThreadInitialized(ThreadInitializedEvent {
        ...
        event_params: ThreadInitializedEventParams {
            ...
            runtime: PraxisRuntimeMetadata {
                praxis_rs_version: "0.1.0".to_string(),
...
async fn initialize_caches_client_and_thread_lifecycle_publishes_once_initialized() {
    ...
    reducer
        .ingest(
            AnalyticsFact::Initialize {
                ...
                product_client_id: DEFAULT_ORIGINATOR.to_string(),
                runtime: PraxisRuntimeMetadata {
                    praxis_rs_version: "0.99.0".to_string(),
    ...
    assert_eq!(
        payload[0]["event_params"]["runtime"]["praxis_rs_version"],
        "0.99.0"
    );
...
fn plugin_used_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::PluginUsed(PraxisPluginUsedEventRequest {
        event_type: "praxis_plugin_used",
        event_params: praxis_plugin_used_metadata(&tracking, sample_plugin_metadata()),
    });
...
fn plugin_management_event_serializes_expected_shape() {
    let event = TrackEventRequest::PluginInstalled(PraxisPluginEventRequest {
        event_type: "praxis_plugin_installed",
        event_params: praxis_plugin_metadata(sample_plugin_metadata()),
    });
        ...
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::SkillInvoked(SkillInvokedInput {
                ...
                invocations: vec![SkillInvocation {
                    ...
                    skill_scope: praxis_protocol::protocol::SkillScope::User,
                    skill_path,
...
async fn reducer_ingests_app_and_plugin_facts() {
    ...
    assert_eq!(payload[0]["event_type"], "praxis_app_mentioned");
    assert_eq!(payload[1]["event_type"], "praxis_app_used");
    assert_eq!(payload[2]["event_type"], "praxis_plugin_used");
}
...
async fn reducer_ingests_plugin_state_changed_fact() {
    ...
    assert_eq!(
        payload,
        json!([{
            "event_type": "praxis_plugin_disabled",
            "event_params": {
                "plugin_id": "sample@test",
                "plugin_name": "sample",
                "marketplace_name": "test",
