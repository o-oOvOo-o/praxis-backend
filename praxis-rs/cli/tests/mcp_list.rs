...
use praxis_core::config::load_global_mcp_servers;
...
fn praxis_command(praxis_home: &Path) -> Result<assert_cmd::Command> {
    let mut cmd = assert_cmd::Command::new(praxis_utils_cargo_bin::cargo_bin("praxis")?);
...
fn list_shows_empty_state() -> Result<()> {
    ...
    let mut cmd = praxis_command(praxis_home.path())?;
...
async fn list_and_get_render_expected_output() -> Result<()> {
    ...
    let mut add = praxis_command(praxis_home.path())?;
    add.args([
        "mcp",
        "add",
        "docs",
    ...
    let mut servers = load_global_mcp_servers(praxis_home.path()).await?;
    ...
    let mut list_cmd = praxis_command(praxis_home.path())?;
    ...
    let mut list_json_cmd = praxis_command(praxis_home.path())?;
    ...
    let mut get_cmd = praxis_command(praxis_home.path())?;
    ...
    assert!(stdout.contains("remove: praxis mcp remove docs"));

    let mut get_json_cmd = praxis_command(praxis_home.path())?;
    get_json_cmd
