...
//! interactive-session layers. Direct `praxis login` intentionally does less: it preserves the
...
//! into a one-shot CLI command while still producing a durable `praxis-login.log` artifact that
...
use praxis_login::logout;
use praxis_login::run_device_code_login;
...
/// Installs a small file-backed tracing layer for direct `praxis login` flows.
///
...
/// command produce a durable `praxis-login.log` artifact without coupling it to the TUI's broader
/// telemetry and feedback initialization.
fn init_login_file_logging(config: &Config) -> Option<WorkerGuard> {
    let log_dir = match praxis_core::config::log_dir(config) {
        Ok(log_dir) => log_dir,
        Err(err) => {
    ...
    let log_path = log_dir.join("praxis-login.log");
    ...
    let env_filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| EnvFilter::new("praxis_cli=info,praxis_core=info,praxis_login=info"));
...
fn print_login_server_start(actual_port: u16, auth_url: &str) {
    eprintln!(
        "Starting local login server on http://localhost:{actual_port}.\nIf your browser did not open, navigate to this URL to authenticate:\n\n{auth_url}\n\nOn a remote or headless machine? Use `praxis login --device-auth` instead."
    );
}
...
pub fn read_api_key_from_stdin() -> String {
    ...
    if stdin.is_terminal() {
        eprintln!(
            "--with-api-key expects the API key on stdin. Try piping it, e.g. `printenv OPENAI_API_KEY | praxis login --with-api-key`."
        );
        std::process::exit(1);
    }
...
/// `praxis login` working in environments where device-code may be disabled/feature-gated.
...
pub async fn run_login_status(cli_config_overrides: CliConfigOverrides) -> ! {
    ...
    match OpenAiAccountAuth::from_auth_storage(&config.praxis_home, config.cli_auth_credentials_store_mode)
    {
...
pub async fn run_logout(cli_config_overrides: CliConfigOverrides) -> ! {
    ...
    match logout(&config.praxis_home, config.cli_auth_credentials_store_mode) {
        Ok(true) => {
