...
use praxis_mcp::mcp::auth::ResolvedMcpOAuthScopes;
...
use praxis_rmcp_client::delete_oauth_tokens;
...
#[derive(Debug, clap::Parser)]
#[command(override_usage = "praxis mcp add [OPTIONS] <NAME> (--url <URL> | -- <COMMAND>...)")]
pub struct AddArgs {
...
async fn perform_oauth_login_retry_without_scopes(
    name: &str,
    url: &str,
    store_mode: praxis_rmcp_client::OAuthCredentialsStoreMode,
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    ConfigEditsBuilder::new(&praxis_home)
        ...
        .apply()
        .await
        .with_context(|| format!("failed to write MCP servers to {}", praxis_home.display()))?;
    ...
    match oauth_login_support(&transport).await {
        ...
        McpOAuthLoginSupport::Unknown(_) => println!(
            "MCP server may or may not require login. Run `praxis mcp login {name}` to login."
        ),
    }
...
async fn run_list(config_overrides: &CliConfigOverrides, list_args: ListArgs) -> Result<()> {
    ...
    if entries.is_empty() {
        println!("No MCP servers configured yet. Try `praxis mcp add my-tool -- my-command`.");
        return Ok(());
    }
...
async fn run_get(config_overrides: &CliConfigOverrides, get_args: GetArgs) -> Result<()> {
    ...
    println!("  remove: praxis mcp remove {}", get_args.name);
