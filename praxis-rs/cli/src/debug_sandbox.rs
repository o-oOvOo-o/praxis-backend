...
pub async fn run_command_under_seatbelt(
    ...
    _praxis_linux_sandbox_exe: Option<PathBuf>,
) -> anyhow::Result<()> {
...
async fn run_command_under_sandbox(
    ...
    if let SandboxType::Windows = sandbox_type {
        #[cfg(target_os = "windows")]
        {
            ...
            use praxis_windows_sandbox::run_windows_sandbox_capture;
            ...
            let base_dir = config.praxis_home.clone();
    ...
    let mut child = match sandbox_type {
        ...
        SandboxType::Landlock => {
            ...
            let praxis_linux_sandbox_exe = config
                .praxis_linux_sandbox_exe
                .expect("praxis-linux-sandbox executable not found");
...
async fn load_debug_sandbox_config(
    ...
    load_debug_sandbox_config_with_praxis_home(
        cli_overrides,
        praxis_linux_sandbox_exe,
        full_auto,
        /*praxis_home*/ None,
...
async fn load_debug_sandbox_config_with_praxis_home(
    cli_overrides: Vec<(String, TomlValue)>,
    praxis_linux_sandbox_exe: Option<PathBuf>,
    full_auto: bool,
    ...
    let config = build_debug_sandbox_config(
        cli_overrides.clone(),
        ConfigOverrides {
            praxis_linux_sandbox_exe: praxis_linux_sandbox_exe.clone(),
            ..Default::default()
        },
        praxis_home.clone(),
    )
...
async fn build_debug_sandbox_config(
    ...
    if let Some(praxis_home) = praxis_home {
        builder = builder
            .praxis_home(praxis_home.clone())
            .fallback_cwd(Some(praxis_home));
    }
    builder.build().await
}
...
mod tests {
    ...
    fn write_permissions_profile_config(
        ...
        std::fs::write(praxis_home.path().join("config.toml"), config)?;
        Ok(())
    }
    ...
    async fn debug_sandbox_honors_active_permission_profiles() -> anyhow::Result<()> {
        ...
        let config = load_debug_sandbox_config_with_praxis_home(
            Vec::new(),
            /*praxis_linux_sandbox_exe*/ None,
            /*full_auto*/ false,
            Some(praxis_home_path),
        )
        .await?;
    ...
    async fn debug_sandbox_rejects_full_auto_for_permission_profiles() -> anyhow::Result<()> {
        ...
        let err = load_debug_sandbox_config_with_praxis_home(
            Vec::new(),
            /*praxis_linux_sandbox_exe*/ None,
            /*full_auto*/ true,
            Some(praxis_home.path().to_path_buf()),
        )
        .await
