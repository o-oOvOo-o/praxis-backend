...
impl McpProcess {
    pub async fn new(praxis_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(praxis_home, &[]).await
    }
    ...
    pub async fn new_with_env(
        ...
        let program = praxis_utils_cargo_bin::cargo_bin("praxis-mcp-server")
            .context("should find binary for praxis-mcp-server")?;
    ...
    pub async fn initialize(&mut self) -> anyhow::Result<()> {
        ...
        let originator = praxis_login::default_client::originator().value;
    ...
    pub async fn send_praxis_tool_call(
        ...
        self.send_request(
            "tools/call",
            Some(serde_json::to_value(praxis_tool_call_params)?),
        )
        .await
    }
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `praxis-mcp-server` child process.
        //
