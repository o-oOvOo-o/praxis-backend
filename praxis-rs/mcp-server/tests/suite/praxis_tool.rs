...
async fn shell_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        Some(create_expected_elicitation_request_params(
            ...
            praxis_request_id.to_string(),
            params.praxis_event_id.clone(),
            params.thread_id,
        )?)
    );
    ...
    assert_eq!(
        JsonRpcResponse {
            ...
            id: RequestId::Number(praxis_request_id),
            result: json!({
                "content": [
                    {
                        "text": "File created!",
...
fn create_expected_elicitation_request_params(
    command: Vec<String>,
    workdir: &Path,
    praxis_mcp_tool_call_id: String,
    praxis_event_id: String,
    thread_id: praxis_protocol::ThreadId,
) -> anyhow::Result<serde_json::Value> {
    let expected_message = format!(
        "Allow Praxis to run `{}` in `{}`?",
        shlex::try_join(command.iter().map(std::convert::AsRef::as_ref))?,
        workdir.to_string_lossy()
    );
    let praxis_parsed_cmd = parse_command::parse_command(&command);
    let params_json = serde_json::to_value(ExecApprovalElicitRequestParams {
        message: expected_message,
        requested_schema: json!({"type":"object","properties":{}}),
        thread_id,
        praxis_elicitation: "exec-approval".to_string(),
        praxis_mcp_tool_call_id,
        praxis_event_id,
        praxis_command: command,
        praxis_cwd: workdir.to_path_buf(),
        praxis_call_id: "call1234".to_string(),
        praxis_parsed_cmd,
    })?;
    Ok(params_json)
}
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_praxis_tool_passes_base_instructions() {
    skip_if_no_network!();

    // Apparently `#[tokio::test]` must return `()`, so we create a helper
    // function that returns `Result` so we can use `?` in favor of `unwrap`.
    if let Err(err) = praxis_tool_passes_base_instructions().await {
        panic!("failure: {err}");
    }
}

async fn praxis_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    .await??;
    assert_eq!(praxis_response.jsonrpc, JsonRpcVersion2_0);
    assert_eq!(praxis_response.id, RequestId::Number(praxis_request_id));
    assert_eq!(
        praxis_response.result,
        json!({
            "content": [
                {
                    "text": "Enjoy!",
...
/// Create a Praxis config that uses the mock server as the model provider.
/// It also uses `approval_policy = "untrusted"` so that we exercise the
/// elicitation code path for shell commands.
fn create_config_toml(praxis_home: &Path, server_uri: &str) -> std::io::Result<()> {
