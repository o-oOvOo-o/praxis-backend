//! Asynchronous worker that executes a **Praxis** tool-call inside a spawned
...
/// To adhere to MCP `tools/call` response format, include the Praxis
...
/// Run a complete Praxis session and stream events back to the client.
///
...
pub async fn run_praxis_tool_session(
        ...
        Err(e) => {
            let result = CallToolResult {
                content: vec![Content::text(format!(
                    "Failed to start Praxis session: {e}"
                ))],
                is_error: Some(true),
    ...
    // Use the original MCP request ID as the `sub_id` for the Praxis submission so that
    ...
    if let Err(e) = thread.submit_with_id(submission).await {
        ...
        running_requests_id_to_praxis_uuid.lock().await.remove(&id);
        return;
    }

    run_praxis_tool_session_inner(
        thread_id,
        thread,
        outgoing,
        id,
...
pub async fn run_praxis_tool_session_reply(
    thread_id: ThreadId,
...
async fn run_praxis_tool_session_inner(
    thread_id: ThreadId,
    thread: Arc<PraxisThread>,
    outgoing: Arc<OutgoingMessageSender>,
    request_id: RequestId,
            ...
            Ok(event) => {
                ...
                match event.msg {
                    ...
                    | EventMsg::DeprecationNotice(_) => {
                        ...
                        // events. Note that
                        // send(praxis_event_to_notification(&event)) above has
