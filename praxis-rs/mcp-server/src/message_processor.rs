...
impl MessageProcessor {
    ...
    pub(crate) fn new(
        ...
        Self {
            ...
            arg0_paths,
            thread_manager,
            running_requests_id_to_praxis_uuid: Arc::new(Mutex::new(HashMap::new())),
        }
    }
    ...
    async fn handle_initialize(
        ...
        let server_info = Implementation {
            name: "praxis-mcp-server".to_string(),
        ...
        // Preserve Praxis's existing non-spec `serverInfo.user_agent` field.
        ...
        if let serde_json::Value::Object(ref mut obj) = server_info_value {
            obj.insert("user_agent".to_string(), json!(get_praxis_user_agent()));
        }
    ...
    async fn handle_list_tools(
        ...
        let result = rmcp::model::ListToolsResult {
            meta: None,
            tools: vec![
                create_tool_for_praxis_tool_call_param(),
                create_tool_for_praxis_tool_call_reply_param(),
            ],
            next_cursor: None,
        };
    ...
    async fn handle_tool_call_codex(
        ...
        let (initial_prompt, config): (String, Config) = match arguments {
            Some(json_val) => match serde_json::from_value::<CodexToolCallParam>(json_val) {
                    ...
                    Err(e) => {
                        let result = CallToolResult {
                            content: vec![rmcp::model::Content::text(format!(
                                "Failed to load Praxis configuration from overrides: {e}"
                            ))],
                            structured_content: None,
                            is_error: Some(true),
                            meta: None,
                ...
                Err(e) => {
                    let result = CallToolResult {
                        content: vec![rmcp::model::Content::text(format!(
                            "Failed to parse configuration for Praxis tool: {e}"
                        ))],
                        structured_content: None,
                        is_error: Some(true),
                        meta: None,
        ...
        // Spawn an async task to handle the Praxis session so that we do not
        // block the synchronous message-processing loop.
        task::spawn(async move {
            // Run the Praxis session and stream events back to the client.
            crate::praxis_tool_runner::run_praxis_tool_session(
                id,
                initial_prompt,
                config,
                outgoing,
                thread_manager,
                running_requests_id_to_praxis_uuid,
            )
            .await;
        });
    }

    async fn handle_tool_call_praxis_session_reply(
        &self,
        ...
        let praxis_tool_call_reply_param: CodexToolCallReplyParam = match arguments {
            ...
            None => {
                tracing::error!(
                    "Missing arguments for praxis-reply tool-call; the `thread_id` and `prompt` fields are required."
                );
                let result = CallToolResult {
                    content: vec![rmcp::model::Content::text(
                        "Missing arguments for praxis-reply tool-call; the `thread_id` and `prompt` fields are required.",
        ...
        let thread_id = match praxis_tool_call_reply_param.get_thread_id() {
            Ok(id) => id,
            Err(e) => {
        ...
        let codex = match self.thread_manager.get_thread(thread_id).await {
            ...
            Err(_) => {
                ...
                let result = crate::praxis_tool_runner::create_call_tool_result_with_thread_id(
                    thread_id,
        ...
        let prompt = praxis_tool_call_reply_param.prompt.clone();
        tokio::spawn({
            let outgoing = outgoing.clone();
            let running_requests_id_to_praxis_uuid = running_requests_id_to_praxis_uuid.clone();
    ...
    async fn handle_cancelled_notification(&self, params: rmcp::model::CancelledNotificationParam) {
        ...
        let thread_id = {
            let map_guard = self.running_requests_id_to_praxis_uuid.lock().await;
        ...
        // Obtain the Praxis thread from the server.
        let praxis_arc = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                return;
        ...
        // Submit interrupt to Praxis.
        if let Err(e) = praxis_arc
            .submit_with_id(Submission {
                id: request_id_string,
                op: praxis_protocol::protocol::Op::Interrupt,
                trace: None,
            })
            .await
        {
            tracing::error!("Failed to submit interrupt to Praxis: {e}");
            return;
        }
        // unregister the id so we don't keep it in the map
        self.running_requests_id_to_praxis_uuid
            .lock()
            .await
            .remove(&request_id);
    }
