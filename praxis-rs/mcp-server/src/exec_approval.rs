...
pub struct ExecApprovalElicitRequestParams {
    ...
    pub praxis_command: Vec<String>,
    pub praxis_cwd: PathBuf,
    pub praxis_parsed_cmd: Vec<ParsedCommand>,
}
...
pub(crate) async fn handle_exec_approval_request(
    ...
    call_id: String,
    approval_id: String,
    praxis_parsed_cmd: Vec<ParsedCommand>,
    thread_id: ThreadId,
) {
    let escaped_command =
        shlex::try_join(command.iter().map(String::as_str)).unwrap_or_else(|_| command.join(" "));
    let message = format!(
        "Allow Praxis to run `{escaped_command}` in `{cwd}`?",
