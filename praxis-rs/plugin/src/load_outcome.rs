...
impl<M: Clone> PluginLoadOutcome<M> {
    ...
    pub fn effective_llm_manifests(&self) -> Vec<PluginLlmManifest> {
        self.plugins
            .iter()
            .filter(|plugin| plugin.is_active())
            .filter_map(|plugin| plugin.llm.clone())
            .collect()
    }
...
/// Implemented by [`PluginLoadOutcome`] so callers (e.g. skills) can depend on `praxis-plugin`
