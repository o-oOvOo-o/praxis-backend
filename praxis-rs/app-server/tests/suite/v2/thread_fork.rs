...
use praxis_app_server_protocol::ThreadStatusChangedNotification;
...
use praxis_core::append_thread_name;
...
#[tokio::test]
async fn thread_fork_inherits_persisted_source_thread_name() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let praxis_home = TempDir::new()?;
    create_config_toml(praxis_home.path(), &server.uri())?;
    ...
    let renamed = "Keep my renamed title";
    append_thread_name(praxis_home.path(), source_thread_id, renamed).await?;
    ...
    assert_eq!(thread.name.as_deref(), Some(renamed));
    let thread_json = fork_result
        .get("thread")
        .and_then(Value::as_object)
        .expect("thread/fork result.thread must be an object");
    ...
    let persisted_name = find_thread_name_by_id(praxis_home.path(), &forked_thread_id).await?;
    assert_eq!(persisted_name.as_deref(), Some(renamed));
