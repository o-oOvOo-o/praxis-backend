...
async fn initialize_uses_client_info_name_as_originator() -> Result<()> {
    ...
    let expected_praxis_home = AbsolutePathBuf::try_from(praxis_home.path().canonicalize()?)?;
    ...
    let InitializeResponse {
        user_agent,
        praxis_home: response_praxis_home,
        platform_family,
        platform_os,
    } = to_response::<InitializeResponse>(response)?;

    assert!(user_agent.starts_with("praxis_vscode/"));
    assert_eq!(response_praxis_home, expected_praxis_home);
...
async fn initialize_respects_originator_override_env_var() -> Result<()> {
    ...
    assert!(user_agent.starts_with("praxis_originator_via_env_var/"));
    assert_eq!(response_praxis_home, expected_praxis_home);
    assert_eq!(platform_family, std::env::consts::FAMILY);
    assert_eq!(platform_os, std::env::consts::OS);
    Ok(())
...
async fn turn_start_notify_payload_includes_initialize_client_name() -> Result<()> {
    ...
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    let praxis_home = TempDir::new()?;
    let notify_file = praxis_home.path().join("notify.json");
    let notify_capture = cargo_bin("praxis-app-server-test-notify-capture")?;
...
fn create_config_toml(
    ...
    create_config_toml_with_extra(praxis_home, server_uri, approval_policy, "")
}
