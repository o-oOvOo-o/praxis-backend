...
use praxis_app_server_protocol::ConfigEdit;
...
use praxis_app_server_protocol::ToolsV2;
...
use praxis_protocol::config_types::WebSearchContextSize;
use praxis_protocol::config_types::WebSearchLocation;
...
fn write_config(praxis_home: &TempDir, contents: &str) -> Result<()> {
    Ok(std::fs::write(
...
async fn config_read_includes_project_layers_for_cwd() -> Result<()> {
    let praxis_home = TempDir::new()?;
    write_config(&praxis_home, r#"model = "gpt-user""#)?;
...
"#,
    ...
    set_project_trust_level(praxis_home.path(), workspace.path(), TrustLevel::Trusted)?;
    ...
    assert_eq!(
        ...
        ConfigLayerSource::Project {
            dot_praxis_folder: project_config
        }
    );
...
"#,
    ...
    let expected_file_path =
        AbsolutePathBuf::resolve_path_against_base("config.toml", praxis_home)?;
...
"#,
    ...
    let write_id = mcp
        .send_config_value_write_request(ConfigValueWriteParams {
            file_path: Some(praxis_home.path().join("config.toml").display().to_string()),
            key_path: "model".to_string(),
            value: json!("gpt-new"),
            merge_strategy: MergeStrategy::Replace,
...
async fn config_batch_write_applies_multiple_edits() -> Result<()> {
    ...
    let praxis_home = tmp_dir.path().canonicalize()?;
    ...
    let batch_id = mcp
        .send_config_batch_write_request(ConfigBatchWriteParams {
            file_path: Some(praxis_home.join("config.toml").display().to_string()),
            edits: vec![
                ConfigEdit {
...
fn assert_layers_user_then_optional_system(
    layers: &[praxis_app_server_protocol::ConfigLayer],
