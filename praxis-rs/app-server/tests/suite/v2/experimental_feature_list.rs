...
use praxis_app_server_protocol::ExperimentalFeature;
...
use praxis_app_server_protocol::ExperimentalFeatureStage;
...
-> Result<()> {
    let praxis_home = TempDir::new()?;
    let project_cwd = praxis_home.path().join("project");
