...
async fn external_auth_refreshes_on_unauthorized() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request(praxis_app_server_protocol::ThreadStartParams {
    ...
    .await??;
    let thread = to_response::<praxis_app_server_protocol::ThreadStartResponse>(thread_resp)?;

    let turn_req = mcp
        .send_turn_start_request(praxis_app_server_protocol::TurnStartParams {
            thread_id: thread.thread.id,
            input: vec![praxis_app_server_protocol::UserInput::Text {
