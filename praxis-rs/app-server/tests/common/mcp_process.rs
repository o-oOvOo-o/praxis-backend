...
pub const DEFAULT_CLIENT_NAME: &str = "praxis-app-server-tests";
...
impl McpProcess {
    ...
    async fn new_with_env_and_args(
        ...
        let program = praxis_utils_cargo_bin::cargo_bin("praxis-app-server")
            .context("should find binary for praxis-app-server")?;
    ...
    pub async fn send_experimental_feature_enablement_set_request(
        &mut self,
        params: praxis_app_server_protocol::ExperimentalFeatureEnablementSetParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `praxis-app-server` child process.
        //
