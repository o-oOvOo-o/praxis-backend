...
use crate::praxis_message_processor::ApiVersion;
...
use praxis_app_server_protocol::AccountRateLimitsUpdatedNotification;
use praxis_app_server_protocol::AdditionalPermissionProfile as V2AdditionalPermissionProfile;
use praxis_app_server_protocol::AgentMessageDeltaNotification;
use praxis_app_server_protocol::ApplyPatchApprovalParams;
use praxis_app_server_protocol::ApplyPatchApprovalResponse;
use praxis_app_server_protocol::CodexErrorInfo as V2CodexErrorInfo;
use praxis_app_server_protocol::CollabAgentState as V2CollabAgentStatus;
use praxis_app_server_protocol::CollabAgentTool;
use praxis_app_server_protocol::CollabAgentToolCallStatus as V2CollabToolCallStatus;
use praxis_app_server_protocol::CommandAction as V2ParsedCommand;
use praxis_app_server_protocol::CommandExecutionApprovalDecision;
use praxis_app_server_protocol::CommandExecutionOutputDeltaNotification;
use praxis_app_server_protocol::CommandExecutionRequestApprovalParams;
use praxis_app_server_protocol::CommandExecutionRequestApprovalResponse;
use praxis_app_server_protocol::CommandExecutionSource;
use praxis_app_server_protocol::CommandExecutionStatus;
use praxis_app_server_protocol::ContextCompactedNotification;
use praxis_app_server_protocol::DeprecationNoticeNotification;
use praxis_app_server_protocol::DynamicToolCallOutputContentItem;
use praxis_app_server_protocol::DynamicToolCallParams;
use praxis_app_server_protocol::DynamicToolCallStatus;
use praxis_app_server_protocol::ErrorNotification;
use praxis_app_server_protocol::ExecCommandApprovalParams;
use praxis_app_server_protocol::ExecCommandApprovalResponse;
use praxis_app_server_protocol::ExecPolicyAmendment as V2ExecPolicyAmendment;
use praxis_app_server_protocol::FileChangeApprovalDecision;
use praxis_app_server_protocol::FileChangeOutputDeltaNotification;
use praxis_app_server_protocol::FileChangeRequestApprovalParams;
use praxis_app_server_protocol::FileChangeRequestApprovalResponse;
use praxis_app_server_protocol::FileUpdateChange;
use praxis_app_server_protocol::GrantedPermissionProfile as V2GrantedPermissionProfile;
use praxis_app_server_protocol::GuardianApprovalReview;
use praxis_app_server_protocol::GuardianApprovalReviewStatus;
use praxis_app_server_protocol::HookCompletedNotification;
use praxis_app_server_protocol::HookStartedNotification;
use praxis_app_server_protocol::InterruptConversationResponse;
use praxis_app_server_protocol::ItemCompletedNotification;
use praxis_app_server_protocol::ItemGuardianApprovalReviewCompletedNotification;
use praxis_app_server_protocol::ItemGuardianApprovalReviewStartedNotification;
...
use praxis_app_server_protocol::ModelReroutedNotification;
use praxis_app_server_protocol::NetworkApprovalContext as V2NetworkApprovalContext;
use praxis_app_server_protocol::NetworkPolicyAmendment as V2NetworkPolicyAmendment;
use praxis_app_server_protocol::NetworkPolicyRuleAction as V2NetworkPolicyRuleAction;
use praxis_app_server_protocol::PatchApplyStatus;
use praxis_app_server_protocol::PermissionsRequestApprovalParams;
use praxis_app_server_protocol::PermissionsRequestApprovalResponse;
use praxis_app_server_protocol::PlanDeltaNotification;
use praxis_app_server_protocol::RawResponseItemCompletedNotification;
use praxis_app_server_protocol::ReasoningSummaryPartAddedNotification;
use praxis_app_server_protocol::ReasoningSummaryTextDeltaNotification;
use praxis_app_server_protocol::ReasoningTextDeltaNotification;
...
use praxis_app_server_protocol::SkillsChangedNotification;
...
use praxis_app_server_protocol::TerminalInteractionNotification;
...
use praxis_app_server_protocol::ThreadRealtimeClosedNotification;
use praxis_app_server_protocol::ThreadRealtimeErrorNotification;
use praxis_app_server_protocol::ThreadRealtimeItemAddedNotification;
use praxis_app_server_protocol::ThreadRealtimeOutputAudioDeltaNotification;
use praxis_app_server_protocol::ThreadRealtimeStartedNotification;
use praxis_app_server_protocol::ThreadRealtimeTranscriptUpdatedNotification;
...
use praxis_app_server_protocol::ToolRequestUserInputOption;
use praxis_app_server_protocol::ToolRequestUserInputParams;
use praxis_app_server_protocol::ToolRequestUserInputQuestion;
use praxis_app_server_protocol::ToolRequestUserInputResponse;
use praxis_app_server_protocol::Turn;
use praxis_app_server_protocol::TurnCompletedNotification;
use praxis_app_server_protocol::TurnDiffUpdatedNotification;
...
use praxis_app_server_protocol::convert_patch_changes;
    ...
    match msg {
        ...
        EventMsg::Error(ev) => {
            ...
            let turn_error = TurnError {
                message: ev.message,
                praxis_error_info: ev.praxis_error_info.map(V2CodexErrorInfo::from),
    ...
    let notification = ItemCompletedNotification {
        ...
        item: ThreadItem::HookPrompt {
            ...
            fragments: hook_prompt
                .fragments
                .into_iter()
                .map(praxis_app_server_protocol::HookPromptFragment::from)
                .collect(),
        },
    };
    outgoing
    ...
    let response = serde_json::from_value::<PermissionsRequestApprovalResponse>(value)
        .unwrap_or_else(|err| {
            ...
            PermissionsRequestApprovalResponse {
                ...
                scope: praxis_app_server_protocol::PermissionGrantScope::Turn,
            }
        });
...
fn collab_resume_end_item(
    ...
    let status = match &end_event.status {
        praxis_protocol::protocol::AgentStatus::Errored(_)
        | praxis_protocol::protocol::AgentStatus::NotFound => V2CollabToolCallStatus::Failed,
        _ => V2CollabToolCallStatus::Completed,
    };
    let receiver_id = end_event.receiver_thread_id.to_string();
    let agents_states = [(
...
mod tests {
        ...
        match notification {
            ServerNotification::ItemGuardianApprovalReviewCompleted(payload) => {
                ...
                assert_eq!(
                    ...
                    Some(praxis_app_server_protocol::GuardianRiskLevel::High)
                );
    ...
    fn collab_resume_end_maps_to_item_completed_resume_agent() {
        ...
        let expected = ThreadItem::CollabAgentToolCall {
            ...
            agents_states: [(
                receiver_id,
                V2CollabAgentStatus::from(praxis_protocol::protocol::AgentStatus::NotFound),
            )]
            .into_iter()
            .collect(),
        };
    ...
    async fn test_handle_error_records_message() -> Result<()> {
        ...
        handle_error(
            ...
            TurnError {
                ...
                praxis_error_info: Some(V2CodexErrorInfo::InternalServerError),
                additional_details: None,
            },
            &thread_state,
        )
    ...
    async fn test_handle_turn_complete_emits_failed_with_error() -> Result<()> {
        ...
        handle_error(
            ...
            TurnError {
                ...
                praxis_error_info: Some(V2CodexErrorInfo::Other),
                additional_details: None,
            },
            &thread_state,
        )
    ...
    async fn test_handle_turn_complete_emits_error_multiple_turns() -> Result<()> {
        ...
        handle_error(
            ...
            TurnError {
                ...
                praxis_error_info: Some(V2CodexErrorInfo::BadRequest),
                additional_details: None,
            },
            &thread_state,
        )
            ...
            OutgoingMessage::AppServerNotification(ServerNotification::ItemCompleted(
                ...
                assert_eq!(
                    ...
                    ThreadItem::HookPrompt {
                        ...
                        fragments: vec![
                            praxis_app_server_protocol::HookPromptFragment {
