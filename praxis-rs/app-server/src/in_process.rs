...
//!
//! # Relationship to `praxis-app-server-client`
//!
//! This module provides the low-level runtime handle ([`InProcessClientHandle`]).
//! Higher-level callers (TUI, exec) should go through `praxis-app-server-client`,
...
/// through `praxis-app-server-client`, which adds worker-task buffering,
