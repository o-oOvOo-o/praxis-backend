...
use praxis_app_server_protocol::ConfigRequirements;
...
use praxis_app_server_protocol::ExperimentalFeatureEnablementSetResponse;
use praxis_app_server_protocol::JSONRPCErrorError;
use praxis_app_server_protocol::NetworkDomainPermission;
use praxis_app_server_protocol::NetworkRequirements;
use praxis_app_server_protocol::NetworkUnixSocketPermission;
...
fn map_requirements_toml_to_api(requirements: ConfigRequirementsToml) -> ConfigRequirements {
    ConfigRequirements {
        allowed_approval_policies: requirements.allowed_approval_policies.map(|policies| {
            policies
                .into_iter()
                .map(praxis_app_server_protocol::AskForApproval::from)
                .collect()
        }),
...
) -> praxis_app_server_protocol::ResidencyRequirement {
    match residency {
        CoreResidencyRequirement::Us => praxis_app_server_protocol::ResidencyRequirement::Us,
    }
}
...
fn map_network_requirements_to_api(
    ...
    let allowed_domains = network
        .domains
        .as_ref()
        .and_then(praxis_core::config_loader::NetworkDomainPermissionsToml::allowed_domains);
    let denied_domains = network
        .domains
        .as_ref()
        .and_then(praxis_core::config_loader::NetworkDomainPermissionsToml::denied_domains);
    let allow_unix_sockets = network
        .unix_sockets
        .as_ref()
        .map(praxis_core::config_loader::NetworkUnixSocketPermissionsToml::allow_unix_sockets)
...
mod tests {
    ...
    fn map_requirements_toml_to_api_converts_core_enums() {
        ...
        assert_eq!(
            ...
            Some(vec![
                praxis_app_server_protocol::AskForApproval::Never,
                praxis_app_server_protocol::AskForApproval::OnRequest,
            ])
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            Some(praxis_app_server_protocol::ResidencyRequirement::Us),
        );
        assert_eq!(
            mapped.network,
    ...
    async fn batch_write_reloads_user_config_when_requested() {
        ...
        let response = config_api
            .batch_write(ConfigBatchWriteParams {
                edits: vec![praxis_app_server_protocol::ConfigEdit {
                    key_path: "model".to_string(),
                    value: json!("gpt-5"),
                    merge_strategy: praxis_app_server_protocol::MergeStrategy::Replace,
                }],
        ...
        assert_eq!(
            response,
            ConfigWriteResponse {
                status: praxis_app_server_protocol::WriteStatus::Ok,
