...
use praxis_app_server_protocol::CommandExecOutputDeltaNotification;
use praxis_app_server_protocol::CommandExecOutputStream;
use praxis_app_server_protocol::CommandExecResizeParams;
use praxis_app_server_protocol::CommandExecResizeResponse;
use praxis_app_server_protocol::CommandExecResponse;
use praxis_app_server_protocol::CommandExecTerminalSize;
use praxis_app_server_protocol::CommandExecTerminateParams;
use praxis_app_server_protocol::CommandExecTerminateResponse;
use praxis_app_server_protocol::CommandExecWriteParams;
use praxis_app_server_protocol::CommandExecWriteResponse;
use praxis_app_server_protocol::JSONRPCErrorError;
use praxis_app_server_protocol::ServerNotification;
use praxis_core::bytes_to_string_smart;
use praxis_core::config::StartedNetworkProxy;
use praxis_core::exec::DEFAULT_EXEC_COMMAND_TIMEOUT_MS;
use praxis_core::exec::ExecExpiration;
use praxis_core::exec::IO_DRAIN_TIMEOUT_MS;
use praxis_core::sandboxing::ExecRequest;
use praxis_sandboxing::SandboxType;
use praxis_utils_pty::DEFAULT_OUTPUT_BYTES_CAP;
use praxis_utils_pty::ProcessHandle;
use praxis_utils_pty::SpawnedProcess;
    ...
    pub(crate) async fn start(
        ...
        if matches!(exec_request.sandbox, SandboxType::WindowsRestrictedToken) {
            ...
            tokio::spawn(async move {
                ...
                match praxis_core::sandboxing::execute_env(
                    exec_request,
                    /*stdout_stream*/ None,
                )
                .await
        ...
        let spawned = if tty {
            praxis_utils_pty::spawn_pty_process(
                program,
                args,
                cwd.as_path(),
                &env,
            ...
            praxis_utils_pty::spawn_pipe_process(program, args, cwd.as_path(), &env, &arg0).await
        } else {
            praxis_utils_pty::spawn_pipe_process_no_stdin(program, args, cwd.as_path(), &env, &arg0)
                .await
        };
...
mod tests {
    ...
    fn windows_sandbox_exec_request() -> ExecRequest {
        ...
        ExecRequest::new(
            ...
            /*network*/ None,
            ExecExpiration::DefaultTimeout,
            praxis_core::exec::ExecCapturePolicy::ShellTool,
        ...
        let err = manager
            .start(StartCommandExecParams {
                ...
                request_id: ConnectionRequestId {
                    ...
                    request_id: praxis_app_server_protocol::RequestId::Integer(42),
                },
    ...
    async fn windows_sandbox_non_streaming_exec_uses_execution_path() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: praxis_app_server_protocol::RequestId::Integer(99),
        };
    ...
    async fn cancellation_expiration_keeps_process_alive_until_terminated() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: praxis_app_server_protocol::RequestId::Integer(100),
        };
        let sandbox_policy = SandboxPolicy::ReadOnly {
            access: ReadOnlyAccess::FullAccess,
            network_access: false,
    ...
    async fn windows_sandbox_process_ids_reject_terminate_requests() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: praxis_app_server_protocol::RequestId::Integer(2),
        };
        let process_id = ConnectionProcessId {
            connection_id: request_id.connection_id,
    ...
    async fn dropped_control_request_is_reported_as_not_running() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: praxis_app_server_protocol::RequestId::Integer(3),
        };
