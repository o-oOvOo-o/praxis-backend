...
mod tests {
    ...
    async fn init_removes_legacy_state_db_files() {
        ...
        for suffix in ["", "-wal", "-shm", "-journal"] {
            let path = praxis_home.join(format!("{unversioned_name}{suffix}"));
            tokio::fs::write(path, b"legacy")
                .await
                .expect("write legacy");
            let old_version_path = praxis_home.join(format!(
        ...
        let unrelated_path = praxis_home.join("state.sqlite_backup");
        tokio::fs::write(&unrelated_path, b"keep")
            .await
            .expect("write unrelated");
        let numeric_path = praxis_home.join("123");
        ...
        for suffix in ["", "-wal", "-shm", "-journal"] {
            let legacy_path = praxis_home.join(format!("{unversioned_name}{suffix}"));
            assert_eq!(
        ...
        assert_eq!(
            tokio::fs::try_exists(praxis_home.join(current_name))
                .await
