...
pub struct ThreadMetadata {
    ...
    /// The latest estimated per-turn USD cost in microdollars.
...
impl ThreadMetadata {
    ...
    pub fn diff_fields(&self, other: &Self) -> Vec<&'static str> {
        ...
        if self.total_cost_micros != other.total_cost_micros {
            diffs.push("total_cost_micros");
        }
        if self.last_cost_micros != other.last_cost_micros {
            diffs.push("last_cost_micros");
        }
        if self.selfwork_plan_path != other.selfwork_plan_path {
            diffs.push("selfwork_plan_path");
        }
...
pub(crate) struct ThreadRow {
    ...
    selfwork_plan_path: Option<String>,
...
impl ThreadRow {
    pub(crate) fn try_from_row(row: &SqliteRow) -> Result<Self> {
        Ok(Self {
            ...
            selfwork_plan_path: row.try_get("selfwork_plan_path")?,
...
impl TryFrom<ThreadRow> for ThreadMetadata {
    ...
    fn try_from(row: ThreadRow) -> std::result::Result<Self, Self::Error> {
        ...
        Ok(Self {
            ...
            total_cost_micros,
            last_cost_micros,
            selfwork_plan_path: selfwork_plan_path
