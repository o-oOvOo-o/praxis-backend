...
impl StateRuntime {
    /// Initialize the state runtime using the provided Praxis home and default provider.
    ///
    /// This opens (and migrates) the SQLite databases under `praxis_home`,
    /// keeping logs in a dedicated file to reduce lock contention with the
    /// rest of the state store.
    pub async fn init(praxis_home: PathBuf, default_provider: String) -> anyhow::Result<Arc<Self>> {
    ...
    /// Return the configured Praxis home directory for this runtime.
    pub fn praxis_home(&self) -> &Path {
...
pub fn state_db_path(praxis_home: &Path) -> PathBuf {
    praxis_home.join(state_db_filename())
}
...
pub fn logs_db_path(praxis_home: &Path) -> PathBuf {
    praxis_home.join(logs_db_filename())
}
...
async fn remove_legacy_db_files(
    ...
    let mut entries = match tokio::fs::read_dir(praxis_home).await {
        Ok(entries) => entries,
        Err(err) => {
            warn!(
                "failed to read praxis_home for {db_label} db cleanup {}: {err}",
                praxis_home.display(),
            );
            return;
        }
