...
use praxis_state::LogQuery;
use praxis_state::LogRow;
...
async fn main() -> anyhow::Result<()> {
    ...
    let praxis_home = db_path
        .parent()
        .map(ToOwned::to_owned)
        .unwrap_or_else(|| PathBuf::from("."));
    let runtime = StateRuntime::init(praxis_home, "logs-client".to_string()).await?;
...
fn resolve_db_path(args: &Args) -> anyhow::Result<PathBuf> {
    ...
    let praxis_home = args.praxis_home.clone().unwrap_or_else(default_praxis_home);
    Ok(praxis_state::logs_db_path(praxis_home.as_path()))
}
