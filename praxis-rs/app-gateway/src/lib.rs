...
use praxis_app_gateway_protocol::TextPosition as AppTextPosition;
use praxis_app_gateway_protocol::TextRange as AppTextRange;
use praxis_core::ExecPolicyError;
use praxis_core::check_execpolicy_for_warnings;
use praxis_core::config_loader::ConfigLoadError;
use praxis_core::config_loader::TextRange as CoreTextRange;
...
mod dynamic_tools;
mod error_code;
mod external_agent_migration_api;
mod fs_api;
mod fs_watch;
mod fuzzy_file_search;
pub mod in_process;
mod json_rpc_error;
mod mcp_tool_event_bridge;
...
pub use crate::transport::AppGatewayTransport;
pub use crate::transport::auth::AppGatewayWebsocketAuthArgs;
pub use crate::transport::auth::AppGatewayWebsocketAuthSettings;
...
fn project_config_warning(config: &Config) -> Option<ConfigWarningNotification> {
    ...
    for layer in config.config_layer_stack.get_layers(
        ...
        if let ConfigLayerSource::Project { dot_praxis_folder } = &layer.name {
...
pub async fn run_main(
    ...
    run_main_with_transport(
        ...
        AppGatewayTransport::Stdio,
        SessionSource::AppGateway,
        AppGatewayWebsocketAuthSettings::default(),
    )
    .await
}
...
pub async fn run_main_with_transport(
    ...
    auth: AppGatewayWebsocketAuthSettings,
) -> IoResult<()> {
        ...
        .await
    {
        Ok(config) => {
            ...
            match effective_toml.try_into() {
                Ok(config_toml) => {
                    if let Err(err) = praxis_core::personality_migration::maybe_migrate_personality(
                        &config.praxis_home,
                        &config_toml,
                    )
                    .await
    ...
    if let Some(warning) = praxis_core::config::system_bwrap_warning() {
        config_warnings.push(ConfigWarningNotification {
            summary: warning,
            details: None,
            path: None,
    ...
    let feedback = PraxisFeedback::new();

    let otel = praxis_core::otel_init::build_provider(
        &config,
        env!("CARGO_PKG_VERSION"),
        Some("praxis-app-gateway"),
        default_analytics_enabled,
    )
    .map_err(|e| {
        std::io::Error::new(
    ...
    let log_db = praxis_rollout::state_db::get_state_runtime(&config)
        .await
        .map(log_db::start);
    let log_db_layer = log_db
        .clone()
        .and_then(|layer| log_db::default_filter().map(|filter| layer.with_filter(filter)));
    ...
    let single_client_mode = matches!(&transport, AppGatewayTransport::Stdio);
    ...
    match transport {
        ...
        AppGatewayTransport::WebSocket { bind_address } => {
...
fn analytics_rpc_transport(transport: AppGatewayTransport) -> AppGatewayRpcTransport {
    match transport {
        AppGatewayTransport::Stdio => AppGatewayRpcTransport::Stdio,
        AppGatewayTransport::WebSocket { .. } => AppGatewayRpcTransport::Websocket,
    }
}
