...
use crate::external_agent_migration_api::ExternalAgentMigrationApi;
...
use crate::praxis_message_processor::PraxisMessageProcessorArgs;
...
use praxis_app_gateway_protocol::ExperimentalApi;
use praxis_app_gateway_protocol::ExperimentalFeatureEnablementSetParams;
use praxis_app_gateway_protocol::ExternalAgentConfigDetectParams;
use praxis_app_gateway_protocol::ExternalAgentConfigImportParams;
...
use praxis_login::AuthMode as LoginAuthMode;
use praxis_login::auth::ExternalAuth;
use praxis_login::auth::ExternalAuthRefreshContext;
use praxis_login::auth::ExternalAuthRefreshReason;
use praxis_login::auth::ExternalAuthTokens;
use praxis_login::default_client::SetOriginatorError;
use praxis_login::default_client::USER_AGENT_SUFFIX;
...
pub(crate) struct MessageProcessor {
    outgoing: Arc<OutgoingMessageSender>,
    praxis_message_processor: PraxisMessageProcessor,
    config_api: ConfigApi,
    external_agent_migration_api: ExternalAgentMigrationApi,
    fs_api: FsApi,
...
pub(crate) struct MessageProcessorArgs {
    ...
    pub(crate) enable_praxis_api_key_env: bool,
...
impl MessageProcessor {
    ...
    pub(crate) fn new(args: MessageProcessorArgs) -> Self {
        ...
        let praxis_message_processor = PraxisMessageProcessor::new(PraxisMessageProcessorArgs {
        ...
        // Keep plugin startup warmups aligned at app-gateway startup.
        ...
        let external_agent_migration_api =
            ExternalAgentMigrationApi::new(config.praxis_home.clone());
        let fs_api = FsApi::default();
        let fs_watch_manager = FsWatchManager::new(outgoing.clone());

        Self {
            outgoing,
            praxis_message_processor,
            config_api,
            external_agent_migration_api,
            fs_api,
            auth_manager,
    ...
    pub(crate) async fn process_request(
        ...
        tracing::trace!(
            ...
            "app-gateway request: {request_method}"
        );
        ...
        let request_span =
            crate::app_gateway_tracing::request_span(&request, transport, connection_id, session);
        ...
        Self::run_request_with_context(
            ...
            async {
                ...
                let praxis_request = match serde_json::from_value::<ClientRequest>(request_json) {
                    Ok(praxis_request) => praxis_request,
                    Err(err) => {
                        let error = JSONRPCErrorError {
                            code: INVALID_REQUEST_ERROR_CODE,
                            message: format!("Invalid request: {err}"),
    ...
    pub(crate) async fn process_client_request(
        ...
        let request_span =
            crate::app_gateway_tracing::typed_request_span(&request, connection_id, session);
        let request_context =
            RequestContext::new(request_id.clone(), request_span, /*parent_trace*/ None);
        tracing::trace!(
            ?connection_id,
            request_id = ?request_id.request_id,
            "app-gateway typed request"
        );
        Self::run_request_with_context(
            Arc::clone(&self.outgoing),
            request_context.clone(),
    ...
    pub(crate) fn thread_created_receiver(&self) -> broadcast::Receiver<ThreadId> {
        self.praxis_message_processor.thread_created_receiver()
    }
    ...
    pub(crate) async fn drain_background_tasks(&self) {
        self.praxis_message_processor.drain_background_tasks().await;
    }

    pub(crate) async fn cancel_active_login(&self) {
        self.praxis_message_processor.cancel_active_login().await;
    }
    ...
    pub(crate) async fn shutdown_threads(&self) {
        self.praxis_message_processor.shutdown_threads().await;
    }
    ...
    async fn handle_client_request(
        ...
        match praxis_request {
            // Handle Initialize internally so PraxisMessageProcessor does not have to concern
            ...
            ClientRequest::Initialize { request_id, params } => {
                ...
                let user_agent = get_praxis_user_agent();
                let praxis_home = match self.config.praxis_home.clone().try_into() {
        ...
        if let Some(reason) = praxis_request.experimental_reason()
        ...
        match praxis_request {
            ...
            ClientRequest::ExternalAgentConfigDetect { request_id, params } => {
                self.handle_external_agent_migration_detect(
                    ConnectionRequestId {
                        connection_id,
                        request_id,
                    },
            ...
            ClientRequest::ExternalAgentConfigImport { request_id, params } => {
                self.handle_external_agent_migration_import(
                    ConnectionRequestId {
                        connection_id,
                        request_id,
                    },
            ...
            other => {
                ...
                self.praxis_message_processor
                    .process_request(
                        connection_id,
                        other,
                        session.app_gateway_client_name.clone(),
                        request_context,
                    )
                    .boxed()
                    .await;
    ...
    async fn handle_config_read(&self, request_id: ConnectionRequestId, params: ConfigReadParams) {
        self.send_result_response(request_id, self.config_api.read(params).await)
            .await;
    }
    ...
    async fn handle_experimental_feature_enablement_set(
        ...
        {
            Ok(response) => {
                self.praxis_message_processor.clear_plugin_related_caches();
                self.praxis_message_processor
    ...
    async fn handle_external_agent_migration_detect(
        ...
        self.send_result_response(
            request_id,
            self.external_agent_migration_api.detect(params).await,
        )
        .await;
    }

    async fn handle_external_agent_migration_import(
        &self,
        request_id: ConnectionRequestId,
        ...
        self.send_result_response(
            request_id,
            self.external_agent_migration_api.import(params).await,
        )
        .await;
    }

    async fn handle_fs_read_file(&self, request_id: ConnectionRequestId, params: FsReadFileParams) {
        self.send_result_response(request_id, self.fs_api.read_file(params).await)
            .await;
    }
    ...
    async fn handle_fs_write_file(
        ...
        self.send_result_response(request_id, self.fs_api.write_file(params).await)
            .await;
    }
    ...
    async fn handle_fs_create_directory(
        ...
        self.send_result_response(request_id, self.fs_api.create_directory(params).await)
            .await;
    }
    ...
    async fn handle_fs_get_metadata(
        ...
        self.send_result_response(request_id, self.fs_api.get_metadata(params).await)
            .await;
    }
    ...
    async fn handle_fs_read_directory(
        ...
        self.send_result_response(request_id, self.fs_api.read_directory(params).await)
            .await;
    }

    async fn handle_fs_remove(&self, request_id: ConnectionRequestId, params: FsRemoveParams) {
        self.send_result_response(request_id, self.fs_api.remove(params).await)
            .await;
    }

    async fn handle_fs_copy(&self, request_id: ConnectionRequestId, params: FsCopyParams) {
        self.send_result_response(request_id, self.fs_api.copy(params).await)
            .await;
    }
    ...
    async fn send_result_response<T: serde::Serialize>(
        &self,
        request_id: ConnectionRequestId,
        result: std::result::Result<T, JSONRPCErrorError>,
    ) {
