...
use crate::json_rpc_error::invalid_params;
...
impl PraxisMessageProcessor {
    pub(super) async fn exec_one_off_command(
        ...
        if params.command.is_empty() {
            let error = invalid_request("command must not be empty");
        ...
        if size.is_some() && !tty {
            let error = invalid_params("command/exec size requires tty: true");
            self.outgoing.send_error(request, error).await;
            return;
        }
        ...
        if disable_timeout && timeout_ms.is_some() {
            let error = invalid_params("command/exec cannot set both timeoutMs and disableTimeout");
            self.outgoing.send_error(request, error).await;
            return;
        }
        ...
        let timeout_ms = match timeout_ms {
            Some(timeout_ms) => match u64::try_from(timeout_ms) {
                ...
                Err(_) => {
                    let error = invalid_params(format!(
        ...
        let started_network_proxy = match self.config.permissions.network.as_ref() {
                ...
                .await
            {
                ...
                Err(err) => {
                    let error =
                        internal_error(format!("failed to start managed network proxy: {err}"));
                    self.outgoing.send_error(request, error).await;
                    return;
                }
            },
        ...
        let (
            ...
            Some(policy) => match self.config.permissions.sandbox_policy.can_set(&policy) {
                ...
                Err(err) => {
                    let error = invalid_request(format!("invalid sandbox policy: {err}"));
                    self.outgoing.send_error(request, error).await;
                    return;
                }
            },
        ...
        match praxis_core::exec::build_exec_request(
            ...
            Err(err) => {
                let error = internal_error(format!("exec failed: {err}"));
                self.outgoing.send_error(request, error).await;
            }
        }
    }
    ...
    pub(super) async fn command_exec_write(
        ...
        let result = self
            ...
            .await;
        self.send_result_response(request_id, result).await;
    }
