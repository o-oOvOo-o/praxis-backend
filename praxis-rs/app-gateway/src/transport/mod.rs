...
pub enum AppGatewayTransport {
    Stdio,
    WebSocket { bind_address: SocketAddr },
}

#[derive(Debug, Clone, Eq, PartialEq)]
pub enum AppGatewayTransportParseError {
    UnsupportedListenUrl(String),
    InvalidWebSocketListenUrl(String),
}

impl std::fmt::Display for AppGatewayTransportParseError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            AppGatewayTransportParseError::UnsupportedListenUrl(listen_url) => write!(
                f,
                "unsupported --listen URL `{listen_url}`; expected `stdio://` or `ws://IP:PORT`"
            ),
            AppGatewayTransportParseError::InvalidWebSocketListenUrl(listen_url) => write!(
                f,
...
impl std::error::Error for AppGatewayTransportParseError {}

impl AppGatewayTransport {
    pub const DEFAULT_LISTEN_URL: &'static str = "stdio://";

    pub fn from_listen_url(listen_url: &str) -> Result<Self, AppGatewayTransportParseError> {
        ...
        if let Some(socket_addr) = listen_url.strip_prefix("ws://") {
            let bind_address = socket_addr.parse::<SocketAddr>().map_err(|_| {
                AppGatewayTransportParseError::InvalidWebSocketListenUrl(listen_url.to_string())
            })?;
            return Ok(Self::WebSocket { bind_address });
        }

        Err(AppGatewayTransportParseError::UnsupportedListenUrl(
            listen_url.to_string(),
        ))
    }
}

impl FromStr for AppGatewayTransport {
    type Err = AppGatewayTransportParseError;
...
mod tests {
    ...
    fn app_gateway_transport_parses_stdio_listen_url() {
        let transport =
            AppGatewayTransport::from_listen_url(AppGatewayTransport::DEFAULT_LISTEN_URL)
                .expect("stdio listen URL should parse");
        assert_eq!(transport, AppGatewayTransport::Stdio);
    }

    #[test]
    fn app_gateway_transport_parses_websocket_listen_url() {
        let transport = AppGatewayTransport::from_listen_url("ws://127.0.0.1:1234")
            .expect("websocket listen URL should parse");
        assert_eq!(
            transport,
            AppGatewayTransport::WebSocket {
    ...
    #[test]
    fn app_gateway_transport_rejects_invalid_websocket_listen_url() {
        let err = AppGatewayTransport::from_listen_url("ws://localhost:1234")
    ...
    #[test]
    fn app_gateway_transport_rejects_unsupported_listen_url() {
        let err = AppGatewayTransport::from_listen_url("http://127.0.0.1:1234")
    ...
    async fn enqueue_incoming_request_returns_overload_error_when_queue_is_full() {
        ...
        let first_message =
            JSONRPCMessage::Notification(praxis_app_gateway_protocol::JSONRPCNotification {
        ...
        let request = JSONRPCMessage::Request(praxis_app_gateway_protocol::JSONRPCRequest {
            id: praxis_app_gateway_protocol::RequestId::Integer(7),
    ...
    async fn enqueue_incoming_response_waits_instead_of_dropping_when_queue_is_full() {
        ...
        let response = JSONRPCMessage::Response(praxis_app_gateway_protocol::JSONRPCResponse {
            id: praxis_app_gateway_protocol::RequestId::Integer(7),
            result: json!({"ok": true}),
        });
    ...
    async fn enqueue_incoming_request_does_not_block_when_writer_queue_is_full() {
        ...
        transport_event_tx
            .send(TransportEvent::IncomingMessage {
                ...
                message: JSONRPCMessage::Notification(
                    praxis_app_gateway_protocol::JSONRPCNotification {
                        method: "initialized".to_string(),
                        params: None,
                    },
                ),
        ...
        writer_tx
            .send(QueuedOutgoingMessage::new(
                OutgoingMessage::AppGatewayNotification(ServerNotification::ConfigWarning(
        ...
        route_outgoing_envelope(
            ...
            OutgoingEnvelope::ToConnection {
                ...
                message: OutgoingMessage::Request(ServerRequest::CommandExecutionRequestApproval {
                    ...
                    params: praxis_app_gateway_protocol::CommandExecutionRequestApprovalParams {
                        ...
                        additional_permissions: Some(
                            praxis_app_gateway_protocol::AdditionalPermissionProfile {
                                network: None,
                                file_system: Some(
    ...
    async fn broadcast_does_not_block_on_slow_connection() {
        ...
        let queued_message = OutgoingMessage::AppGatewayNotification(
            ServerNotification::ConfigWarning(ConfigWarningNotification {
        ...
        let broadcast_message = OutgoingMessage::AppGatewayNotification(
            ServerNotification::ConfigWarning(ConfigWarningNotification {
