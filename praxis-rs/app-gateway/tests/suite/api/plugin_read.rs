...
[plugins."demo-plugin@praxis-curated"]
enabled = true
"#,
    )?;
    write_installed_plugin(&praxis_home, "praxis-curated", "demo-plugin")?;
    ...
    assert_eq!(response.plugin.marketplace_name, "praxis-curated");
    assert_eq!(response.plugin.marketplace_path, marketplace_path);
    assert_eq!(response.plugin.summary.id, "demo-plugin@praxis-curated");
...
async fn plugin_read_accepts_legacy_string_default_prompt() -> Result<()> {
  ...
  "interface": {
    ...
    write_plugins_enabled_config(&praxis_home)?;
...
async fn plugin_read_returns_invalid_request_when_plugin_manifest_is_missing() -> Result<()> {
  ...
  "plugins": [
    ...
    assert!(
        err.error
            .message
            .contains("missing or invalid .praxis-plugin/plugin.json")
    );
    Ok(())
}
...
fn write_plugins_enabled_config(praxis_home: &TempDir) -> Result<()> {
    std::fs::write(
        praxis_home.path().join("config.toml"),
        r#"[features]
plugins = true
