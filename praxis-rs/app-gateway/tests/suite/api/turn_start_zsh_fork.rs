...
// `app-gateway/tests/suite/zsh` via DotSlash on first use, so `dotslash` and
...
#[tokio::test]
async fn turn_start_shell_zsh_fork_executes_command() -> Result<()> {
    ...
    let mut mcp = create_zsh_test_mcp_process(&praxis_home, &workspace).await?;
    ...
    let turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            model: Some("mock-model".to_string()),
            effort: Some(praxis_protocol::openai_models::ReasoningEffort::Medium),
            summary: Some(praxis_protocol::config_types::ReasoningSummary::Auto),
            ..Default::default()
        })
        .await?;
...
#[tokio::test]
async fn turn_start_shell_zsh_fork_exec_approval_decline() -> Result<()> {
    skip_if_no_network!(Ok(()));
...
#[tokio::test]
async fn turn_start_shell_zsh_fork_exec_approval_cancel() -> Result<()> {
    skip_if_no_network!(Ok(()));
...
#[tokio::test]
async fn turn_start_shell_zsh_fork_subcommand_decline_marks_parent_declined() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            cwd: Some(workspace.clone()),
            approval_policy: Some(praxis_app_gateway_protocol::AskForApproval::UnlessTrusted),
...
async fn create_zsh_test_mcp_process(praxis_home: &Path, zdotdir: &Path) -> Result<McpProcess> {
    let zdotdir = zdotdir.to_string_lossy().into_owned();
    McpProcess::new_with_env(praxis_home, &[("ZDOTDIR", Some(zdotdir.as_str()))]).await
}
...
fn find_test_zsh_path() -> Result<Option<std::path::PathBuf>> {
    ...
    let dotslash_zsh = repo_root.join("praxis-rs/app-gateway/tests/suite/zsh");
