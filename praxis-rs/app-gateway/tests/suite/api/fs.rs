...
use praxis_app_gateway_protocol::FsChangedNotification;
...
use praxis_app_gateway_protocol::FsWatchResponse;
...
async fn fs_get_metadata_returns_only_used_fields() -> Result<()> {
    ...
    let request_id = mcp
        .send_fs_get_metadata_request(praxis_app_gateway_protocol::FsGetMetadataParams {
...
async fn fs_methods_cover_current_fs_utils_surface() -> Result<()> {
    ...
    let create_directory_request_id = mcp
        .send_fs_create_directory_request(praxis_app_gateway_protocol::FsCreateDirectoryParams {
    ...
    let write_request_id = mcp
        .send_fs_write_file_request(FsWriteFileParams {
            ...
            data_base64: STANDARD.encode("hello from app-gateway"),
        })
        .await?;
    timeout(
        DEFAULT_READ_TIMEOUT,
    ...
    let read_request_id = mcp
        .send_fs_read_file_request(praxis_app_gateway_protocol::FsReadFileParams {
            path: absolute_path(nested_file.clone()),
        })
        .await?;
    ...
    let read_directory_request_id = mcp
        .send_fs_read_directory_request(praxis_app_gateway_protocol::FsReadDirectoryParams {
    ...
    .await??;
    let mut entries =
        to_response::<praxis_app_gateway_protocol::FsReadDirectoryResponse>(readdir_response)?
            .entries;
    ...
    let remove_request_id = mcp
        .send_fs_remove_request(praxis_app_gateway_protocol::FsRemoveParams {
...
-> Result<()> {
    ...
    let watch_request_id = mcp
        .send_fs_watch_request(praxis_app_gateway_protocol::FsWatchParams {
