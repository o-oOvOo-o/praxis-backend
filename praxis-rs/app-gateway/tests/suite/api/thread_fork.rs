...
async fn thread_fork_inherits_persisted_source_thread_name() -> Result<()> {
    ...
    let state_runtime =
        praxis_state::StateRuntime::init(praxis_home.path().to_path_buf(), "mock_provider".into())
            .await?;
    ThreadNameWriter::new(Some(state_runtime.as_ref()))
        .write_name(source_thread_id, renamed)
        .await?;
    ...
    let persisted_name = ThreadNameResolver::new(Some(state_runtime.as_ref()))
        .resolve_name(forked_thread_id)
        .await;
