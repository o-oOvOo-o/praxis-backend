    ...
    let request_id = mcp
        .send_plugin_install_request(PluginInstallParams {
            marketplace_path: AbsolutePathBuf::try_from(
                praxis_home.path().join("missing-marketplace.json"),
            )?,
...
async fn plugin_install_returns_invalid_request_for_disallowed_product_plugin() -> Result<()> {
  ...
  "plugins": [
    ...
    let mut mcp =
        McpProcess::new_with_args(praxis_home.path(), &["--session-source", "atlas"]).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn plugin_install_force_remote_sync_enables_remote_plugin_before_local_install() -> Result<()>
{
    ...
    assert!(
        praxis_home
            .path()
            .join("plugins/cache/debug/sample-plugin/local/.praxis-plugin/plugin.json")
            .is_file()
    );
...
async fn plugin_install_tracks_analytics_event() -> Result<()> {
    ...
    let praxis_home = TempDir::new()?;
    write_analytics_config(praxis_home.path(), &analytics_server.uri())?;
    write_chatgpt_auth(
        praxis_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
async fn plugin_install_returns_apps_needing_auth() -> Result<()> {
    ...
    assert_eq!(
        response.activation_delta.plugin_id.as_deref(),
        Some("sample-plugin@debug")
    );
    assert_eq!(
        response.activation_delta.installed_path.as_ref(),
        Some(&installed_plugin_root(
            &praxis_home,
            "debug",
            "sample-plugin"
        )?)
    );
    assert!(response.activation_delta.apps_changed);
    assert_eq!(
        response.activation_delta.app_connector_ids,
        vec!["alpha".to_string()]
    );
...
async fn plugin_install_filters_disallowed_apps_needing_auth() -> Result<()> {
    ...
    assert_eq!(response.auth_policy, PluginAuthPolicy::OnUse);
    assert_eq!(
        response.apps_needing_auth,
        vec![AppSummary {
            id: "alpha".to_string(),
...
fn write_analytics_config(praxis_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
        praxis_home.join("config.toml"),
