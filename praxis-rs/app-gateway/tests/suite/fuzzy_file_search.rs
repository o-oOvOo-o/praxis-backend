...
use praxis_app_gateway_protocol::FuzzyFileSearchSessionCompletedNotification;
use praxis_app_gateway_protocol::FuzzyFileSearchSessionUpdatedNotification;
...
fn create_config_toml(praxis_home: &Path) -> std::io::Result<()> {
...
async fn initialized_mcp(praxis_home: &TempDir) -> Result<McpProcess> {
    create_config_toml(praxis_home.path())?;
...
async fn test_fuzzy_file_search_sorts_and_includes_indices() -> Result<()> {
    // Prepare a temporary Praxis home and a separate root with test files.
