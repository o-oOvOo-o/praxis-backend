...
This repository uses Bazel to build the Rust workspace under `praxis-rs`.
...
- `rules_rs` imports third-party crates from `praxis-rs/Cargo.toml` and
  `praxis-rs/Cargo.lock` via `crate.from_cargo(...)` and exposes them under
  `@crates`.
- `../defs.bzl` provides `praxis_rust_crate`, which wraps `rust_library`,
...
- Each crate in `praxis-rs/*/BUILD.bazel` typically uses `praxis_rust_crate` and
...
2. Create a `BUILD.bazel` that calls `praxis_rust_crate` (see nearby crates for
   examples).
3. If a dependency needs special handling (compile/runtime data, additional binaries
   ...
   `praxis_rust_crate` to configure it.
