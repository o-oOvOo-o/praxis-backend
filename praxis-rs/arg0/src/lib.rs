...
pub struct Arg0DispatchPaths {
    /// Stable path to the current Praxis executable for child re-execs.
    ///
    ...
    /// instead of the real Praxis CLI.
...
pub fn arg0_dispatch() -> Option<Arg0PathEntryGuard> {
    ...
    if exe_name == EXECVE_WRAPPER_ARG0 {
        ...
        let exit_code = runtime
            .block_on(praxis_shell_escalation::run_shell_escalation_execve_wrapper(file, argv));
        match exit_code {
    ...
    if argv1 == CODEX_CORE_APPLY_PATCH_ARG1 {
        ...
        let exit_code = match patch_arg {
            Some(patch_arg) => {
                ...
                match praxis_apply_patch::apply_patch(&patch_arg, &mut stdout, &mut stderr) {
                    Ok(()) => 0,
                    Err(_) => 1,
                }
            }
    ...
    match prepend_path_entry_for_praxis_aliases() {
        Ok(path_entry) => Some(path_entry),
        Err(err) => {
            // It is possible that Praxis will proceed successfully even if
...
/// While we want to deploy the Praxis CLI as a single executable for simplicity,
...
fn linux_sandbox_exe_path(
    ...
    // Prefer the `praxis-linux-sandbox` alias when available so callers can
    // re-exec through a path whose basename still triggers arg0 dispatch on
    // bubblewrap builds that do not support `--argv0`.
    path_entry_guard
        .and_then(|path_entry| path_entry.paths().praxis_linux_sandbox_exe.clone())
        .or(current_exe)
}
...
///   with the "secret" --praxis-run-as-apply-patch flag.
///
...
pub fn prepend_path_entry_for_praxis_aliases() -> std::io::Result<Arg0PathEntryGuard> {
    let praxis_home = find_praxis_home()?;
    #[cfg(not(debug_assertions))]
    {
        // Guard against placing helpers in system temp directories outside debug builds.
        let temp_root = std::env::temp_dir();
        if praxis_home.starts_with(&temp_root) {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                format!(
                    "Refusing to create helper binaries under temporary dir {temp_root:?} (praxis_home: {praxis_home:?})"
                ),
            ));
        }
    }
...
"#,
    ...
    let paths = Arg0DispatchPaths {
        praxis_self_exe: std::env::current_exe().ok(),
...
mod tests {
    ...
    #[test]
    fn linux_sandbox_exe_path_prefers_praxis_linux_sandbox_alias() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let lock_file = create_lock(temp_dir.path())?;
        let alias_path = temp_dir.path().join("praxis-linux-sandbox");
