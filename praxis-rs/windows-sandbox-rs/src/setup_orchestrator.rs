...
pub fn sandbox_dir(praxis_home: &Path) -> PathBuf {
    praxis_home.join(".sandbox")
}

pub fn sandbox_bin_dir(praxis_home: &Path) -> PathBuf {
    praxis_home.join(".sandbox-bin")
}

pub fn sandbox_secrets_dir(praxis_home: &Path) -> PathBuf {
    praxis_home.join(".sandbox-secrets")
}

pub fn setup_marker_path(praxis_home: &Path) -> PathBuf {
    sandbox_dir(praxis_home).join("setup_marker.json")
}

pub fn sandbox_users_path(praxis_home: &Path) -> PathBuf {
    sandbox_secrets_dir(praxis_home).join("sandbox_users.json")
}
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    let mut read_roots = gather_read_roots(command_cwd, policy, praxis_home);
...
fn run_setup_refresh_inner(
    ...
    let payload = ElevationPayload {
        ...
        praxis_home: request.praxis_home.to_path_buf(),
    ...
    let cwd = std::env::current_dir().unwrap_or_else(|_| request.praxis_home.to_path_buf());
    log_note(
        &format!(
        ...
        Some(&sandbox_dir(request.praxis_home)),
    );
    let status = cmd
        .status()
        .map_err(|e| {
...
fn gather_helper_read_roots(praxis_home: &Path) -> Vec<PathBuf> {
    ...
    let helper_dir = helper_bin_dir(praxis_home);
...
fn gather_legacy_full_read_roots(
    ...
    let mut roots = gather_helper_read_roots(praxis_home);
    roots.extend(
...
pub(crate) fn gather_read_roots(
    ...
    if policy.has_full_disk_read_access() {
        gather_legacy_full_read_roots(command_cwd, policy, praxis_home)
    } else {
        gather_restricted_read_roots(command_cwd, policy, praxis_home)
    }
}
...
fn find_setup_exe() -> PathBuf {
    if let Ok(exe) = std::env::current_exe()
        && let Some(dir) = exe.parent()
    {
        let candidate = dir.join("praxis-windows-sandbox-setup.exe");
        if candidate.exists() {
            return candidate;
        }
    }
    PathBuf::from("praxis-windows-sandbox-setup.exe")
}
...
fn report_helper_failure(
    ...
    match read_setup_error_report(praxis_home) {
...
fn run_setup_exe(
    ...
    let cleared_report = match clear_setup_error_report(praxis_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
                &format!(
    ...
    if !needs_elevation {
        ...
        if let Err(err) = clear_setup_error_report(praxis_home) {
            log_note(
                &format!(
...
pub fn run_elevated_setup(
    ...
    let sbx_dir = sandbox_dir(request.praxis_home);
    ...
    run_setup_exe(&payload, needs_elevation, request.praxis_home)
}
...
fn build_payload_roots(
    ...
    let write_roots = filter_sensitive_write_roots(write_roots, request.praxis_home);
    let mut read_roots = if let Some(roots) = overrides.read_roots {
        canonical_existing(&roots)
    } else {
        gather_read_roots(request.command_cwd, request.policy, request.praxis_home)
    };
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, praxis_home: &Path) -> Vec<PathBuf> {
    ...
    let sbx_bin_dir_key = canonical_path_key(&sandbox_bin_dir(praxis_home));
    let sbx_bin_dir_prefix = format!("{}/", sbx_bin_dir_key.trim_end_matches('/'));
    let secrets_dir_key = canonical_path_key(&sandbox_secrets_dir(praxis_home));
...
mod tests {
    ...
    fn gather_read_roots_includes_helper_bin_dir() {
        let tmp = TempDir::new().expect("tempdir");
        let praxis_home = tmp.path().join("praxis-home");
        let command_cwd = tmp.path().join("workspace");
        fs::create_dir_all(&command_cwd).expect("create workspace");
        let policy = SandboxPolicy::new_read_only_policy();

        let roots = gather_read_roots(&command_cwd, &policy, &praxis_home);
        let expected =
            dunce::canonicalize(helper_bin_dir(&praxis_home)).expect("canonical helper dir");
    ...
    fn full_read_roots_preserve_legacy_platform_defaults() {
        ...
        let roots = gather_legacy_full_read_roots(&command_cwd, &policy, &praxis_home);
