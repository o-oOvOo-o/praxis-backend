...
use anyhow::Context;
use anyhow::Result;
use praxis_windows_sandbox::PipeSpawnHandles;
use praxis_windows_sandbox::SandboxPolicy;
use praxis_windows_sandbox::StderrMode;
use praxis_windows_sandbox::StdinMode;
use praxis_windows_sandbox::allow_null_device;
use praxis_windows_sandbox::convert_string_sid_to_sid;
use praxis_windows_sandbox::create_readonly_token_with_caps_from;
use praxis_windows_sandbox::create_workspace_write_token_with_caps_from;
use praxis_windows_sandbox::get_current_token_for_restriction;
use praxis_windows_sandbox::hide_current_user_profile_dir;
use praxis_windows_sandbox::ipc_framed::ErrorPayload;
use praxis_windows_sandbox::ipc_framed::ExitPayload;
use praxis_windows_sandbox::ipc_framed::FramedMessage;
use praxis_windows_sandbox::ipc_framed::Message;
use praxis_windows_sandbox::ipc_framed::OutputPayload;
use praxis_windows_sandbox::ipc_framed::OutputStream;
use praxis_windows_sandbox::ipc_framed::decode_bytes;
use praxis_windows_sandbox::ipc_framed::encode_bytes;
use praxis_windows_sandbox::ipc_framed::read_frame;
use praxis_windows_sandbox::ipc_framed::write_frame;
use praxis_windows_sandbox::log_note;
use praxis_windows_sandbox::parse_policy;
use praxis_windows_sandbox::read_handle_loop;
use praxis_windows_sandbox::spawn_process_with_pipes;
...
fn read_spawn_request(
    reader: &mut File,
) -> Result<praxis_windows_sandbox::ipc_framed::SpawnRequest> {
...
fn spawn_ipc_process(
    req: &praxis_windows_sandbox::ipc_framed::SpawnRequest,
) -> Result<IpcSpawnedProcess> {
    let log_dir = req.praxis_home.clone();
    hide_current_user_profile_dir(req.praxis_home.as_path());
    log_note(
        &format!(
            "runner start cwd={} cmd={:?} real_praxis_home={}",
            req.cwd.display(),
            req.command,
            req.real_praxis_home.display()
        ),
        Some(&req.praxis_home),
    );
    ...
    let (pi, stdout_handle, stderr_handle, stdin_handle) = if req.tty {
        let (pi, conpty) = praxis_windows_sandbox::spawn_conpty_process_as_user(
            h_token,
            &req.command,
            &effective_cwd,
            &req.env,
...
pub fn main() -> Result<()> {
    ...
    let msg = FramedMessage {
        ...
        message: Message::SpawnReady {
            payload: praxis_windows_sandbox::ipc_framed::SpawnReady {
