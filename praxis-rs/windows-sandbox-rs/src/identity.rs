...
pub fn sandbox_setup_is_complete(praxis_home: &Path) -> bool {
    let marker_ok = matches!(load_marker(praxis_home), Ok(Some(marker)) if marker.version_matches());
    if !marker_ok {
        return false;
    }
    matches!(load_users(praxis_home), Ok(Some(users)) if users.version_matches())
}

fn load_marker(praxis_home: &Path) -> Result<Option<SetupMarker>> {
    let path = setup_marker_path(praxis_home);
...
fn load_users(praxis_home: &Path) -> Result<Option<SandboxUsersFile>> {
    let path = sandbox_users_path(praxis_home);
...
fn select_identity(
    ...
    let _marker = match load_marker(praxis_home)? {
        Some(m) if m.version_matches() => m,
        _ => return Ok(None),
    };
    let users = match load_users(praxis_home)? {
...
pub fn require_logon_sandbox_creds(
    ...
    let sandbox_dir = crate::setup::sandbox_dir(praxis_home);
    let needed_read = gather_read_roots(command_cwd, policy, praxis_home);
    ...
    let mut identity = match load_marker(praxis_home)? {
        Some(marker) if marker.version_matches() => {
                ...
                marker.request_mismatch_reason(network_identity, &desired_offline_proxy_settings)
            {
                ...
                let selected = select_identity(network_identity, praxis_home)?;
                if selected.is_none() {
                    setup_reason =
    ...
    if identity.is_none() {
        ...
        identity = select_identity(network_identity, praxis_home)?;
    }
