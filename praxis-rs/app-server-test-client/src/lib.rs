...
use praxis_app_server_protocol::DynamicToolSpec;
...
use praxis_app_server_protocol::InitializeResponse;
...
use praxis_app_server_protocol::ReadOnlyAccess;
use praxis_app_server_protocol::RequestId;
use praxis_app_server_protocol::SandboxPolicy;
...
const OTEL_SERVICE_NAME: &str = "praxis-app-server-test-client";
const TRACE_DISABLED_MESSAGE: &str =
    "Not enabled - enable tracing in $CODEX_HOME/config.toml to get a trace URL!";

/// Minimal launcher that initializes the Praxis app-server and logs the handshake.
#[derive(Parser)]
#[command(author = "Praxis", version, about = "Bootstrap Praxis app-server", long_about = None)]
struct Cli {
    /// Path to the `praxis` CLI binary. When set, requests use stdio by
    /// spawning `praxis app-server` as a child process.
...
#[derive(Subcommand)]
enum CliCommand {
    /// Start `praxis app-server` on a websocket endpoint in the background.
    ///
    /// Logs are written to:
    ///   `/tmp/praxis-app-server-test-client/`
    Serve {
        /// WebSocket listen URL passed to `praxis app-server --listen`.
    ...
    /// Send a user message through the Praxis app-server.
    SendMessage {
    ...
    /// Fetch the current account rate limits from the Praxis app-server.
    GetAccountRateLimits,
    /// List the available models from the Praxis app-server.
    #[command(name = "model-list")]
    ModelList,
    /// List stored threads from the Praxis app-server.
...
fn resolve_endpoint(praxis_bin: Option<PathBuf>, url: Option<String>) -> Result<Endpoint> {
    if praxis_bin.is_some() && url.is_some() {
        bail!("--praxis-bin and --url are mutually exclusive");
    }
    if let Some(praxis_bin) = praxis_bin {
        return Ok(Endpoint::SpawnCodex(praxis_bin));
    }
    if let Some(url) = url {
...
fn resolve_shared_websocket_url(
    ...
    if praxis_bin.is_some() {
        bail!(
            "{command} requires --url or an already-running websocket app-server; --praxis-bin would spawn a private stdio app-server instead"
        );
    }
...
impl BackgroundAppServer {
    fn spawn(praxis_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        ...
        let process = cmd
            ...
            .spawn()
            .with_context(|| format!("failed to start `{}` app-server", praxis_bin.display()))?;
...
fn serve(praxis_bin: &Path, config_overrides: &[String], listen: &str, kill: bool) -> Result<()> {
    let runtime_dir = PathBuf::from("/tmp/praxis-app-server-test-client");
    ...
    println!("started {PRIMARY_CLI_COMMAND} app-server");
...
pub async fn send_message_v2(
    ...
    let endpoint = Endpoint::SpawnCodex(praxis_bin.to_path_buf());
    send_message_v2_endpoint(
        &endpoint,
        config_overrides,
        user_message,
...
fn live_elicitation_timeout_pause(
    ...
    let websocket_url = match (praxis_bin, url) {
        ...
        (Some(praxis_bin), None) => {
            let server = BackgroundAppServer::spawn(&praxis_bin, config_overrides)?;
    ...
    let app_server_test_client_bin = std::env::current_exe()
        .context("failed to resolve praxis-app-server-test-client binary path")?;
...
impl CodexClient {
    fn connect(endpoint: &Endpoint, config_overrides: &[String]) -> Result<Self> {
        match endpoint {
            Endpoint::SpawnCodex(praxis_bin) => Self::spawn_stdio(praxis_bin, config_overrides),
            Endpoint::ConnectWs(url) => Self::connect_websocket(url),
        }
    }

    fn spawn_stdio(praxis_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        ...
        let mut praxis_app_server = cmd
            .arg("app-server")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .spawn()
            .with_context(|| format!("failed to start `{praxis_bin_display}` app-server"))?;

        let stdin = praxis_app_server
            .stdin
            .take()
            .context("praxis app-server stdin unavailable")?;
        let stdout = praxis_app_server
            .stdout
            .take()
                ...
                Err(err) => {
                    if Instant::now() >= deadline {
                        return Err(err).with_context(|| {
                            format!(
                                "failed to connect to websocket app-server at `{url}`; if no server is running, start one with `praxis-app-server-test-client serve --listen {url}`"
                            )
                        });
                    }
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    name: "praxis-toy-app-server".to_string(),
                    title: Some("Praxis Toy App Server".to_string()),
    ...
    fn login_account_chatgpt(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: praxis_app_server_protocol::LoginAccountParams::Chatgpt,
        };
    ...
    fn login_account_chatgpt_device_code(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: praxis_app_server_protocol::LoginAccountParams::ChatgptDeviceCode,
        };
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdin, .. } => {
                if let Some(stdin) = stdin.as_mut() {
                    ...
                    stdin
                        .flush()
                        .context("failed to flush payload to praxis app-server")?;
                    return Ok(());
                }
                bail!("praxis app-server stdin closed")
            }
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdout, .. } => {
                ...
                let bytes = stdout
                    ...
                    .context("failed to read from praxis app-server")?;
                if bytes == 0 {
                    bail!("praxis app-server closed stdout");
                }
                Ok(response_line)
            }
...
impl Drop for CodexClient {
    fn drop(&mut self) {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            println!("[praxis app-server exited: {status}]");
            return;
        }
