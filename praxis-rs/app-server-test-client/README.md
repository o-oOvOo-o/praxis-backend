...
cargo build -p praxis-cli --bin codex

# 2) Start websocket app-server in background
cargo run -p praxis-app-server-test-client -- \
  --praxis-bin ./target/debug/codex \
  serve --listen ws://127.0.0.1:4222 --kill

# 3) Call app-server (defaults to ws://127.0.0.1:4222)
cargo run -p praxis-app-server-test-client -- model-list
```
...
```bash
cargo run -p praxis-app-server-test-client -- watch
```

## Testing Thread Rejoin Behavior

Build and start an app server using commands above. The app-server log is written to `/tmp/praxis-app-server-test-client/app-server.log`
...
```bash
cargo run -p praxis-app-server-test-client -- send-message-v2 "seed thread for rejoin test"
cargo run -p praxis-app-server-test-client -- thread-list --limit 5
```
...
```bash
cargo run --bin praxis-app-server-test-client -- \
...
```bash
cargo run --bin praxis-app-server-test-client -- thread-resume <THREAD_ID>
```
