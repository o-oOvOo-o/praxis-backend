...
fn manager_attaches_metadata_tags_to_metrics() -> Result<()> {
    let (metrics, exporter) = build_metrics_with_defaults(&[("service", "praxis-cli")])?;
    ...
    let metric =
        find_metric(&resource_metrics, "praxis.session_started").expect("counter metric missing");
...
fn manager_attaches_optional_service_name_tag() -> Result<()> {
    ...
    manager.counter("praxis.session_started", /*inc*/ 1, &[]);
    manager.shutdown_metrics()?;
