...
use praxis_otel::metrics::MetricsError;
use praxis_otel::metrics::Result;

fn build_in_memory_client() -> Result<MetricsClient> {
    let exporter = InMemoryMetricExporter::default();
    let config =
        MetricsConfig::in_memory("test", "praxis-cli", env!("CARGO_PKG_VERSION"), exporter);
...
fn counter_rejects_invalid_tag_key() -> Result<()> {
    ...
    let err = metrics
        .counter("praxis.turns", /*inc*/ 1, &[("bad key", "value")])
        .unwrap_err();
    assert!(matches!(
        err,
        MetricsError::InvalidTagComponent { label, value }
...
fn counter_rejects_negative_increment() -> Result<()> {
    let metrics = build_in_memory_client()?;
    let err = metrics
        .counter("praxis.turns", /*inc*/ -1, &[])
        .unwrap_err();
    assert!(matches!(
        err,
        MetricsError::NegativeCounterIncrement { name, inc } if name == "praxis.turns" && inc == -1
    ));
    metrics.shutdown()?;
    Ok(())
}
