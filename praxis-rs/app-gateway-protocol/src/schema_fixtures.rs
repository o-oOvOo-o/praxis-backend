...
pub fn generate_typescript_schema_fixture_subtree_for_tests() -> Result<BTreeMap<PathBuf, Vec<u8>>>
{
    ...
    collect_typescript_fixture_file::<GatewayCapability>(&mut files, &mut seen)?;
    collect_typescript_fixture_file::<GatewayCapabilityKind>(&mut files, &mut seen)?;
    collect_typescript_fixture_file::<GatewayClientInfo>(&mut files, &mut seen)?;
    collect_typescript_fixture_file::<GatewayErrorPayload>(&mut files, &mut seen)?;
    collect_typescript_fixture_file::<GatewayEventEnvelope>(&mut files, &mut seen)?;
    collect_typescript_fixture_file::<GatewayMetadata>(&mut files, &mut seen)?;
    collect_typescript_fixture_file::<GatewayMode>(&mut files, &mut seen)?;
    collect_typescript_fixture_file::<GatewayRequestEnvelope>(&mut files, &mut seen)?;
    collect_typescript_fixture_file::<GatewayResponseEnvelope>(&mut files, &mut seen)?;
    collect_typescript_fixture_file::<GatewayTransport>(&mut files, &mut seen)?;
    collect_typescript_fixture_file::<HostExtensionInfo>(&mut files, &mut seen)?;
    collect_typescript_fixture_file::<HostKind>(&mut files, &mut seen)?;
    collect_typescript_fixture_file::<MetraBridgeDescriptor>(&mut files, &mut seen)?;
    collect_typescript_fixture_file::<MetraSemanticSnapshot>(&mut files, &mut seen)?;
    collect_typescript_fixture_file::<MetraSurfaceDescriptor>(&mut files, &mut seen)?;
...
///
/// This is intended to be used by tooling (e.g., `just write-app-gateway-schema`).
...
fn canonicalize_json(value: &Value) -> Value {
    match value {
        Value::Array(items) => {
            ...
            // only by schema fixture tooling to compare JSON schema trees against freshly
            // generated output. Some parts of schema generation end up
