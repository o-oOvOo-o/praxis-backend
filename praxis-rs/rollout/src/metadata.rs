...
use praxis_state::BackfillState;
use praxis_state::BackfillStats;
use praxis_state::BackfillStatus;
use praxis_state::DB_ERROR_METRIC;
use praxis_state::DB_METRIC_BACKFILL;
use praxis_state::DB_METRIC_BACKFILL_DURATION_MS;
use praxis_state::ExtractionOutcome;
use praxis_state::ThreadMetadataBuilder;
use praxis_state::apply_rollout_item;
use std::path::Path;
...
pub(crate) async fn backfill_sessions(
    runtime: &praxis_state::StateRuntime,
    config: &impl RolloutConfigView,
) {
    let metric_client = praxis_otel::metrics::global();
    ...
    let sessions_root = config.praxis_home().join(SESSIONS_SUBDIR);
    let archived_root = config.praxis_home().join(ARCHIVED_SESSIONS_SUBDIR);
        ...
        match collect_rollout_paths(&root).await {
            Ok(paths) => {
                rollout_paths.extend(paths.into_iter().map(|path| BackfillRolloutPath {
                    watermark: backfill_watermark_for_path(config.praxis_home(), &path),
                    path,
                    archived,
                }));
            }
...
fn backfill_watermark_for_path(praxis_home: &Path, path: &Path) -> String {
    path.strip_prefix(praxis_home)
        .unwrap_or(path)
        .to_string_lossy()
        .replace('\\', "/")
}
