...
fn test_config(praxis_home: PathBuf) -> RolloutConfig {
    RolloutConfig {
        sqlite_home: praxis_home.clone(),
        cwd: praxis_home.clone(),
        praxis_home,
...
async fn backfill_sessions_resumes_from_watermark_and_marks_complete() {
    let dir = tempdir().expect("tempdir");
    let praxis_home = dir.path().to_path_buf();
    ...
    let runtime =
        praxis_state::StateRuntime::init(praxis_home.clone(), "test-provider".to_string())
            .await
            .expect("initialize runtime");
    let first_watermark = backfill_watermark_for_path(praxis_home.as_path(), first_path.as_path());
    ...
    let config = test_config(praxis_home.clone());
...
async fn backfill_sessions_normalizes_cwd_before_upsert() {
    ...
    let praxis_home = dir.path().to_path_buf();
    let thread_uuid = Uuid::new_v4();
    let session_cwd = praxis_home.join(".");
...
fn write_rollout_in_sessions_with_cwd(
    ...
    let sessions_dir = praxis_home.join("sessions");
