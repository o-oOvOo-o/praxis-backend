...
use crate::mcp_connection_manager::praxis_apps_tools_cache_key;
...
/// MCP runtime settings derived from `praxis_core::config::Config`.
///
/// This struct should contain only long-lived configuration values that the
/// `praxis-mcp` crate needs to construct server transports, enforce MCP
...
/// [`with_praxis_apps_mcp`] and [`collect_mcp_snapshot`] so config objects do
/// not go stale when auth changes.
#[derive(Debug, Clone)]
pub struct McpConfig {
    /// Base URL for ChatGPT-hosted app MCP servers, copied from the root config.
    pub chatgpt_base_url: String,
    /// Praxis home directory used for MCP OAuth state and app-tool cache files.
    ...
    /// Optional path to `praxis-linux-sandbox` for sandboxed MCP tool execution.
...
fn normalize_praxis_apps_base_url(base_url: &str) -> String {
...
fn praxis_apps_mcp_url_for_base_url(base_url: &str) -> String {
    let base_url = normalize_praxis_apps_base_url(base_url);
...
pub(crate) fn praxis_apps_mcp_url(config: &McpConfig) -> String {
...
fn praxis_apps_mcp_server_config(config: &McpConfig, auth: Option<&CodexAuth>) -> McpServerConfig {
    ...
    let http_headers = if bearer_token_env_var.is_some() {
        ...
        praxis_apps_mcp_http_headers(auth)
    };
    let url = praxis_apps_mcp_url(config);
