...
use streamable_http::StreamableHttpResponseClient;
use streamable_http::StreamableHttpResponseClientError;
use streamable_http::build_http_client;
...
impl ProcessGroupGuard {
    ...
    fn maybe_terminate_process_group(&self) {
        ...
        let should_escalate =
            match praxis_utils_pty::process_group::terminate_process_group(process_group_id) {
                Ok(exists) => exists,
                Err(error) => {
        ...
        if should_escalate {
            std::thread::spawn(move || {
                ...
                if let Err(error) =
                    praxis_utils_pty::process_group::kill_process_group(process_group_id)
                {
