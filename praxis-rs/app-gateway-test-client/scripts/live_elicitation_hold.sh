...
require_env APP_GATEWAY_URL
require_env APP_GATEWAY_TEST_CLIENT_BIN

thread_id="${PRAXIS_THREAD_ID:-${THREAD_ID-}}"
if [ -z "$thread_id" ]; then
  echo "missing required env var: PRAXIS_THREAD_ID" >&2
  exit 1
fi
...
cleanup() {
  if [ "$incremented" -eq 1 ]; then
    "$APP_GATEWAY_TEST_CLIENT_BIN" --url "$APP_GATEWAY_URL" \
