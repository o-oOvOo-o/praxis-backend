...
impl App {
    fn transcript_overlay_width(tui: &tui::Tui) -> u16 {
        tui.terminal.viewport_area.width.max(1)
    }

    fn apply_transcript_search_overlay_state(
        &mut self,
        state: Option<TranscriptSearchOverlayState>,
    ) {
        if let Some(Overlay::Transcript(overlay)) = &mut self.overlay {
            overlay.set_search_state(state);
        }
    }

    fn refresh_transcript_search_overlay_state(&mut self, tui: &tui::Tui) {
        let width = Self::transcript_overlay_width(tui);
        let state = self
            .chat_widget
            .transcript_search_overlay_state(width, &self.transcript_cells);
        self.apply_transcript_search_overlay_state(state);
    }

    fn set_transcript_search_query(&mut self, tui: &mut tui::Tui, query: String) {
        let width = Self::transcript_overlay_width(tui);
        let state =
            self.chat_widget
                .set_transcript_search_query(query, width, &self.transcript_cells);
        self.apply_transcript_search_overlay_state(state);
        tui.frame_requester().schedule_frame();
    }

    fn step_transcript_search(&mut self, tui: &mut tui::Tui, backwards: bool) {
        let width = Self::transcript_overlay_width(tui);
        let state = if backwards {
            self.chat_widget
                .transcript_search_prev(width, &self.transcript_cells)
        } else {
            self.chat_widget
                .transcript_search_next(width, &self.transcript_cells)
        };
        self.apply_transcript_search_overlay_state(state);
        tui.frame_requester().schedule_frame();
    }

    fn handle_transcript_search_overlay_event(
        &mut self,
        tui: &mut tui::Tui,
        event: &TuiEvent,
    ) -> bool {
        ...
        match event {
            TuiEvent::Paste(pasted) if self.chat_widget.is_transcript_search_open() => {
                let pasted = pasted.replace('\r', "").replace('\n', " ");
                if pasted.is_empty() {
                    return true;
                }
                let next_query =
                    format!("{}{}", self.chat_widget.transcript_search_query(), pasted);
                self.set_transcript_search_query(tui, next_query);
                true
            }
            TuiEvent::Key(key_event)
                if matches!(key_event.kind, KeyEventKind::Press | KeyEventKind::Repeat) =>
            {
                let search_open = self.chat_widget.is_transcript_search_open();
                match key_event.code {
                    ...
                    {
                        if search_open {
                            self.step_transcript_search(tui, /*backwards*/ false);
                        } else {
                            let _ = self.chat_widget.open_transcript_search();
                            self.refresh_transcript_search_overlay_state(tui);
                            tui.frame_requester().schedule_frame();
                        }
                        true
                    }
                            ...
                            && ch.eq_ignore_ascii_case(&'g') =>
                    {
                        ...
                        self.step_transcript_search(
                            tui,
                            /*backwards*/ key_event.modifiers.contains(KeyModifiers::SHIFT),
                        );
                        true
                    }
                    KeyCode::Esc if search_open => {
                        self.chat_widget.close_transcript_search();
                        self.apply_transcript_search_overlay_state(None);
                        tui.frame_requester().schedule_frame();
                        true
                    }
                    KeyCode::Enter if search_open => {
                        self.step_transcript_search(
                            tui,
                            /*backwards*/ key_event.modifiers.contains(KeyModifiers::SHIFT),
                        );
                    ...
                    KeyCode::Backspace if search_open => {
                        let mut query = self.chat_widget.transcript_search_query().to_string();
                        query.pop();
                        self.set_transcript_search_query(tui, query);
                        true
                    }
                    KeyCode::Char(ch)
                        if search_open
    ...
    pub(crate) async fn handle_backtrack_overlay_event(
        ...
        if self.handle_transcript_search_overlay_event(tui, &event) {
            return Ok(true);
        }
