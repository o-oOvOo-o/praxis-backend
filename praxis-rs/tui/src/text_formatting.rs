...
/// Truncate `text` to `max_graphemes` graphemes. Using graphemes to avoid accidentally truncating in the middle of a multi-praxisoint character.
...
mod tests {
    ...
    fn test_workspace_truncate_doesnt_truncate_short_path() {
        let sep = std::path::MAIN_SEPARATOR;
        let path = format!("{sep}Users{sep}praxis{sep}Public");
    ...
    fn test_workspace_truncate_truncates_long_windows_path() {
        let sep = std::path::MAIN_SEPARATOR;
        let path = format!(
            "C:{sep}Users{sep}praxis{sep}Projects{sep}super{sep}long{sep}windows{sep}path{sep}file.txt"
        );
        let truncated = workspace_truncate_path(&path, /*max_width*/ 36);

        let expected = format!("C:{sep}Users{sep}praxis{sep}…{sep}path{sep}file.txt");
