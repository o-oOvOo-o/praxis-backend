...
use praxis_utils_sandbox_summary::summarize_sandbox_policy;
use ratatui::prelude::*;
...
impl HistoryCell for StatusHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        lines.push(Line::from(vec![
            ...
            Span::from("Praxis CLI").bold(),
            Span::from(" ").dim(),
            Span::from(format!("(v{PRAXIS_CLI_VERSION})")).dim(),
        ]));
        ...
        let account_value = self.account.as_ref().map(|account| match account {
            ...
            StatusAccountDisplay::ApiKey => {
                "API key configured (run praxis login to use ChatGPT)".to_string()
            }
        });
