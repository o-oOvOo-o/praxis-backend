...
#[cfg(test)]
use praxis_git_utils::CommitLogEntry;
use praxis_git_utils::current_branch_name;
use praxis_git_utils::get_git_repo_root;
use praxis_git_utils::local_git_branches;
use praxis_git_utils::recent_commits;
...
const SELFWORK_PICKER_VIEW_ID: &str = "selfwork-plan-selection";
const SELFWORK_USAGE: &str =
    "Use /selfwork to choose a markdown plan, or /selfwork start <plan.md> (alias: /loop).";
const SELFWORK_STALL_LIMIT: u8 = 3;
const SELFWORK_PLAN_SCAN_LIMIT: usize = 200;
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
struct ActiveCellRenderCacheKey {
    width: u16,
    revision: u64,
...
#[derive(Debug, Clone)]
struct SelfworkPlanInspection {
    path: PathBuf,
    digest: u64,
    checklist_total: usize,
    checklist_unchecked: usize,
    complete: bool,
}

#[derive(Debug, Clone)]
struct SelfworkPlanCandidate {
    path: PathBuf,
    display_path: String,
    description: String,
...
#[derive(Debug, Clone)]
struct SelfworkPlanDiscovery {
    root: PathBuf,
    candidates: Vec<SelfworkPlanCandidate>,
    truncated: bool,
}
...
impl ChatWidget {
    ...
    fn sync_status_budget_message(&mut self) {
    ...
    fn on_patch_apply_end(&mut self, event: praxis_protocol::protocol::PatchApplyEndEvent) {
        let ev2 = event.clone();
        self.defer_or_handle(
    ...
    fn on_hook_started(&mut self, event: praxis_protocol::protocol::HookStartedEvent) {
    ...
    fn on_hook_completed(&mut self, event: praxis_protocol::protocol::HookCompletedEvent) {
    ...
    fn handle_selfwork_default_invocation(&mut self) {
    ...
    pub(crate) fn start_selfwork_from_input(&mut self, raw_path: String) {
        match self.resolve_selfwork_plan_path(&raw_path) {
    ...
    pub(crate) fn show_selfwork_plan_prompt(&mut self) {
        ...
        let root = self.selfwork_search_root();
        let context = format!(
            "Project root: {}",
    ...
    fn open_selfwork_plan_picker_or_prompt(&mut self) {
        if !self.is_session_configured() {
            self.add_error_message(
                "Selfwork is unavailable until the thread session is ready.".to_string(),
            );
        ...
        let discovery = match self.discover_selfwork_plan_candidates() {
    ...
    fn selfwork_search_root(&self) -> PathBuf {
        self.current_cwd
            .as_deref()
    ...
    fn discover_selfwork_plan_candidates(&self) -> Result<SelfworkPlanDiscovery, String> {
        let root = self.selfwork_search_root();
        let (paths, truncated) = self.collect_selfwork_plan_paths(root.as_path())?;
        let candidates = paths
            .into_iter()
            .filter_map(|path| {
                self.build_selfwork_plan_candidate(root.as_path(), path)
                    .ok()
            })
            .collect();
    ...
    fn collect_selfwork_plan_paths(&self, root: &Path) -> Result<(Vec<PathBuf>, bool), String> {
        ...
        while let Some(dir) = pending.pop_front() {
            let entries = std::fs::read_dir(&dir).map_err(|err| {
                format!("Failed to scan markdown plans in {}: {err}", dir.display())
            })?;
            ...
            for entry in children {
                ...
                if file_type.is_dir() {
                    if Self::should_descend_selfwork_dir(&name) {
                        pending.push_back(path);
                    }
                    continue;
                }
                if !file_type.is_file() || !Self::is_markdown_plan_file(&name) {
                    continue;
                }
                paths.push(path);
        ...
        paths.sort_by_key(|path| Self::selfwork_candidate_sort_key(root, path));
        Ok((paths, truncated))
    }
    ...
    fn build_selfwork_plan_candidate(
        ...
        let inspection = self.inspect_selfwork_plan(path.as_path())?;
        let contents = std::fs::read_to_string(&path)
            .map_err(|err| format!("Failed to read selfwork plan {}: {err}", path.display()))?;
        let status = Self::selfwork_status_summary(&inspection);
        let preview = Self::selfwork_preview_from_contents(&contents);
    ...
    fn resolve_selfwork_plan_path(&self, raw: &str) -> Result<PathBuf, String> {
    ...
    fn inspect_selfwork_plan(&self, path: &Path) -> Result<SelfworkPlanInspection, String> {
        let contents = std::fs::read_to_string(path)
            .map_err(|err| format!("Failed to read selfwork plan {}: {err}", path.display()))?;
        ...
        for line in contents.lines() {
            ...
            if let Some(checked) = Self::selfwork_checklist_state(trimmed) {
                checklist_total += 1;
                if !checked {
    ...
    fn selfwork_prompt(plan_path: &Path) -> String {
        format!(
    ...
    pub(crate) fn activate_selfwork(&mut self, plan_path: PathBuf) {
        ...
        let inspection = match self.inspect_selfwork_plan(&plan_path) {
    ...
    fn show_selfwork_status(&mut self) {
        ...
        let hint = match self.inspect_selfwork_plan(&path) {
            Err(err) => Some(err),
    ...
    fn maybe_start_selfwork_turn_now(&mut self) -> bool {
    ...
    fn maybe_continue_selfwork_after_turn(&mut self, was_selfwork_turn: bool) {
        let Some(plan_path) = self.selfwork_plan_path.clone() else {
            return;
        };
        ...
        match cmd {
            ...
            SlashCommand::Selfwork => {
                ...
                match command.to_ascii_lowercase().as_str() {
                    ...
                    _ => match self.resolve_selfwork_plan_path(prepared_trimmed) {
                        Ok(path) => self.activate_selfwork(path),
                        Err(err) => self.add_error_message(err),
                    },
                }
    ...
    fn expire_in_app_toast(&mut self) {
    ...
    fn prefetch_connectors_with_options(&mut self, force_refetch: bool) {
        ...
        tokio::spawn(async move {
            ...
            let should_schedule_force_refetch =
                !force_refetch && !accessible_result.praxis_apps_ready;
    ...
    fn open_personality_popup_for_current_model(&mut self) {
        ...
        header.push(Line::from("Choose a communication style for Praxis.".dim()));
    ...
    fn is_current_model_selection(&self, preset: &ModelPreset) -> bool {
    ...
    fn is_auto_model(model: &str) -> bool {
        model.starts_with("praxis-auto-")
    }

    fn auto_model_order(model: &str) -> usize {
        match model {
            "praxis-auto-fast" => 0,
            "praxis-auto-balanced" => 1,
            "praxis-auto-thorough" => 2,
            _ => 3,
        }
    }
    ...
    pub(crate) fn open_all_models_popup(&mut self, presets: Vec<ModelPreset>) {
        ...
        for preset in presets.into_iter() {
            ...
            let is_current = self.is_current_model_selection(&preset);
        ...
        let header = self.model_menu_header(
            ...
            "Access legacy models by running praxis -m <model_name> or in your config.toml",
        );
        self.bottom_pane.show_selection_view(SelectionViewParams {
    ...
    fn plugins_for_mentions(&self) -> Option<&[praxis_core::plugins::PluginCapabilitySummary]> {
    ...
    fn connectors_popup_params(
        ...
        for connector in connectors {
            ...
            let instructions = if connector.is_accessible {
                ...
                "Install this app in your browser, then reload Praxis."
            };
...
impl ChatWidget {
    ...
    fn active_cell_render_cache(
        &self,
        width: u16,
    ) -> Option<std::cell::Ref<'_, ActiveCellRenderCache>> {
        ...
        Some(std::cell::Ref::map(
            self.active_cell_render_cache.borrow(),
            |cache| {
                cache
                    .as_ref()
    ...
    fn active_cell_total_height(&self, width: u16) -> u16 {
...
impl Notification {
    ...
    fn user_input_request_summary(
        questions: &[praxis_protocol::request_user_input::RequestUserInputQuestion],
    ) -> Option<String> {
...
const DEFAULT_COMPOSER_PLACEHOLDER: &str = "Message Praxis  / for commands  @ for context";
...
fn hook_event_label(event_name: praxis_protocol::protocol::HookEventName) -> &'static str {
    match event_name {
        praxis_protocol::protocol::HookEventName::PreToolUse => "PreToolUse",
        praxis_protocol::protocol::HookEventName::PostToolUse => "PostToolUse",
        praxis_protocol::protocol::HookEventName::SessionStart => "SessionStart",
        praxis_protocol::protocol::HookEventName::UserPromptSubmit => "UserPromptSubmit",
