    ...
    fn flush_current_line(&mut self) {
        if let Some(line) = self.current_line_content.take() {
                ...
                && let Some(width) = self.wrap_width
            {
                ...
                let wrapped_lines = if line_contains_hyperlink_like_token(&line) {
                    word_wrap_line(&line, hyperlink_preserving_wrap_options(opts))
                } else {
...
fn hyperlink_preserving_wrap_options<'a>(opts: RtOptions<'a>) -> RtOptions<'a> {
    opts.word_separator(textwrap::WordSeparator::AsciiSpace)
        .word_splitter(textwrap::WordSplitter::Custom(split_non_hyperlink_word))
        .break_words(/*break_words*/ false)
}

fn split_non_hyperlink_word(word: &str) -> Vec<usize> {
    if is_hyperlink_like_token(word) {
        return Vec::new();
    }
...
fn line_contains_hyperlink_like_token(line: &Line<'_>) -> bool {
    let text: String = line
        .spans
        .iter()
        .map(|span| span.content.as_ref())
        .collect();
    text.split_ascii_whitespace().any(is_hyperlink_like_token)
}

fn is_hyperlink_like_token(raw_token: &str) -> bool {
    let token = trim_hyperlinkish_token(raw_token);
    !token.is_empty() && (text_contains_url_like(token) || is_local_path_like_link(token))
}

fn trim_hyperlinkish_token(token: &str) -> &str {
