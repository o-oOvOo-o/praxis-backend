...
/// into `name` that receive semantic search-highlight styling, and
/// `wrap_indent` is interpreted in terminal cell columns.
#[derive(Default)]
pub(crate) struct GenericDisplayRow {
    pub name: String,
    pub name_prefix_spans: Vec<Span<'static>>,
    pub display_shortcut: Option<KeyBinding>,
    pub match_indices: Option<Vec<usize>>, // indices to highlight (char positions)
    ...
    if selected {
        for line in lines.iter_mut() {
            line.spans.iter_mut().for_each(|span| {
                span.style = selection_overlay(span.style);
            });
        }
    }
    if is_disabled {
        ...
        for (char_idx, ch) in row.name.chars().enumerate() {
            ...
            if idx_iter.peek().is_some_and(|next| **next == char_idx) {
                ...
                name_spans.push(Span::styled(
                    ch.to_string(),
                    search_highlight_overlay(Style::default()),
                ));
            } else {
...
mod tests {
    ...
    #[test]
    fn match_indices_use_semantic_highlight_background() {
        ...
        let highlighted_spans: Vec<&Span<'static>> = line
            .spans
            .iter()
            .filter(|span| span.style.bg.is_some())
            .collect();
        assert_eq!(highlighted_spans.len(), 2);
        assert!(
            highlighted_spans
                .iter()
    ...
    #[test]
    fn selected_rows_use_background_overlay_instead_of_foreground_swap() {
        ...
        apply_row_state_style(
            &mut lines, /*selected*/ true, /*is_disabled*/ false,
        );

        assert!(
            lines[0].spans.iter().all(|span| span.style.bg.is_some()),
            "expected semantic selection to set a row background"
        );
        assert!(
            lines[0]
                .spans
