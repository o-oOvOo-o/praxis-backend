...
const SHORTCUTS_HINT_LABEL: &str = " shortcuts";
const QUEUE_HINT_LABEL: &str = " to queue";
const QUEUE_HINT_SHORT_LABEL: &str = " queue";
...
pub(crate) fn footer_show_shortcuts_hint(mode: FooterMode, is_task_running: bool) -> bool {
    matches!(mode, FooterMode::ComposerEmpty) && !is_task_running
}

pub(crate) fn footer_show_queue_hint(mode: FooterMode, is_task_running: bool) -> bool {
    matches!(mode, FooterMode::ComposerHasDraft) && is_task_running
}
...
fn left_side_line(
    ...
    match state.hint {
        ...
        SummaryHintKind::Shortcuts => {
            ...
            line.push_span(SHORTCUTS_HINT_LABEL.dim());
        }
        SummaryHintKind::QueueMessage => {
            line.push_span(key_hint::plain(KeyCode::Tab));
            line.push_span(QUEUE_HINT_LABEL.dim());
        }
        SummaryHintKind::QueueShort => {
            line.push_span(key_hint::plain(KeyCode::Tab));
            line.push_span(QUEUE_HINT_SHORT_LABEL.dim());
        }
    };
        ...
        terminal
            .draw(|f| {
                ...
                let show_shortcuts_hint =
                    footer_show_shortcuts_hint(props.mode, props.is_task_running);
                let show_queue_hint = footer_show_queue_hint(props.mode, props.is_task_running);
