...
use praxis_feedback::feedback_diagnostics::FEEDBACK_DIAGNOSTICS_ATTACHMENT_FILENAME;
use praxis_feedback::feedback_diagnostics::FeedbackDiagnostics;
...
const BASE_CLI_BUG_ISSUE_URL: &str =
    "https://github.com/o-oOvOo-o/praxis-backend/issues/new?template=3-cli.yml";
/// Internal routing link for employee feedback follow-ups. This must not be shown to external users.
const PRAXIS_FEEDBACK_INTERNAL_URL: &str = "http://go/praxis-feedback-internal";
    ...
    match issue_url {
        Some(url) if feedback_audience == FeedbackAudience::Internal => {
            lines.extend([
                ...
                Line::from(vec![
                    "    ".into(),
                    format!("https://go/praxis-feedback/{thread_id}").bold(),
                ]),
            ]);
        }
        Some(url) => {
...
fn slack_feedback_url(_thread_id: &str) -> String {
    PRAXIS_FEEDBACK_INTERNAL_URL.to_string()
}
...
pub(crate) fn feedback_upload_consent_params(
    ...
    let mut header_lines: Vec<Box<dyn crate::render::renderable::Renderable>> = vec![
        ...
        Line::from(vec!["  • ".into(), "praxis-logs.log".into()]).into(),
    ];
    ...
    super::SelectionViewParams {
        ...
        items: vec![
            super::SelectionItem {
                name: "Yes".to_string(),
                description: Some(
                    "Share the current Praxis session logs with the team for troubleshooting."
                        .to_string(),
                ),
...
mod tests {
    ...
    use crate::app_event::AppEvent;
    use crate::app_event_sender::AppEventSender;
    use praxis_feedback::feedback_diagnostics::FeedbackDiagnostic;
    ...
    fn issue_url_available_for_bug_bad_result_safety_check_and_other() {
        ...
        let expected_external_url = "https://github.com/o-oOvOo-o/praxis-backend/issues/new?template=3-cli.yml&steps=Uploaded%20thread:%20t";
    ...
    fn feedback_success_cell_matches_employee_bug_copy() {
        ...
        assert_eq!(
            rendered,
            "• Feedback uploaded. Please report this in #praxis-feedback:\n\n  http://go/praxis-feedback-internal\n\n  Share this and add some info about your problem:\n    https://go/praxis-feedback/thread-2"
        );
    }
