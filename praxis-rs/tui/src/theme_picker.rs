...
//! under `{PRAXIS_HOME}/themes/`.  It provides:
//!
...
//!   the preview panel and any visible code blocks. The preview also shows the
//!   semantic selection/highlight tokens derived from the active terminal
//!   palette so `/theme` makes the auto light/dark resolution visible.
...
use crate::style::interactive_badge_style;
use crate::style::search_highlight_style;
...
fn preview_semantic_lines(appearance: TerminalAppearance) -> [Line<'static>; 2] {
    let auto_theme =
        highlight::adaptive_default_theme_selection_for_terminal_theme(appearance.theme).1;
    [
        Line::from(vec![
            Span::styled(format!(" Auto: {auto_theme} "), interactive_badge_style()),
            Span::from(format!(
                " {} via {}",
                appearance.theme.as_str(),
                appearance.source.as_str()
            ))
            .dim(),
        ]),
        Line::from(vec![
            Span::styled(" Selected row ", selection_style()),
            Span::from(" "),
            Span::styled("match", search_highlight_style()),
            Span::from(" token").dim(),
        ]),
    ]
}
...
fn render_preview(
    ...
    let semantic_lines = preview_semantic_lines(terminal_appearance());
...
impl Renderable for ThemePreviewNarrowRenderable {
    fn desired_height(&self, _width: u16) -> u16 {
        (NARROW_PREVIEW_ROWS.len() + 2) as u16
    }
...
fn theme_picker_subtitle(praxis_home: Option<&Path>, terminal_width: Option<u16>) -> String {
    let themes_dir = praxis_home.map(|home| home.join("themes"));
...
pub(crate) fn build_theme_picker_params(
    ...
    let entries = highlight::list_available_themes(praxis_home);
    let praxis_home_owned = praxis_home.map(Path::to_path_buf);
    ...
    let preview_home = praxis_home_owned.clone();
...
mod tests {
    ...
    fn appearance(theme: TerminalTheme, source: TerminalThemeSource) -> TerminalAppearance {
        TerminalAppearance {
            fg: None,
            bg: None,
            theme,
    ...
    fn theme_picker_uses_half_width_with_stacked_fallback_preview() {
        let params = build_theme_picker_params(
            /*current_name*/ None, /*praxis_home*/ None, /*terminal_width*/ None,
        );
    ...
    #[test]
    fn preview_semantic_lines_describe_auto_theme_resolution() {
        let light_lines = preview_semantic_lines(appearance(
            TerminalTheme::Light,
            TerminalThemeSource::ColorFgBg,
        ));
        let dark_lines = preview_semantic_lines(appearance(
            TerminalTheme::Dark,
            TerminalThemeSource::DefaultColorsQuery,
        ));

        assert!(line_text(&light_lines[0]).contains("catppuccin-latte"));
        assert!(line_text(&light_lines[0]).contains("light via COLORFGBG"));
        assert!(line_text(&dark_lines[0]).contains("catppuccin-mocha"));
        assert!(line_text(&dark_lines[0]).contains("dark via terminal palette"));
    }

    #[test]
    fn narrow_preview_renders_semantic_selection_and_match_labels() {
        let lines = render_lines(
        ...
        assert!(lines.iter().any(|line| line.contains("Selected row")));
        assert!(lines.iter().any(|line| line.contains("match")));
    }
    ...
    #[test]
    fn subtitle_uses_tilde_path_when_praxis_home_under_home_directory() {
        let home = dirs::home_dir().expect("home directory should be available");
        let praxis_home = home.join(".praxis");

        let subtitle = theme_picker_subtitle(Some(&praxis_home), Some(200));
    ...
    fn subtitle_falls_back_when_tilde_path_subtitle_is_too_wide() {
        ...
        let praxis_home = home.join(long_segment).join(".praxis");

        let subtitle = theme_picker_subtitle(Some(&praxis_home), Some(140));
    ...
    fn subtitle_falls_back_to_preview_instructions_without_tilde_path() {
        let subtitle =
            theme_picker_subtitle(/*praxis_home*/ None, /*terminal_width*/ None);
        assert_eq!(subtitle, PREVIEW_FALLBACK_SUBTITLE);
    }
    ...
    fn subtitle_falls_back_for_94_column_terminal_side_by_side_layout() {
        ...
        let subtitle = theme_picker_subtitle(Some(&praxis_home), Some(94));
