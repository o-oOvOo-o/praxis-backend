...
const PAID_TOOLTIP: &str = "*New* Try the **Praxis App** with 2x rate limits until *April 2nd*. Run 'praxis app' or visit https://chatgpt.com/codex?app-landing-page=true";
const PAID_TOOLTIP_WINDOWS: &str = "*New* Try the **Praxis App**, now available on **Windows**, with 2x rate limits until *April 2nd*. Run 'praxis app' or visit https://chatgpt.com/codex?app-landing-page=true";
...
const OTHER_TOOLTIP: &str = "*New* Build faster with the **Praxis App**. Run 'praxis app' or visit https://chatgpt.com/codex?app-landing-page=true";
const OTHER_TOOLTIP_NON_MAC: &str = "*New* Build faster with Praxis.";
const FREE_GO_TOOLTIP: &str =
    "*New* For a limited time, Praxis is included in your plan for free - let's build together.";
...
lazy_static! {
    static ref TOOLTIPS: Vec<&'static str> = RAW_TOOLTIPS
        ...
        .filter(|line| {
            ...
            if !IS_MACOS && !IS_WINDOWS && line.contains("praxis app") {
                return false;
            }
            true
        })
...
/// Pick a random tooltip to show to the user when starting Praxis.
...
pub(crate) mod announcement {
    ...
    pub(crate) fn parse_announcement_tip_toml(text: &str) -> Option<String> {
        ...
        for raw in announcements {
            ...
            if tip.version_matches(PRAXIS_CLI_VERSION)
...
from_date = "2000-01-01"
    ...
    fn announcement_tip_toml_parse_comments() {
        let toml = r#"
# Example announcement tips for Praxis TUI.
...
[[announcements]]
content = "Welcome to Praxis! Check out the new onboarding flow."
