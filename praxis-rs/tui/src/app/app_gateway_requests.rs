...
pub(super) struct AppGatewayRequestResolution {
    pub(super) request_id: AppGatewayRequestId,
    pub(super) result: serde_json::Value,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub(super) struct UnsupportedAppGatewayRequest {
    pub(super) request_id: AppGatewayRequestId,
    pub(super) message: String,
}

#[derive(Debug, Default)]
pub(super) struct PendingAppGatewayRequests {
    exec_approvals: HashMap<String, AppGatewayRequestId>,
...
impl PendingAppGatewayRequests {
    ...
    ) -> Option<UnsupportedAppGatewayRequest> {
        match request {
    ...
    pub(super) fn take_resolution<T>(
        &mut self,
        op: T,
    ) -> Result<Option<AppGatewayRequestResolution>, String>
    where
        T: Into<AppCommand>,
    {
        ...
        let resolution = match op.view() {
            AppCommandView::ExecApproval { id, decision, .. } => self
                ...
                .remove(id)
                .map(|request_id| {
                    Ok::<AppGatewayRequestResolution, String>(AppGatewayRequestResolution {
                        request_id,
                ...
                .map(|request_id| {
                    Ok::<AppGatewayRequestResolution, String>(AppGatewayRequestResolution {
                        ...
                        result: serde_json::to_value(
                            ...
                            .map_err(|err| {
                                format!(
                                    "failed to decode request_user_input response for app-gateway: {err}"
                                )
                            })?,
                        )
                        .map_err(|err| {
    ...
    pub(super) fn resolve_notification(&mut self, request_id: &AppGatewayRequestId) {
...
#[cfg(test)]
mod tests {
    use super::PendingAppGatewayRequests;
    ...
    use praxis_app_gateway_protocol::PermissionGrantScope;
    use praxis_app_gateway_protocol::PermissionsRequestApprovalParams;
    use praxis_app_gateway_protocol::PermissionsRequestApprovalResponse;
    use praxis_app_gateway_protocol::RequestId as AppGatewayRequestId;
    use praxis_app_gateway_protocol::ServerRequest;
    use praxis_app_gateway_protocol::ToolRequestUserInputAnswer;
    ...
    #[test]
    fn resolves_exec_approval_through_app_gateway_request_id() {
        let mut pending = PendingAppGatewayRequests::default();
        let request = ServerRequest::CommandExecutionRequestApproval {
            request_id: AppGatewayRequestId::Integer(41),
        ...
        assert_eq!(resolution.request_id, AppGatewayRequestId::Integer(41));
        assert_eq!(resolution.result, json!({ "decision": "accept" }));
    }

    #[test]
    fn resolves_permissions_and_user_input_through_app_gateway_request_id() {
        let mut pending = PendingAppGatewayRequests::default();
        ...
        assert_eq!(
            pending.note_server_request(&ServerRequest::ToolRequestUserInput {
                request_id: AppGatewayRequestId::Integer(8),
        ...
        let permissions = pending
            .take_resolution(&Op::RequestPermissionsResponse {
                id: "perm-1".to_string(),
                response: praxis_protocol::request_permissions::RequestPermissionsResponse {
                    ...
                    scope: praxis_protocol::request_permissions::PermissionGrantScope::Session,
                },
            })
            .expect("permissions response should serialize")
            .expect("permissions request should be pending");
        assert_eq!(permissions.request_id, AppGatewayRequestId::Integer(7));
        assert_eq!(
            ...
            PermissionsRequestApprovalResponse {
                permissions: praxis_app_gateway_protocol::GrantedPermissionProfile {
        ...
        let user_input = pending
            .take_resolution(&Op::UserInputAnswer {
                id: "turn-2".to_string(),
                response: praxis_protocol::request_user_input::RequestUserInputResponse {
                    answers: std::iter::once((
                        "question".to_string(),
                        praxis_protocol::request_user_input::RequestUserInputAnswer {
        ...
        assert_eq!(user_input.request_id, AppGatewayRequestId::Integer(8));
        assert_eq!(
    ...
    fn correlates_mcp_elicitation_server_request_with_resolution() {
        ...
        assert_eq!(
            pending.note_server_request(&ServerRequest::McpServerElicitationRequest {
                request_id: AppGatewayRequestId::Integer(12),
        ...
        assert_eq!(resolution.request_id, AppGatewayRequestId::Integer(12));
        assert_eq!(
            resolution.result,
            json!({
                "action": "accept",
    ...
    fn rejects_dynamic_tool_calls_as_unsupported() {
        ...
        let unsupported = pending
            .note_server_request(&ServerRequest::DynamicToolCall {
                request_id: AppGatewayRequestId::Integer(99),
                params: praxis_app_gateway_protocol::DynamicToolCallParams {
                    thread_id: "thread-1".to_string(),
                    turn_id: "turn-1".to_string(),
        ...
        assert_eq!(unsupported.request_id, AppGatewayRequestId::Integer(99));
        assert_eq!(
            unsupported.message,
    ...
    fn does_not_mark_chatgpt_auth_refresh_as_unsupported() {
        ...
        assert_eq!(
            pending.note_server_request(&ServerRequest::ChatgptAuthTokensRefresh {
                request_id: AppGatewayRequestId::Integer(100),
                params: praxis_app_gateway_protocol::ChatgptAuthTokensRefreshParams {
                    reason:
                        praxis_app_gateway_protocol::ChatgptAuthTokensRefreshReason::Unauthorized,
    ...
    fn rejects_invalid_patch_decisions_for_file_change_requests() {
        ...
        assert_eq!(
            pending.note_server_request(&ServerRequest::FileChangeRequestApproval {
                request_id: AppGatewayRequestId::Integer(13),
