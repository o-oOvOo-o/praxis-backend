...
impl TerminalThemeSource {
    pub(crate) const fn as_str(self) -> &'static str {
        match self {
            Self::DefaultColorsQuery => "terminal palette",
            Self::ColorFgBg => "COLORFGBG",
            Self::FallbackDark => "fallback",
        }
    }
}
...
pub(crate) struct TerminalAppearance {
    ...
    pub(crate) source: TerminalThemeSource,
}
...
pub fn best_color(target: (u8, u8, u8)) -> Color {
    best_color_for_level(target, stdout_color_level(), /*ansi16_fallback*/ None)
}

/// Returns the closest color to the target color that the terminal can display,
/// falling back to a semantic ANSI color when truecolor/256-color output is
/// unavailable or unreported.
pub(crate) fn best_color_with_ansi_fallback(target: (u8, u8, u8), ansi16_fallback: Color) -> Color {
    best_color_for_level(target, stdout_color_level(), Some(ansi16_fallback))
}
...
pub(crate) fn terminal_appearance() -> TerminalAppearance {
    resolve_terminal_appearance(default_colors(), std::env::var("COLORFGBG").ok().as_deref())
}

pub(crate) fn terminal_theme() -> TerminalTheme {
    terminal_appearance().theme
}
...
fn best_color_for_level(
    target: (u8, u8, u8),
    color_level: StdoutColorLevel,
    ansi16_fallback: Option<Color>,
) -> Color {
    match color_level {
        StdoutColorLevel::TrueColor => rgb_color(target),
        StdoutColorLevel::Ansi256 => closest_xterm_fixed_color(target)
            .map(|(i, _)| indexed_color(i as u8))
            .or(ansi16_fallback)
            .unwrap_or_default(),
        StdoutColorLevel::Ansi16 | StdoutColorLevel::Unknown => ansi16_fallback.unwrap_or_default(),
    }
}

fn closest_xterm_fixed_color(target: (u8, u8, u8)) -> Option<(usize, (u8, u8, u8))> {
    xterm_fixed_colors().min_by(|(_, a), (_, b)| {
...
fn resolve_terminal_appearance(
    ...
    if let Some(colors) = colors {
        return TerminalAppearance {
            fg: Some(colors.fg),
            bg: Some(colors.bg),
            theme: theme_from_background(colors.bg),
            source: TerminalThemeSource::DefaultColorsQuery,
        };
    }

    if let Some(theme) = terminal_theme_from_colorfgbg(colorfgbg) {
        let approximated = approximate_colors_for_theme(theme);
        return TerminalAppearance {
            fg: Some(approximated.fg),
            bg: Some(approximated.bg),
            theme,
...
fn theme_from_background(bg: (u8, u8, u8)) -> TerminalTheme {
    if is_light(bg) {
        TerminalTheme::Light
    } else {
        TerminalTheme::Dark
...
fn terminal_theme_from_colorfgbg(colorfgbg: Option<&str>) -> Option<TerminalTheme> {
    let bg_index = colorfgbg_background_index(colorfgbg?)?;
    // rxvt / COLORFGBG convention: 0-6 and 8 are dark backgrounds; 7 and 9-15
    // are light. Larger indices map to the xterm 256-color table.
    if bg_index <= 15 {
        return Some(if bg_index <= 6 || bg_index == 8 {
            TerminalTheme::Dark
        } else {
            TerminalTheme::Light
        });
    ...
    XTERM_COLORS
        ...
        .copied()
        .map(theme_from_background)
}

fn colorfgbg_background_index(colorfgbg: &str) -> Option<u8> {
    let bg = colorfgbg.split(';').next_back()?;
    let index = bg.parse::<u16>().ok()?;
    u8::try_from(index).ok()
}

fn approximate_colors_for_theme(theme: TerminalTheme) -> DefaultColors {
    match theme {
        TerminalTheme::Light => DefaultColors {
            fg: (17, 24, 39),
            bg: (248, 250, 252),
        },
        TerminalTheme::Dark => DefaultColors {
            fg: (226, 232, 240),
            bg: (15, 23, 42),
        },
    }
...
mod tests {
    ...
    #[test]
    fn resolve_terminal_appearance_prefers_queried_default_colors() {
        let appearance = resolve_terminal_appearance(
            Some(DefaultColors {
                fg: (240, 240, 240),
                bg: (12, 20, 30),
            }),
    ...
    #[test]
    fn resolve_terminal_appearance_uses_colorfgbg_when_palette_query_is_unavailable() {
        let appearance = resolve_terminal_appearance(/*colors*/ None, Some("15;7"));

        assert_eq!(appearance.theme, TerminalTheme::Light);
        assert_eq!(appearance.source, TerminalThemeSource::ColorFgBg);
        assert_eq!(appearance.bg, Some((248, 250, 252)));
    }

    #[test]
    fn resolve_terminal_appearance_falls_back_to_dark_when_no_signals_exist() {
        let appearance = resolve_terminal_appearance(/*colors*/ None, /*colorfgbg*/ None);
    ...
    #[test]
    fn terminal_theme_from_extended_colorfgbg_uses_xterm_palette_lightness() {
        assert_eq!(
            terminal_theme_from_colorfgbg(Some("15;188")),
            Some(TerminalTheme::Light)
        );
        assert_eq!(
            terminal_theme_from_colorfgbg(Some("15;235")),
            Some(TerminalTheme::Dark)
        );
    }

    #[test]
    fn best_color_with_ansi_fallback_uses_named_color_when_level_is_unknown() {
        assert_eq!(
            best_color_for_level((10, 20, 30), StdoutColorLevel::Unknown, Some(Color::Blue)),
            Color::Blue
        );
    }
}
