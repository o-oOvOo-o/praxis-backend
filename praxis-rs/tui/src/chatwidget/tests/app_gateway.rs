...
#[tokio::test]
async fn live_app_gateway_user_message_item_completed_does_not_duplicate_rendered_prompt() {
    ...
    chat.handle_server_notification(
        ServerNotification::ItemCompleted(ItemCompletedNotification {
            ...
            item: AppGatewayThreadItem::UserMessage {
                id: "user-1".to_string(),
...
#[tokio::test]
async fn live_app_gateway_turn_completed_clears_working_status_after_answer_item() {
    ...
    chat.handle_server_notification(
        ServerNotification::ItemCompleted(ItemCompletedNotification {
            thread_id: "thread-1".to_string(),
            turn_id: "turn-1".to_string(),
            item: AppGatewayThreadItem::AgentMessage {
                id: "msg-1".to_string(),
...
#[tokio::test]
async fn live_app_gateway_file_change_item_started_preserves_changes() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    ...
    chat.handle_server_notification(
        ServerNotification::ItemStarted(ItemStartedNotification {
            thread_id: "thread-1".to_string(),
            turn_id: "turn-1".to_string(),
            item: AppGatewayThreadItem::FileChange {
                ...
                status: AppGatewayPatchApplyStatus::InProgress,
            },
        }),
        /*replay_kind*/ None,
    );
...
#[tokio::test]
async fn live_app_gateway_command_execution_strips_shell_wrapper() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    ...
    chat.handle_server_notification(
        ServerNotification::ItemStarted(ItemStartedNotification {
            thread_id: "thread-1".to_string(),
            turn_id: "turn-1".to_string(),
            item: AppGatewayThreadItem::CommandExecution {
                id: "cmd-1".to_string(),
                command: command.clone(),
                cwd: PathBuf::from("/tmp"),
                process_id: None,
                source: AppGatewayCommandExecutionSource::UserShell,
    ...
    chat.handle_server_notification(
        ServerNotification::ItemCompleted(ItemCompletedNotification {
            ...
            item: AppGatewayThreadItem::CommandExecution {
                ...
                process_id: None,
                source: AppGatewayCommandExecutionSource::UserShell,
                status: AppGatewayCommandExecutionStatus::Completed,
                command_actions: vec![AppGatewayCommandAction::Unknown {
                    command: script.to_string(),
                }],
    ...
    assert_chatwidget_snapshot!(
        "live_app_gateway_command_execution_strips_shell_wrapper",
        blob
    );
}

#[test]
fn app_gateway_patch_changes_to_core_preserves_diffs() {
...
#[tokio::test]
async fn live_app_gateway_collab_wait_items_render_history() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    ...
    chat.handle_server_notification(
        ServerNotification::ItemStarted(ItemStartedNotification {
            thread_id: "thread-1".to_string(),
            turn_id: "turn-1".to_string(),
            item: AppGatewayThreadItem::CollabAgentToolCall {
                id: "wait-1".to_string(),
                tool: AppGatewayCollabAgentTool::Wait,
                status: AppGatewayCollabAgentToolCallStatus::InProgress,
    ...
    chat.handle_server_notification(
        ServerNotification::ItemCompleted(ItemCompletedNotification {
            ...
            item: AppGatewayThreadItem::CollabAgentToolCall {
                id: "wait-1".to_string(),
                tool: AppGatewayCollabAgentTool::Wait,
                status: AppGatewayCollabAgentToolCallStatus::Completed,
                sender_thread_id: sender_thread_id.to_string(),
                receiver_thread_ids: vec![
                    receiver_thread_id.to_string(),
                    other_receiver_thread_id.to_string(),
                ...
                agents_states: HashMap::from([
                    (
                        receiver_thread_id.to_string(),
                        AppGatewayCollabAgentState {
                            status: AppGatewayCollabAgentStatus::Completed,
                    ...
                    (
                        other_receiver_thread_id.to_string(),
                        AppGatewayCollabAgentState {
                            status: AppGatewayCollabAgentStatus::Running,
                            message: None,
                        },
                    ),
                ]),
    ...
    let combined = drain_insert_history(&mut rx)
        ...
        .collect::<Vec<_>>()
        .join("\n");
    assert_chatwidget_snapshot!("app_gateway_collab_wait_items_render_history", combined);
}

#[tokio::test]
async fn live_app_gateway_collab_spawn_completed_renders_requested_model_and_effort() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    let sender_thread_id =
    ...
    chat.handle_server_notification(
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: AppGatewayThreadItem::CollabAgentToolCall {
                ...
                tool: AppGatewayCollabAgentTool::SpawnAgent,
                status: AppGatewayCollabAgentToolCallStatus::InProgress,
                sender_thread_id: sender_thread_id.to_string(),
        ...
        ServerNotification::ItemCompleted(ItemCompletedNotification {
            ...
            item: AppGatewayThreadItem::CollabAgentToolCall {
                ...
                agents_states: HashMap::from([(
                    ...
                    AppGatewayCollabAgentState {
                        status: AppGatewayCollabAgentStatus::PendingInit,
                        message: None,
                    },
                )]),
            },
    ...
    let combined = drain_insert_history(&mut rx)
        ...
        .join("\n");
    assert_chatwidget_snapshot!(
        "app_gateway_collab_spawn_completed_renders_requested_model_and_effort",
        combined
    );
}

#[tokio::test]
async fn live_app_gateway_failed_turn_does_not_duplicate_error_history() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    ...
    chat.handle_server_notification(
        ServerNotification::TurnCompleted(TurnCompletedNotification {
            ...
            turn: AppGatewayTurn {
                id: "turn-1".to_string(),
                items: Vec::new(),
                status: AppGatewayTurnStatus::Failed,
                error: Some(AppGatewayTurnError {
                    message: "permission denied".to_string(),
                    praxis_error_info: None,
                    additional_details: None,
                }),
...
#[tokio::test]
async fn live_app_gateway_stream_recovery_restores_previous_status_header() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
...
#[tokio::test]
async fn live_app_gateway_server_overloaded_error_renders_warning() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
...
#[tokio::test]
async fn live_app_gateway_invalid_thread_name_update_is_ignored() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    ...
    chat.handle_server_notification(
        ServerNotification::ThreadNameUpdated(
            praxis_app_gateway_protocol::ThreadNameUpdatedNotification {
...
#[tokio::test]
async fn live_app_gateway_thread_closed_requests_immediate_exit() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
