...
pub(super) async fn test_config() -> Config {
    ...
    let praxis_home = tempfile::Builder::new()
        .prefix("chatwidget-tests-")
        .tempdir()
        .expect("tempdir")
        .keep();
    let mut config =
        Config::load_default_with_cli_overrides_for_praxis_home(praxis_home.clone(), Vec::new())
            .expect("config");
    config.praxis_home = praxis_home.clone();
    config.sqlite_home = praxis_home.clone();
    config.log_dir = praxis_home.join("log");
...
pub(super) async fn make_chatwidget_manual(
    ...
    let resolved_model = model_override
        .map(str::to_owned)
        .unwrap_or_else(|| praxis_core::test_support::get_model_offline(cfg.model.as_deref()));
    ...
    let mut widget = ChatWidget {
        app_event_tx,
        praxis_op_target: super::PraxisOpTarget::Direct(op_tx),
        bottom_pane: bottom,
        active_cell: None,
        ...
        in_app_toasts: crate::toast_queue::ToastQueue::new(/*capacity*/ 3),
        ...
        active_cell_render_cache: std::cell::RefCell::new(None),
        workspace_active_tail_cache: std::cell::RefCell::new(None),
        workspace_transcript_cache: std::cell::RefCell::new(WorkspaceTranscriptCache::default()),
...
pub(super) fn begin_exec_with_source(
    ...
    let parsed_cmd: Vec<ParsedCommand> =
        praxis_shell_command::parse_command::parse_command(&command);
...
pub(super) async fn assert_hook_events_snapshot(
    event_name: praxis_protocol::protocol::HookEventName,
    run_id: &str,
    status_message: &str,
    snapshot_name: &str,
) {
    ...
    chat.handle_praxis_event(Event {
        id: "hook-1".into(),
        msg: EventMsg::HookStarted(praxis_protocol::protocol::HookStartedEvent {
            turn_id: None,
            run: praxis_protocol::protocol::HookRunSummary {
                id: run_id.to_string(),
                event_name,
                handler_type: praxis_protocol::protocol::HookHandlerType::Command,
                execution_mode: praxis_protocol::protocol::HookExecutionMode::Sync,
                scope: praxis_protocol::protocol::HookScope::Turn,
                source_path: PathBuf::from("/tmp/hooks.json"),
                display_order: 0,
                status: praxis_protocol::protocol::HookRunStatus::Running,
    ...
    chat.handle_praxis_event(Event {
        id: "hook-1".into(),
        msg: EventMsg::HookCompleted(praxis_protocol::protocol::HookCompletedEvent {
            turn_id: None,
            run: praxis_protocol::protocol::HookRunSummary {
                id: run_id.to_string(),
                event_name,
                ...
                source_path: PathBuf::from("/tmp/hooks.json"),
                display_order: 0,
                status: praxis_protocol::protocol::HookRunStatus::Completed,
                status_message: Some(status_message.to_string()),
                started_at: 1,
                ...
                entries: vec![
                    praxis_protocol::protocol::HookOutputEntry {
                        kind: praxis_protocol::protocol::HookOutputEntryKind::Warning,
                        text: "Heads up from the hook".to_string(),
                    },
                    praxis_protocol::protocol::HookOutputEntry {
                        kind: praxis_protocol::protocol::HookOutputEntryKind::Context,
