...
async fn slash_init_skips_when_project_doc_exists() {
    ...
    match op_rx.try_recv() {
        ...
        other => panic!("expected no Praxis op to be sent, got {other:?}"),
    }
...
async fn slash_rollout_displays_current_path() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    let rollout_path = PathBuf::from("/tmp/praxis-test-rollout.jsonl");
...
#[tokio::test]
async fn slash_selfwork_without_active_plan_opens_picker() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    let tempdir = tempfile::tempdir().unwrap();
    std::fs::create_dir_all(tempdir.path().join("plans")).unwrap();
    std::fs::write(
    ...
    .unwrap();
    std::fs::write(
        tempdir.path().join("plans").join("notes.md"),
        "# Notes\n\nContext only.\n",
    )
    .unwrap();
    chat.config.cwd = tempdir.path().to_path_buf().abs();
    ...
    chat.dispatch_command(SlashCommand::Selfwork);

    let popup = render_bottom_popup(&chat, /*width*/ 100);
    assert!(
        popup.contains("Choose Selfwork Plan"),
        "missing picker title:\n{popup}"
    );
    assert!(
        popup.contains("plan.md"),
        "missing primary plan entry:\n{popup}"
    );
    assert!(
        popup.contains("2 unfinished of 2 checklist items"),
        "missing checklist summary:\n{popup}"
    );
    assert!(
        popup.contains("Type a path manually"),
        "missing manual path fallback:\n{popup}"
    );
}

#[tokio::test]
async fn slash_selfwork_picker_enter_dispatches_activation_event() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    let tempdir = tempfile::tempdir().unwrap();
    let expected_plan = tempdir.path().join("plan.md");
    std::fs::write(&expected_plan, "- [ ] One task\n").unwrap();
    let expected_plan = expected_plan.canonicalize().unwrap();
    chat.config.cwd = tempdir.path().to_path_buf().abs();
    chat.thread_id = Some(ThreadId::new());
    ...
    match rx.try_recv() {
        Ok(AppEvent::ActivateSelfworkPlan { plan_path }) => {
            assert_eq!(plan_path.canonicalize().unwrap(), expected_plan);
        }
        other => panic!("expected selfwork activation event, got {other:?}"),
    }
}

#[tokio::test]
async fn slash_selfwork_without_markdown_candidates_opens_manual_prompt() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    let tempdir = tempfile::tempdir().unwrap();
    chat.config.cwd = tempdir.path().to_path_buf().abs();
    chat.thread_id = Some(ThreadId::new());
    ...
    let popup = render_bottom_popup(&chat, /*width*/ 100);
    assert!(
        popup.contains("Type a markdown plan path and press Enter"),
        "expected manual selfwork prompt, got:\n{popup}"
    );
}
