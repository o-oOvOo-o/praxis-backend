...
/// custom selection.
///
/// Keep project and thread name first so terminal tabs stay scannable, then
/// append spinner while work is active.
pub(super) const DEFAULT_TERMINAL_TITLE_ITEMS: [&str; 3] = ["project", "thread", "spinner"];
...
impl ChatWidget {
    ...
    fn refresh_footer_right_badge_from_selections(&mut self, selections: &StatusSurfaceSelections) {
        let badge = if selections.status_line_items.is_empty() {
            let effort =
                Self::status_line_reasoning_effort_label(self.effective_reasoning_effort());
            Some(Line::from(vec![
                Span::styled("●", Style::default().fg(Color::Rgb(255, 153, 68)).bold()),
                Span::raw(" "),
                Span::styled(effort.to_string(), Style::default().bold()),
                Span::styled(" · ", Style::default().dim()),
                Span::styled("/effort", Style::default().dim()),
            ]))
        } else {
            None
        };
        self.bottom_pane.set_footer_right_badge(badge);
    }

    /// Clears the terminal title Praxis most recently wrote, if any.
    ///
    ...
    pub(crate) fn refresh_status_surfaces(&mut self) {
        ...
        self.refresh_footer_right_badge_from_selections(&selections);
        ...
        self.config
            ...
            .find_map(|layer| match &layer.name {
                ConfigLayerSource::Project { dot_praxis_folder } => {
                    dot_praxis_folder.as_path().parent().map(Path::to_path_buf)
                }
                _ => None,
            })
    }
    ...
    ///
    /// This prefers the current directory name so terminal tabs track the
    /// user's immediate working location, and falls back to the cached
    /// project-root label when the cwd has no displayable leaf name.
    fn terminal_title_project_name(&mut self) -> Option<String> {
        let cwd = self.status_line_cwd().to_path_buf();
        let cwd_name = cwd
            .file_name()
            .map(|name| name.to_string_lossy().to_string());
        let project = cwd_name
