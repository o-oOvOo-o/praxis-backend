...
use praxis_app_server_protocol::Model as ApiModel;
...
use praxis_app_server_protocol::ReviewDelivery;
...
fn sandbox_mode_from_policy(
    ...
    match policy {
        ...
        SandboxPolicy::ReadOnly { .. } => Some(praxis_app_server_protocol::SandboxMode::ReadOnly),
...
) -> praxis_app_server_protocol::ReviewTarget {
    match target {
        CoreReviewTarget::UncommittedChanges => {
            praxis_app_server_protocol::ReviewTarget::UncommittedChanges
        }
        CoreReviewTarget::BaseBranch { branch } => {
            praxis_app_server_protocol::ReviewTarget::BaseBranch { branch }
        }
        CoreReviewTarget::Commit { sha, title } => {
            praxis_app_server_protocol::ReviewTarget::Commit { sha, title }
        }
        CoreReviewTarget::Custom { instructions } => {
            praxis_app_server_protocol::ReviewTarget::Custom { instructions }
        }
    }
}
...
pub(crate) fn app_server_rate_limit_snapshot_to_core(
    snapshot: praxis_app_server_protocol::RateLimitSnapshot,
) -> RateLimitSnapshot {
    RateLimitSnapshot {
...
fn app_server_rate_limit_window_to_core(
    window: praxis_app_server_protocol::RateLimitWindow,
) -> RateLimitWindow {
    RateLimitWindow {
...
fn app_server_credits_snapshot_to_core(
    snapshot: praxis_app_server_protocol::CreditsSnapshot,
) -> CreditsSnapshot {
    CreditsSnapshot {
        ...
        let response = ThreadResumeResponse {
            thread: praxis_app_server_protocol::Thread {
                ...
                turns: vec![Turn {
                    id: "turn-1".to_string(),
                    items: vec![
                        praxis_app_server_protocol::ThreadItem::UserMessage {
                            id: "user-1".to_string(),
                            content: vec![praxis_app_server_protocol::UserInput::Text {
                                text: "hello from history".to_string(),
                                text_elements: Vec::new(),
                            }],
                        },
                        praxis_app_server_protocol::ThreadItem::AgentMessage {
            ...
            approvals_reviewer: praxis_app_server_protocol::ApprovalsReviewer::User,
