...
use praxis_app_server_protocol::GrantedPermissionProfile;
...
pub(crate) fn network_approval_context_to_core(
    ...
    NetworkApprovalContext {
        ...
        protocol: match value.protocol {
            praxis_app_server_protocol::NetworkApprovalProtocol::Http => {
                NetworkApprovalProtocol::Http
            }
            praxis_app_server_protocol::NetworkApprovalProtocol::Https => {
                NetworkApprovalProtocol::Https
            }
            praxis_app_server_protocol::NetworkApprovalProtocol::Socks5Tcp => {
                NetworkApprovalProtocol::Socks5Tcp
            }
            praxis_app_server_protocol::NetworkApprovalProtocol::Socks5Udp => {
...
mod tests {
    ...
    fn converts_app_server_network_approval_context_to_core() {
        assert_eq!(
            network_approval_context_to_core(praxis_app_server_protocol::NetworkApprovalContext {
    ...
    fn converts_request_permissions_into_granted_permissions() {
        assert_eq!(
            ...
            praxis_app_server_protocol::GrantedPermissionProfile {
                network: Some(praxis_app_server_protocol::AdditionalNetworkPermissions {
                    enabled: Some(true),
                }),
                file_system: Some(
