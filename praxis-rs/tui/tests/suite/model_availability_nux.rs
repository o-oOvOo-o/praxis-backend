...
async fn resume_startup_does_not_consume_model_availability_nux_count() -> Result<()> {
    // run_praxis_cli() does not work on Windows due to PTY limitations.
    if cfg!(windows) {
        return Ok(());
    }
    ...
    let source_catalog_path = praxis_utils_cargo_bin::find_resource!("../core/models.json")?;
    ...
    let custom_catalog_path = praxis_home.path().join("catalog.json");
    std::fs::write(
        &custom_catalog_path,
...
"#
    ...
    std::fs::write(praxis_home.path().join("config.toml"), config_contents)?;
    ...
    env.insert(
        ...
        praxis_home.path().display().to_string(),
    );
    ...
    let spawned = praxis_utils_pty::spawn_pty_process(
        ...
        &env,
        &None,
        praxis_utils_pty::TerminalSize::default(),
    )
    .await?;

    let mut output = Vec::new();
    let praxis_utils_pty::SpawnedProcess {
        session,
        stdout_rx,
        stderr_rx,
        exit_rx,
    } = spawned;
    let mut output_rx = praxis_utils_pty::combine_output_receivers(stdout_rx, stderr_rx);
