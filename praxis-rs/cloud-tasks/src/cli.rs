...
pub enum Command {
    /// Submit a new Praxis Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a Praxis Cloud task.
    Status(StatusCommand),
    /// List Praxis Cloud tasks.
    List(ListCommand),
    /// Apply the diff for a Praxis Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a Praxis Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in Praxis Cloud.
    #[arg(value_name = "QUERY")]
    pub query: Option<String>,

    /// Target environment identifier (see `praxis cloud` to browse).
    ...
    /// Git branch to run in Praxis Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// Praxis Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// Praxis Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// Praxis Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
