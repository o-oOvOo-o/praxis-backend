...
`praxis-exec-server` is a small standalone JSON-RPC server for spawning
and controlling subprocesses through `praxis-utils-pty`.
...
- a standalone binary: `praxis-exec-server`
...
- `outputBytesCap`: maximum retained stdout/stderr bytes per stream for the
  in-memory buffer. Defaults to `praxis_utils_pty::DEFAULT_OUTPUT_BYTES_CAP`.
- `arg0`: optional argv0 override forwarded to `praxis-utils-pty`.
