...
#[derive(Parser, Debug)]
#[command(
    about = "Generate TypeScript bindings and JSON Schemas for the Praxis app-server protocol"
)]
struct Args {
...
fn main() -> Result<()> {
    ...
    praxis_app_server_protocol::generate_ts_with_options(
        ...
        praxis_app_server_protocol::GenerateTsOptions {
            experimental_api: args.experimental,
            ..praxis_app_server_protocol::GenerateTsOptions::default()
        },
    )?;
    praxis_app_server_protocol::generate_json_with_experimental(&args.out_dir, args.experimental)
}
