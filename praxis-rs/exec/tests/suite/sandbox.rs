...
async fn spawn_command_under_sandbox(
    ...
    use praxis_core::landlock::spawn_command_under_linux_sandbox;
    let praxis_linux_sandbox_exe = core_test_support::find_praxis_linux_sandbox_exe()
...
#[tokio::test]
async fn sandbox_blocks_first_time_dot_praxis_creation() {
    core_test_support::skip_if_sandbox!();
    #[cfg(target_os = "linux")]
    let sandbox_env = match linux_sandbox_test_env().await {
        Some(env) => env,
