...
use praxis_execpolicy::rule::PatternToken;
use praxis_execpolicy::rule::PrefixPattern;
use praxis_execpolicy::rule::PrefixRule;
use praxis_execpolicy::rule::RuleRef;
...
pub enum RequirementsExecPolicyParseError {
    ...
    #[error(
        "rules prefix_rule at index {rule_index} has decision 'allow', which is not permitted in requirements.toml: Praxis merges these rules with other config and uses the most restrictive result (use 'prompt' or 'forbidden')"
    )]
...
impl RequirementsExecPolicyToml {
    ...
    /// representation used by `praxis-execpolicy`.
