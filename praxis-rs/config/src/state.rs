...
use praxis_app_server_protocol::ConfigLayer;
...
fn verify_layer_ordering(layers: &[ConfigLayerEntry]) -> std::io::Result<Option<usize>> {
    ...
    let mut previous_project_dot_praxis_folder: Option<&AbsolutePathBuf> = None;
    for (index, layer) in layers.iter().enumerate() {
        ...
        if let ConfigLayerSource::Project {
            dot_praxis_folder: current_project_dot_praxis_folder,
        } = &layer.name
        {
            if let Some(previous) = previous_project_dot_praxis_folder {
                ...
                if previous == current_project_dot_praxis_folder
                    || !current_project_dot_praxis_folder
                        .as_path()
                        .ancestors()
            ...
            previous_project_dot_praxis_folder = Some(current_project_dot_praxis_folder);
        }
    }
