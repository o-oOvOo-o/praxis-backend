...
const PROXY_SOCKET_DIR_PREFIX: &str = "praxis-linux-sandbox-proxy-";
...
fn proxy_socket_parent_dir() -> PathBuf {
    if let Some(praxis_home) = std::env::var_os("PRAXIS_HOME") {
        let candidate = PathBuf::from(praxis_home).join("tmp");
...
mod tests {
    ...
    fn cleanup_proxy_socket_dir_removes_bridge_artifacts() {
        ...
        let socket_dir = root.path().join("praxis-linux-sandbox-proxy-test");
    ...
    fn parse_proxy_socket_dir_owner_pid_reads_owner_pid() {
        assert_eq!(
            parse_proxy_socket_dir_owner_pid("praxis-linux-sandbox-proxy-1234-0"),
            Some(1234)
        );
        assert_eq!(
            parse_proxy_socket_dir_owner_pid("praxis-linux-sandbox-proxy-x"),
            None
        );
