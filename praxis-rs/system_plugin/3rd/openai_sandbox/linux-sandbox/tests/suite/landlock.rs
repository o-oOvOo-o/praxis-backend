...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> praxis_core::exec::ExecToolCallOutput {
...
async fn run_cmd_result_with_writable_roots(
    ...
    network_access: bool,
) -> Result<praxis_core::exec::ExecToolCallOutput> {
...
async fn run_cmd_result_with_policies(
    ...
    let sandbox_program = env!("CARGO_BIN_EXE_praxis-linux-sandbox");
    let praxis_linux_sandbox_exe = Some(PathBuf::from(sandbox_program));
...
fn is_bwrap_unavailable_output(output: &praxis_core::exec::ExecToolCallOutput) -> bool {
...
fn expect_denied(
    result: Result<praxis_core::exec::ExecToolCallOutput>,
    context: &str,
) -> praxis_core::exec::ExecToolCallOutput {
    match result {
        Ok(output) => {
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let sandbox_program = env!("CARGO_BIN_EXE_praxis-linux-sandbox");
    let praxis_linux_sandbox_exe: Option<PathBuf> = Some(PathBuf::from(sandbox_program));
...
#[tokio::test]
async fn sandbox_blocks_git_and_praxis_writes_inside_writable_root() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let praxis_output = expect_denied(
        run_cmd_result_with_writable_roots(
            &[
                "bash",
                "-lc",
                &format!("echo denied > {}", praxis_target.to_string_lossy()),
            ],
            &[tmpdir.path().to_path_buf()],
            LONG_TIMEOUT_MS,
            /*use_legacy_landlock*/ false,
...
#[tokio::test]
async fn sandbox_blocks_praxis_symlink_replacement_attack() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let tmpdir = tempfile::tempdir().expect("tempdir");
    let decoy = tmpdir.path().join("decoy-praxis");
...
async fn sandbox_blocks_explicit_split_policy_carveouts_under_bwrap() {
    ...
    let sandbox_helper_dir = PathBuf::from(env!("CARGO_BIN_EXE_praxis-linux-sandbox"))
        .parent()
