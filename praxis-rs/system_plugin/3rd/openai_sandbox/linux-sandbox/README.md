# praxis-linux-sandbox

This crate is responsible for producing:

- a `praxis-linux-sandbox` standalone executable for Linux that is bundled with the Node.js version of the Praxis CLI
- a lib crate that exposes the business logic of the executable as `run_main()` so that
  - the `praxis-exec` CLI can check if its arg0 is `praxis-linux-sandbox` and, if so, execute as if it were `praxis-linux-sandbox`
...
- When the default bubblewrap pipeline is active, protected subpaths under writable roots (for
  example `.git`,
  resolved `gitdir:`, `.praxis`, and legacy `.codex`) are re-applied as read-only via `--ro-bind`.
...
**Notes**
- Legacy Codex state paths remain protected as compatibility carveouts.
