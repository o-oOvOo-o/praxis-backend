...
pub struct CapSids {
    ...
    /// apply per-workspace denies (e.g. protect `CWD/.praxis`)
    /// without permanently affecting other workspaces.
    #[serde(default)]
    pub workspace_by_cwd: HashMap<String, String>,
}

pub fn cap_sid_file(praxis_home: &Path) -> PathBuf {
    praxis_home.join("cap_sid")
}
...
pub fn load_or_create_cap_sids(praxis_home: &Path) -> Result<CapSids> {
    let path = cap_sid_file(praxis_home);
    if path.exists() {
...
pub fn workspace_cap_sid_for_cwd(praxis_home: &Path, cwd: &Path) -> Result<String> {
    let path = cap_sid_file(praxis_home);
    let mut caps = load_or_create_cap_sids(praxis_home)?;
...
mod tests {
    ...
    fn equivalent_cwd_spellings_share_workspace_sid_key() {
        ...
        let first_sid =
            workspace_cap_sid_for_cwd(&praxis_home, canonical.as_path()).expect("first sid");
        let second_sid =
            workspace_cap_sid_for_cwd(&praxis_home, alt_spelling.as_path()).expect("second sid");

        assert_eq!(first_sid, second_sid);

        let caps = load_or_create_cap_sids(&praxis_home).expect("load caps");
