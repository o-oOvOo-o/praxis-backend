...
#[cfg(target_os = "windows")]
pub use workspace_acl::protect_workspace_praxis_dir;
...
mod windows_impl {
    ...
    use super::workspace_acl::protect_workspace_legacy_codex_dir;
    use super::workspace_acl::protect_workspace_praxis_dir;
    use anyhow::Result;
    ...
    pub fn run_windows_sandbox_capture_with_extra_deny_write_paths(
        ...
        ensure_praxis_home_exists(praxis_home)?;
        ...
        let (h_token, psid_generic, psid_workspace): (HANDLE, *mut c_void, Option<*mut c_void>) = unsafe {
            match &policy {
                ...
                SandboxPolicy::WorkspaceWrite { .. } => {
                    ...
                    let ws_sid = workspace_cap_sid_for_cwd(praxis_home, cwd)?;
                    #[allow(clippy::expect_used)]
                    let psid_workspace =
        ...
        unsafe {
            ...
            if let Some(psid) = psid_workspace {
                ...
                let _ = protect_workspace_praxis_dir(&current_dir, psid);
                let _ = protect_workspace_legacy_codex_dir(&current_dir, psid);
    ...
    pub fn run_windows_sandbox_legacy_preflight(
        ...
        unsafe {
            ...
            let _ = protect_workspace_praxis_dir(&current_dir, psid_workspace);
            let _ = protect_workspace_legacy_codex_dir(&current_dir, psid_workspace);
