...
use praxis_windows_sandbox::LOG_FILE_NAME;
use praxis_windows_sandbox::SETUP_VERSION;
use praxis_windows_sandbox::SetupErrorCode;
use praxis_windows_sandbox::SetupErrorReport;
use praxis_windows_sandbox::SetupFailure;
use praxis_windows_sandbox::canonicalize_path;
use praxis_windows_sandbox::convert_string_sid_to_sid;
use praxis_windows_sandbox::ensure_allow_mask_aces_with_inheritance;
use praxis_windows_sandbox::ensure_allow_write_aces;
use praxis_windows_sandbox::extract_setup_failure;
use praxis_windows_sandbox::hide_newly_created_users;
use praxis_windows_sandbox::is_command_cwd_root;
use praxis_windows_sandbox::load_or_create_cap_sids;
use praxis_windows_sandbox::log_note;
use praxis_windows_sandbox::path_mask_allows;
use praxis_windows_sandbox::protect_workspace_agents_dir;
use praxis_windows_sandbox::protect_workspace_legacy_codex_dir;
use praxis_windows_sandbox::protect_workspace_praxis_dir;
use praxis_windows_sandbox::sandbox_bin_dir;
use praxis_windows_sandbox::sandbox_dir;
use praxis_windows_sandbox::sandbox_secrets_dir;
use praxis_windows_sandbox::string_from_sid_bytes;
use praxis_windows_sandbox::to_wide;
use praxis_windows_sandbox::workspace_cap_sid_for_cwd;
use praxis_windows_sandbox::write_setup_error_report;
...
pub fn main() -> Result<()> {
    ...
    if let Err(e) = &ret {
        ...
        if let Ok(praxis_home) = std::env::var("PRAXIS_HOME") {
            let sbx_dir = sandbox_dir(Path::new(&praxis_home));
...
fn real_main() -> Result<()> {
    ...
    let sbx_dir = sandbox_dir(&payload.praxis_home);
    ...
    if let Err(err) = &result {
        ...
        if let Err(write_err) = write_setup_error_report(&payload.praxis_home, &report) {
            let _ = log_line(
                &mut log,
...
fn run_setup_full(payload: &Payload, log: &mut File, sbx_dir: &Path) -> Result<()> {
    ...
    let caps = load_or_create_cap_sids(&payload.praxis_home).map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
    ...
    let workspace_sid_str = workspace_cap_sid_for_cwd(&payload.praxis_home, &payload.command_cwd)?;
    ...
    lock_sandbox_dir(
        &sandbox_bin_dir(&payload.praxis_home),
        &payload.real_user,
        &sandbox_group_sid,
        GRANT_ACCESS,
        FILE_GENERIC_READ | FILE_GENERIC_EXECUTE,
    ...
    .map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
            ...
            format!(
                ...
                sandbox_bin_dir(&payload.praxis_home).display()
            ),
        ))
    })?;
    ...
    if !refresh_only {
        lock_sandbox_dir(
            &sandbox_dir(&payload.praxis_home),
            &payload.real_user,
            &sandbox_group_sid,
            GRANT_ACCESS,
            FILE_GENERIC_READ | FILE_GENERIC_WRITE | FILE_GENERIC_EXECUTE | DELETE,
        ...
        .map_err(|err| {
            anyhow::Error::new(SetupFailure::new(
                ...
                format!(
                    ...
                    sandbox_dir(&payload.praxis_home).display()
                ),
            ))
        })?;
        lock_sandbox_dir(
            &sandbox_secrets_dir(&payload.praxis_home),
            &payload.real_user,
            &sandbox_group_sid,
            DENY_ACCESS,
            FILE_GENERIC_READ | FILE_GENERIC_WRITE | FILE_GENERIC_EXECUTE | DELETE,
        ...
        .map_err(|err| {
            anyhow::Error::new(SetupFailure::new(
                ...
                format!(
                    ...
                    sandbox_secrets_dir(&payload.praxis_home).display()
                ),
            ))
        })?;
        let legacy_users = sandbox_dir(&payload.praxis_home).join("sandbox_users.json");
        if legacy_users.exists() {
            let _ = std::fs::remove_file(&legacy_users);
        }
    }

    // Protect the current workspace's Praxis, legacy Codex, and `.agents` directories from tampering
    ...
    match unsafe { protect_workspace_praxis_dir(&payload.command_cwd, workspace_psid) } {
        Ok(true) => {
            let cwd_praxis = payload.command_cwd.join(".praxis");
            log_line(
                log,
                &format!(
                    "applied deny ACE to protect workspace .praxis {}",
                    cwd_praxis.display()
                ),
            )?;
        }
        ...
        Err(err) => {
            ...
            log_line(
                log,
                &format!("deny ACE failed on {}: {err}", cwd_praxis.display()),
            )?;
        }
    }
    match unsafe { protect_workspace_legacy_codex_dir(&payload.command_cwd, workspace_psid) } {
        Ok(true) => {
            ...
            log_line(
                log,
                &format!(
                    "applied deny ACE to protect legacy workspace .codex {}",
                    cwd_codex.display()
                ),
            )?;
        }
        ...
        Err(err) => {
            ...
            log_line(
                log,
                &format!("deny ACE failed on legacy {}: {err}", cwd_codex.display()),
            )?;
        }
    }
