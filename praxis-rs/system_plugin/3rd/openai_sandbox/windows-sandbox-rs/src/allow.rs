        ...
        let add_writable_root =
            |root: PathBuf,
             ...
             add_deny: &mut dyn FnMut(PathBuf)| {
                ...
                for protected_subdir in [".git", ".praxis", ".codex", ".agents"] {
...
mod tests {
    ...
    #[test]
    fn denies_praxis_and_agents_inside_writable_root() {
        let tmp = TempDir::new().expect("tempdir");
        let command_cwd = tmp.path().join("workspace");
        let praxis_dir = command_cwd.join(".praxis");
        let legacy_codex_dir = command_cwd.join(".codex");
        ...
        let expected_deny: HashSet<PathBuf> = [
            dunce::canonicalize(&praxis_dir).unwrap(),
