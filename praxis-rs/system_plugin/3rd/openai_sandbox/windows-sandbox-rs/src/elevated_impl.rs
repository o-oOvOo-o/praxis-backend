...
mod windows_impl {
    ...
    /// Creates the sandbox user's Praxis home directory if it does not already exist.
    fn ensure_praxis_home_exists(p: &Path) -> Result<()> {
    ...
    /// Resolves the command runner path, preferring PRAXIS_HOME/.sandbox/bin.
    fn find_runner_exe(praxis_home: &Path, log_dir: Option<&Path>) -> PathBuf {
        resolve_helper_for_launch(HelperExecutable::CommandRunner, praxis_home, log_dir)
    }
    ...
    pub fn run_windows_sandbox_capture(
        ...
        let sandbox_base = praxis_home.join(".sandbox");
        ensure_praxis_home_exists(&sandbox_base)?;
        ...
        let caps = load_or_create_cap_sids(praxis_home)?;
        let (psid_to_use, cap_sids) = match &policy {
            ...
            SandboxPolicy::WorkspaceWrite { .. } => {
                ...
                (
                    ...
                    vec![
                        caps.workspace,
                        crate::cap::workspace_cap_sid_for_cwd(praxis_home, cwd)?,
                    ],
                )
            }
            SandboxPolicy::DangerFullAccess | SandboxPolicy::ExternalSandbox { .. } => {
        ...
        let runner_exe = find_runner_exe(praxis_home, logs_base_dir);
        let runner_cmdline = runner_exe
            .to_str()
            .map(ToString::to_string)
            .unwrap_or_else(|| "praxis-command-runner.exe".to_string());
            ...
            let spawn_request = FramedMessage {
                ...
                message: Message::SpawnRequest {
                    payload: Box::new(SpawnRequest {
                        ...
                        praxis_home: sandbox_base.clone(),
                        real_praxis_home: praxis_home.to_path_buf(),
                        cap_sids,
                        timeout_ms,
                        tty: false,
                        stdin_open: false,
