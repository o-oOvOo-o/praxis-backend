...
use praxis_utils_cargo_bin::CargoBinError;
...
fn enable_deterministic_unified_exec_process_ids_for_tests() {
    praxis_core::test_support::set_thread_manager_test_mode(/*enabled*/ true);
    praxis_core::test_support::set_deterministic_process_ids(/*enabled*/ true);
}

#[ctor]
fn configure_arg0_dispatch_for_test_binaries() {
    let _ = TEST_ARG0_PATH_ENTRY.get_or_init(praxis_arg0::arg0_dispatch);
}
...
fn configure_insta_workspace_root_for_snapshot_tests() {
    ...
    let workspace_root = praxis_utils_cargo_bin::repo_root()
        .ok()
        .map(|root| root.join("praxis-rs"));
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        praxis_linux_sandbox_exe: Some(
            find_praxis_linux_sandbox_exe().expect("should find binary for praxis-linux-sandbox"),
        ),
...
#[cfg(target_os = "linux")]
pub fn find_praxis_linux_sandbox_exe() -> Result<PathBuf, CargoBinError> {
    ...
    praxis_utils_cargo_bin::cargo_bin("praxis-linux-sandbox")
}
...
pub fn format_with_current_shell(command: &str) -> Vec<String> {
    praxis_core::shell::default_user_shell()
...
pub fn stdio_server_bin() -> Result<String, CargoBinError> {
    praxis_utils_cargo_bin::cargo_bin("test_stdio_server").map(|p| p.to_string_lossy().to_string())
}
...
#[macro_export]
macro_rules! praxis_linux_sandbox_exe_or_skip {
    () => {{
        #[cfg(target_os = "linux")]
        {
            match $crate::find_praxis_linux_sandbox_exe() {
                Ok(path) => Some(path),
                Err(err) => {
                    eprintln!("praxis-linux-sandbox binary not available, skipping test: {err}");
                    return;
                }
            }
        }
