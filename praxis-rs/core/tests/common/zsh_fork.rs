...
pub fn zsh_fork_runtime(test_name: &str) -> Result<Option<ZshForkRuntime>> {
    ...
    let Ok(main_execve_wrapper_exe) = praxis_utils_cargo_bin::cargo_bin("praxis-execve-wrapper")
    else {
        eprintln!("skipping {test_name}: unable to resolve `praxis-execve-wrapper` binary");
        return Ok(None);
    };
