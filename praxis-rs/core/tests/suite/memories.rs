...
async fn init_state_db(home: &Arc<TempDir>) -> Result<Arc<praxis_state::StateRuntime>> {
    let db =
        praxis_state::StateRuntime::init(home.path().to_path_buf(), "test-provider".into()).await?;
    db.mark_backfill_complete(/*last_watermark*/ None).await?;
    Ok(db)
}

async fn seed_stage1_output(
    db: &praxis_state::StateRuntime,
    praxis_home: &Path,
    ...
    let mut metadata_builder = praxis_state::ThreadMetadataBuilder::new(
        thread_id,
        praxis_home.join(format!("rollout-{thread_id}.jsonl")),
        updated_at,
        SessionSource::Cli,
    );
    metadata_builder.cwd = praxis_home.join(format!("workspace-{rollout_slug}"));
