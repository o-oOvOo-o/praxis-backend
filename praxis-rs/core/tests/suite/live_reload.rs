...
fn enable_trusted_project(config: &mut praxis_core::config::Config) {
...
async fn live_skills_reload_refreshes_skill_cache_after_skill_change() -> Result<()> {
    ...
    let skill_path = dunce::canonicalize(test.praxis_home_path().join("skills/demo/SKILL.md"))?;
    ...
    write_skill(test.praxis_home_path(), "demo", "demo skill", skill_v2);
    ...
    if saw_skills_update.is_err() {
        ...
        test.thread_manager.skills_capability().clear_cache();
    }
