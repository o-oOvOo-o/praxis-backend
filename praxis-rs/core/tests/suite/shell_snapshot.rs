...
async fn wait_for_snapshot(praxis_home: &Path) -> Result<PathBuf> {
    let snapshot_dir = praxis_home.join("shell_snapshots");
...
async fn linux_unified_exec_uses_shell_snapshot() -> Result<()> {
    ...
    assert!(run.snapshot_path.starts_with(&run.praxis_home));
...
async fn shell_command_snapshot_preserves_shell_environment_policy_set() -> Result<()> {
    ...
    assert!(snapshot_path.starts_with(praxis_home));
