...
async fn openai_model_header_mismatch_emits_warning_event_and_warning_item() -> Result<()> {
    ...
    let mut builder = test_praxis().with_model(REQUESTED_MODEL);
    ...
    let reroute = wait_for_event(&test.thread, |event| {
    ...
    let warning = wait_for_event(&test.thread, |event| matches!(event, EventMsg::Warning(_))).await;
    ...
    let warning_item = wait_for_event(&test.thread, |event| {
        matches!(
            event,
    ...
    let _ = wait_for_event(&test.thread, |event| {
...
async fn response_model_field_mismatch_emits_warning_when_header_matches_requested() -> Result<()> {
    ...
    let warning = wait_for_event(&test.thread, |event| {
        matches!(
            event,
