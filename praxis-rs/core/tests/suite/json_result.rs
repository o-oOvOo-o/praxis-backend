...
async fn praxis_returns_json_result_for_gpt5() -> anyhow::Result<()> {
    praxis_returns_json_result("gpt-5.1".to_string()).await
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn praxis_returns_json_result_for_gpt5_codex() -> anyhow::Result<()> {
    praxis_returns_json_result("gpt-5.1-codex".to_string()).await
}

async fn praxis_returns_json_result(model: String) -> anyhow::Result<()> {
    ...
    let match_json_text_param = move |req: &wiremock::Request| {
        ...
        format.get("name") == Some(&serde_json::Value::String("praxis_output_schema".into()))
