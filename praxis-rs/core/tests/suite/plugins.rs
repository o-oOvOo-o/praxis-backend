...
async fn explicit_plugin_mentions_inject_plugin_guidance() -> Result<()> {
    ...
    assert!(
        request_tools
            .iter()
            .any(|name| name == "mcp__praxis_apps__google_calendar_create_event"),
    ...
    let calendar_description = tool_description(
        &request_body,
        "mcp__praxis_apps__google_calendar_create_event",
    )
...
async fn explicit_plugin_mentions_track_plugin_used_analytics() -> Result<()> {
        ...
        if let Some(event) = requests
            ...
            .find_map(|request| {
                ...
                payload["events"].as_array().and_then(|events| {
                    events
                        .iter()
                        .find(|event| event["event_type"] == "praxis_plugin_used")
                        .cloned()
                })
            })
        {
    ...
    assert_eq!(
        ...
        serde_json::json!(praxis_login::default_client::originator().value)
    );
