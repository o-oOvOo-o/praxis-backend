...
async fn user_turn_includes_skill_instructions() -> Result<()> {
    ...
    let skill_path = test.praxis_home_path().join("skills/demo/SKILL.md");
...
async fn list_skills_includes_system_cache_entries() -> Result<()> {
    ...
    let system_skill_path = system_skill_md_path(test.praxis_home_path(), SYSTEM_SKILL_NAME);
    assert!(
        system_skill_path.exists(),
    ...
    assert_eq!(skill.scope, praxis_protocol::protocol::SkillScope::System);
