...
//! request payload that Praxis would send to the model and assert that the
...
async fn compact_conversation(conversation: &Arc<PraxisThread>) {
    conversation
        .submit(Op::Compact)
        .await
...
fn fetch_conversation_path(conversation: &Arc<PraxisThread>) -> std::path::PathBuf {
