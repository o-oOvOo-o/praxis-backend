...
async fn responses_stream_includes_subagent_header_on_review() {
    ...
    let praxis_home = TempDir::new().expect("failed to create TempDir");
    ...
    assert_eq!(request.header("x-praxis-sandbox"), None);
}
...
async fn responses_stream_includes_turn_metadata_header_for_git_workspace_e2e() {
    ...
    let initial_header = first_request
        .single_request()
        .header("x-praxis-turn-metadata")
        .expect("x-praxis-turn-metadata header should be present");
    let initial_parsed: serde_json::Value =
        serde_json::from_str(&initial_header).expect("x-praxis-turn-metadata should be valid JSON");
    ...
    assert!(
        ...
        "turn_id should not be empty in x-praxis-turn-metadata"
    );
    assert_eq!(
        initial_parsed
            .get("sandbox")
