...
Note that `praxis-core` makes some assumptions about certain helper utilities being available in the environment. Currently, this support matrix is:
...
Expects the binary containing `praxis-core` to simulate the virtual `apply_patch` CLI when `arg1` is `--praxis-run-as-apply-patch`. See the `praxis-arg0` crate for details.
