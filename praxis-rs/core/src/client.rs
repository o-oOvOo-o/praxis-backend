...
//! provider selection, conversation id, and transport fallback state).
//!
...
/// the mapped `PraxisErr` is returned to the caller.
        ...
        && recovery.has_next()
    {
        ...
        return match recovery.next().await {
            ...
            Err(RefreshTokenError::Permanent(failed)) => {
                ...
                Err(PraxisErr::RefreshTokenFailed(failed))
            }
...
async fn handle_unauthorized_unexpected_status(
    ...
    let debug = ResponseDebugContext {
        ...
        cf_ray: err.cf_ray.clone(),
        auth_error: err.identity_authorization_error.clone(),
        auth_error_code: err.identity_error_code.clone(),
    };
    ...
    Err(PraxisErr::UnexpectedStatus(UnexpectedResponseError {
        status: err.status,
        body: err.body.clone(),
        url: err.url.clone(),
        cf_ray: err.cf_ray.clone(),
        request_id: err.request_id.clone(),
        identity_authorization_error: err.identity_authorization_error.clone(),
        identity_error_code: err.identity_error_code.clone(),
    }))
}
