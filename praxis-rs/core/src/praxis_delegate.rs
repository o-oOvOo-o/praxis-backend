...
use crate::praxis::SUBMISSION_CHANNEL_CAPACITY;
...
#[cfg(test)]
use crate::praxis::completed_session_loop_termination;

/// Start an interactive sub-Praxis thread and return IO channels.
///
...
pub(crate) async fn run_praxis_thread_interactive(
    ...
    let praxis_for_events = Arc::clone(&praxis);
    ...
    let praxis_for_ops = Arc::clone(&praxis);
    tokio::spawn(async move {
        forward_ops(praxis_for_ops, rx_ops, cancel_token_ops).await;
    });

    Ok(Praxis {
        tx_sub: tx_ops,
        rx_event: rx_sub,
        agent_status: praxis.agent_status.clone(),
        session: Arc::clone(&praxis.session),
        session_loop_termination: praxis.session_loop_termination.clone(),
    })
}
...
async fn forward_events(
    ...
    loop {
        tokio::select! {
            _ = &mut cancelled => {
                shutdown_delegate(&praxis).await;
                break;
            }
            event = praxis.next_event() => {
                ...
                match event {
                    ...
                    other => {
                        if !forward_event_or_shutdown(&praxis, &tx_sub, &cancel_token, other).await
                        {
                            break;
                        }
                    }
...
async fn shutdown_delegate(praxis: &Praxis) {
    let _ = praxis.submit(Op::Interrupt).await;
    let _ = praxis.submit(Op::Shutdown {}).await;

    let _ = timeout(Duration::from_millis(500), async {
        while let Ok(event) = praxis.next_event().await {
            if matches!(
                event.msg,
...
async fn forward_ops(
    ...
    loop {
        ...
        let _ = praxis.submit_with_id(submission).await;
    }
}
        ...
        if let Some(files) = maybe_files {
            ...
            let patch = changes
                ...
                .map(|(path, change)| match change {
                    praxis_protocol::protocol::FileChange::Add { content } => {
                        format!("*** Add File: {}\n{}", path.display(), content)
                    }
                    praxis_protocol::protocol::FileChange::Delete { content } => {
                        format!("*** Delete File: {}\n{}", path.display(), content)
                    }
...
async fn handle_request_user_input(
    ...
    if routes_approval_to_guardian(parent_ctx)
        ...
        .await
    {
        let _ = praxis.submit(Op::UserInputAnswer { id, response }).await;
        return;
    }
...
where
    F: core::future::Future<Output = praxis_protocol::protocol::ReviewDecision>,
{
    tokio::select! {
        biased;
        _ = cancel_token.cancelled() => {
            ...
            parent_session
                .notify_approval(approval_id, praxis_protocol::protocol::ReviewDecision::Abort)
                .await;
