...
async fn apply_role_preserves_unspecified_keys() {
    ...
    .await;
    config.praxis_linux_sandbox_exe = Some(PathBuf::from("/tmp/praxis-linux-sandbox"));
    config.main_execve_wrapper_exe = Some(PathBuf::from("/tmp/praxis-execve-wrapper"));
    let role_path = write_role_config(
        &home,
        "effort-only.toml",
    ...
    assert_eq!(
        config.praxis_linux_sandbox_exe,
        Some(PathBuf::from("/tmp/praxis-linux-sandbox"))
    );
    assert_eq!(
        config.main_execve_wrapper_exe,
        Some(PathBuf::from("/tmp/praxis-execve-wrapper"))
    );
}
