...
async fn refresh_accessible_connectors_cache_from_mcp_tools_writes_latest_installed_apps() {
    ...
    let tools = HashMap::from([
        ...
        (
            "mcp__praxis_apps__openai_hidden".to_string(),
            praxis_app_tool(
                "openai_hidden",
...
async fn cloud_requirements_disable_connector_applies_without_user_apps_table() {
    let praxis_home = tempdir().expect("tempdir should succeed");
    std::fs::write(praxis_home.path().join(CONFIG_TOML_FILE), "").expect("write config");
...
async fn local_requirements_disable_connector_overrides_user_apps_config() {
    ...
    let config_toml_path =
        AbsolutePathBuf::try_from(praxis_home.path().join(CONFIG_TOML_FILE)).expect("abs path");
    let mut config = ConfigBuilder::default()
        .praxis_home(praxis_home.path().to_path_buf())
        .fallback_cwd(Some(praxis_home.path().to_path_buf()))
        .build()
