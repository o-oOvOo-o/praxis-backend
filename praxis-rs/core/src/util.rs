...
/// `praxis_feedback::PraxisFeedback::metadata_layer()` is installed, these fields are captured and
...
///
/// ```rust
/// praxis_core::feedback_tags!(model = "gpt-5", cached = true);
/// praxis_core::feedback_tags!(provider = provider_id, request_id = request_id);
/// ```
#[macro_export]
...
pub(crate) fn emit_feedback_request_tags_with_auth_env(
    ...
    feedback_tags!(
        ...
        auth_env_praxis_api_key_present = auth_env.praxis_api_key_env_present,
        auth_env_praxis_api_key_enabled = auth_env.praxis_api_key_env_enabled,
...
pub const PRIMARY_CLI_COMMAND: &str = "praxis";
...
pub fn resume_command(thread_name: Option<&str>, thread_id: Option<ThreadId>) -> Option<String> {
    ...
    resume_target.map(|target| {
        ...
        if needs_double_dash {
            format!("{PRIMARY_CLI_COMMAND} resume -- {escaped}")
        } else {
            format!("{PRIMARY_CLI_COMMAND} resume {escaped}")
        }
    })
