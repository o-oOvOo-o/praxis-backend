...
use praxis_otel::metrics::names::CURATED_PLUGINS_STARTUP_SYNC_FINAL_METRIC;
use praxis_otel::metrics::names::CURATED_PLUGINS_STARTUP_SYNC_METRIC;
use reqwest::Client;
...
pub(super) fn start_startup_remote_plugin_sync_once(
    ...
    let marker_path = startup_remote_plugin_sync_marker_path(praxis_home.as_path());
    ...
    tokio::spawn(async move {
        ...
        if !wait_for_startup_remote_plugin_sync_prerequisites(praxis_home.as_path()).await {
            warn!(
                praxis_home = %praxis_home.display(),
            ...
            .await
        {
            Ok(sync_result) => {
                ...
                if let Err(err) =
                    write_startup_remote_plugin_sync_marker(praxis_home.as_path()).await
                {
                    warn!(
                        error = %err,
...
fn startup_remote_plugin_sync_marker_path(praxis_home: &Path) -> PathBuf {
    praxis_home.join(STARTUP_REMOTE_PLUGIN_SYNC_MARKER_FILE)
}
...
async fn wait_for_startup_remote_plugin_sync_prerequisites(praxis_home: &Path) -> bool {
    ...
    loop {
        if startup_remote_plugin_sync_prerequisites_ready(praxis_home) {
            return true;
        }
...
async fn write_startup_remote_plugin_sync_marker(praxis_home: &Path) -> std::io::Result<()> {
    let marker_path = startup_remote_plugin_sync_marker_path(praxis_home);
