...
use crate::praxis::model_request::build_prompt;
use crate::praxis::model_request::built_tools;
use praxis_otel::SessionTelemetry;
use praxis_otel::metrics::names::STARTUP_PREWARM_AGE_AT_FIRST_TURN_METRIC;
use praxis_otel::metrics::names::STARTUP_PREWARM_DURATION_METRIC;
use praxis_protocol::models::BaseInstructions;

pub(crate) struct SessionStartupPrewarmHandle {
    task: JoinHandle<PraxisResult<ModelClientSession>>,
    started_at: Instant,
    timeout: Duration,
}
...
impl SessionStartupPrewarmHandle {
    ...
    fn resolution_from_join_result(
        result: std::result::Result<PraxisResult<ModelClientSession>, tokio::task::JoinError>,
        started_at: Instant,
    ) -> SessionStartupPrewarmResolution {
        match result {
...
async fn schedule_startup_prewarm_inner(
...
) -> PraxisResult<ModelClientSession> {
    ...
    let mut client_session = session.services.model_runtime.new_session_for(
        &startup_turn_context.config.model_provider_id,
        &startup_turn_context.provider,
    );
    client_session
        .prewarm_websocket(
            &startup_prompt,
