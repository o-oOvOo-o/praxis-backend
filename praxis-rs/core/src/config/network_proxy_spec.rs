...
use praxis_network_proxy::NetworkDecision;
...
use praxis_network_proxy::NetworkProxyConstraints;
use praxis_network_proxy::NetworkProxyHandle;
use praxis_network_proxy::NetworkProxyState;
use praxis_network_proxy::build_config_state;
use praxis_network_proxy::host_and_port_from_network_addr;
    ...
    fn apply_requirements(
        ...
        if requirements.unix_sockets.is_some() {
            let allow_unix_sockets = requirements
                .unix_sockets
                .as_ref()
                .map(praxis_config::NetworkUnixSocketPermissionsToml::allow_unix_sockets)
                .unwrap_or_default();
            config
                .network
    ...
    for host in hosts {
        if incoming.insert(host.clone()) {
            config.network.upsert_domain_permission(
                host,
                if allow {
                    praxis_network_proxy::NetworkDomainPermission::Allow
                } else {
                    praxis_network_proxy::NetworkDomainPermission::Deny
                },
                normalize_host,
            );
        }
