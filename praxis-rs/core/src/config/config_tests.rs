...
fn default_permissions_profile_populates_runtime_sandbox_policy() -> std::io::Result<()> {
    ...
    let memories_root = praxis_home.path().join("memories").abs();
    assert_eq!(
...
fn permissions_profiles_allow_unknown_special_paths() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` is not recognized by this version of Praxis and will be ignored."
        )),
        "{:?}",
...
fn permissions_profiles_allow_unknown_special_paths_with_nested_entries() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` with nested entry `docs` is not recognized by this version of Praxis and will be ignored."
        )),
        "{:?}",
        config.startup_warnings
    );
...
fn permissions_profiles_allow_missing_filesystem_with_warning() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Permissions profile `workspace` does not define any recognized filesystem entries for this version of Praxis."
        )),
        "{:?}",
        config.startup_warnings
    );
...
async fn set_model_updates_defaults() -> anyhow::Result<()> {
    ...
    ConfigEditsBuilder::new(praxis_home.path())
...
fn loads_compact_prompt_from_file() -> std::io::Result<()> {
    let praxis_home = TempDir::new()?;
    let workspace = praxis_home.path().join("workspace");
...
fn load_config_rejects_missing_agent_role_config_file() -> std::io::Result<()> {
    let praxis_home = TempDir::new()?;
    let missing_path = praxis_home.path().join("agents").join("researcher.toml");
    let cfg = ConfigToml {
...
async fn agent_role_relative_config_file_resolves_against_config_toml() -> std::io::Result<()> {
    let praxis_home = TempDir::new()?;
    let role_config_path = praxis_home.path().join("agents").join("researcher.toml");
...
"#
    ...
    let standalone_agents_dir = repo_root.path().join(".praxis").join("agents");
...
async fn loads_legacy_split_agent_roles_from_config_toml() -> std::io::Result<()> {
    let praxis_home = TempDir::new()?;
    let researcher_path = praxis_home.path().join("agents").join("researcher.toml");
    let reviewer_path = praxis_home.path().join("agents").join("reviewer.toml");
    tokio::fs::create_dir_all(
        researcher_path
            .parent()
            .expect("role config should have a parent directory"),
...
"#
    ...
    let home_agents_dir = praxis_home.path().join("agents");
...
fn model_catalog_json_loads_from_path() -> std::io::Result<()> {
    let praxis_home = TempDir::new()?;
    let catalog_path = praxis_home.path().join("catalog.json");
