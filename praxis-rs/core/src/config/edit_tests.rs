...
"#,
    ...
    let raw = std::fs::read_to_string(praxis_home.join(CONFIG_TOML_FILE)).expect("read config");
...
fn blocking_set_model_writes_through_symlink_chain() {
    ...
    let link_path = praxis_home.join("config-link.toml");
...
fn blocking_set_model_replaces_symlink_on_cycle() {
    let tmp = tempdir().expect("tmpdir");
    let praxis_home = tmp.path();
    let link_a = praxis_home.join("a.toml");
    let link_b = praxis_home.join("b.toml");
    let config_path = praxis_home.join(CONFIG_TOML_FILE);
...
"#;
    std::fs::write(praxis_home.join(CONFIG_TOML_FILE), original).expect("seed config");
    ...
    let updated = std::fs::read_to_string(praxis_home.join(CONFIG_TOML_FILE)).expect("read config");
...
#[test]
fn blocking_set_hide_gpt_5_1_praxis_max_migration_prompt_preserves_table() {
    let tmp = tempdir().expect("tmpdir");
    let praxis_home = tmp.path();
    std::fs::write(
        praxis_home.join(CONFIG_TOML_FILE),
...
fn blocking_clear_path_noop_when_missing() {
    ...
    assert!(
        !praxis_home.join(CONFIG_TOML_FILE).exists(),
...
"#;
    ...
    contents = std::fs::read_to_string(praxis_home.join(CONFIG_TOML_FILE)).expect("read config");
