...
impl ConfigService {
    ...
    pub fn new_with_defaults(praxis_home: PathBuf) -> Self {
        Self {
            praxis_home,
    ...
    async fn apply_edits(
        ...
        let allowed_path =
            AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, &self.praxis_home)
        ...
        let user_config_toml =
            deserialize_config_toml_with_base(user_config.clone(), &self.praxis_home).map_err(
                |err| {
                    ConfigServiceError::write(
                        ConfigWriteErrorCode::ConfigValidationError,
                        format!("Invalid configuration: {err}"),
...
fn override_message(layer: &ConfigLayerSource) -> String {
    match layer {
        ...
        ConfigLayerSource::Project { dot_praxis_folder } => format!(
            "Overridden by project config: {}/{CONFIG_TOML_FILE}",
            dot_praxis_folder.display(),
        ),
