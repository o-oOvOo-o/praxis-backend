...
const CHATGPT_ERROR_BODY_PREVIEW_BYTES: usize = 512;
...
pub async fn list_accessible_connectors_from_mcp_tools_with_options_and_status(
    ...
    let mcp_servers = with_praxis_apps_mcp(HashMap::new(), auth.as_ref(), &mcp_config);
    ...
    let refreshed_tools = if force_refetch {
        match mcp_connection_manager
            .hard_refresh_praxis_apps_tools_cache()
            .await
        {
            Ok(tools) => Some(tools),
            Err(err) => {
    ...
    if praxis_apps_ready || !accessible_connectors.is_empty() {
        write_cached_accessible_connectors(cache_key, &accessible_connectors);
    }
    let accessible_connectors =
...
fn tool_suggest_connector_ids(config: &Config) -> HashSet<String> {
    let mut connector_ids = PluginsManager::new(config.praxis_home.clone())
...
async fn list_directory_connectors_for_tool_suggest_with_auth(
    ...
    praxis_connectors::list_all_connectors_with_options(
        cache_key,
        is_workspace_account,
        /*force_refetch*/ false,
        |path| {
...
fn plugin_app_to_app_info(connector_id: AppConnectorId) -> AppInfo {
    ...
    // connector_name values from praxis_apps tool discovery.
