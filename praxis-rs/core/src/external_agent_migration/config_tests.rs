...
fn fixture_paths() -> (TempDir, PathBuf, PathBuf) {
    ...
    let praxis_home = root.path().join(".codex");
    (root, claude_home, praxis_home)
}

fn service_for_paths(claude_home: PathBuf, praxis_home: PathBuf) -> ExternalAgentMigrationService {
    ExternalAgentMigrationService::new_for_test(praxis_home, claude_home)
}

#[test]
fn detect_home_lists_config_skills_and_agents_md() {
    let (_root, claude_home, praxis_home) = fixture_paths();
    let agents_skills = praxis_home
        .parent()
    ...
    let items = service_for_paths(claude_home.clone(), praxis_home.clone())
        .detect(ExternalAgentMigrationDetectOptions {
            include_home: true,
            cwds: None,
        })
        .expect("detect");
    ...
    let expected = vec![
        ExternalAgentMigrationItem {
            item_type: ExternalAgentMigrationItemType::Config,
            description: format!(
                "Migrate {} into {}",
                claude_home.join("settings.json").display(),
                praxis_home.join("config.toml").display()
            ),
            cwd: None,
        },
        ExternalAgentMigrationItem {
            item_type: ExternalAgentMigrationItemType::Skills,
            description: format!(
        ...
        ExternalAgentMigrationItem {
            item_type: ExternalAgentMigrationItemType::AgentsMd,
            description: format!(
                "Import {} to {}",
                claude_home.join("CLAUDE.md").display(),
                praxis_home.join("AGENTS.md").display()
            ),
            cwd: None,
        },
    ];
...
fn import_home_migrates_supported_config_fields_skills_and_agents_md() {
    ...
    service_for_paths(claude_home, praxis_home.clone())
        .import(vec![
            ExternalAgentMigrationItem {
                item_type: ExternalAgentMigrationItemType::AgentsMd,
    ...
    assert_eq!(
        fs::read_to_string(praxis_home.join("AGENTS.md")).expect("read agents"),
        "Praxis guidance"
    );

    assert_eq!(
        fs::read_to_string(praxis_home.join("config.toml")).expect("read config"),
        "sandbox_mode = \"workspace-write\"\n\n[shell_environment_policy]\ninherit = \"core\"\n\n[shell_environment_policy.set]\nCI = \"false\"\nFOO = \"bar\"\nMAX_RETRIES = \"3\"\nMY_TEAM = \"codex\"\n"
    );
    assert_eq!(
        fs::read_to_string(agents_skills.join("skill-a").join("SKILL.md"))
            .expect("read copied skill"),
        "Use Praxis and Praxis utilities."
    );
}
...
fn import_home_skips_empty_config_migration() {
    ...
    service_for_paths(claude_home, praxis_home.clone())
        .import(vec![ExternalAgentMigrationItem {
            item_type: ExternalAgentMigrationItemType::Config,
            description: String::new(),
            cwd: None,
        }])
    ...
    assert!(!praxis_home.join("config.toml").exists());
}
...
fn detect_home_skips_config_when_target_already_has_supported_fields() {
    ...
    let items = service_for_paths(claude_home, praxis_home)
        .detect(ExternalAgentMigrationDetectOptions {
            include_home: true,
            cwds: None,
        })
    ...
    assert_eq!(items, Vec::<ExternalAgentMigrationItem>::new());
}
...
fn detect_repo_prefers_non_empty_dot_claude_agents_source() {
    ...
    assert_eq!(
        items,
        vec![ExternalAgentMigrationItem {
            item_type: ExternalAgentMigrationItemType::AgentsMd,
            description: format!(
                "Import {} to {}",
...
fn import_skills_returns_only_new_skill_directory_count() {
    ...
    let copied_count = service_for_paths(claude_home, praxis_home)
