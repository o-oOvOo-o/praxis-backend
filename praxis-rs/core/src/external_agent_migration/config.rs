...
impl ExternalAgentMigrationService {
    ...
    pub fn import(&self, migration_items: Vec<ExternalAgentMigrationItem>) -> io::Result<()> {
        for migration_item in migration_items {
            match migration_item.item_type {
                ...
                ExternalAgentMigrationItemType::McpServerConfig => {}
            }
        }
    ...
    fn detect_migrations(
        ...
        items: &mut Vec<ExternalAgentMigrationItem>,
    ) -> io::Result<()> {
        ...
        let target_config = repo_root.map_or_else(
            || self.praxis_home.join("config.toml"),
        ...
        if source_settings.is_file() {
            ...
            if !is_empty_toml_table(&migrated) {
                ...
                if should_include {
                    items.push(ExternalAgentMigrationItem {
        ...
        let target_agents_md = repo_root.map_or_else(
            || self.praxis_home.join("AGENTS.md"),
