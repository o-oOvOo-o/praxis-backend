...
use praxis_otel::metrics::names::TURN_TTFM_DURATION_METRIC;
use praxis_otel::metrics::names::TURN_TTFT_DURATION_METRIC;
    ...
    match item {
        ...
        ResponseItem::Reasoning {
            ...
            summary.iter().any(|entry| match entry {
                praxis_protocol::models::ReasoningItemReasoningSummary::SummaryText { text } => {
                    !text.is_empty()
                }
            }) || content.as_ref().is_some_and(|entries| {
                entries.iter().any(|entry| match entry {
                    praxis_protocol::models::ReasoningItemContent::ReasoningText { text }
