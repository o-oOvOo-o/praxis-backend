...
use praxis_secrets::redact_secrets;
...
pub(in crate::memories) struct RequestContext {
    pub(in crate::memories) provider_id: String,
    pub(in crate::memories) provider: ModelProviderInfo,
...
impl RequestContext {
    pub(in crate::memories) fn from_turn_context(
        ...
        Self {
            ...
            provider: turn_context.provider.clone(),
            model_info,
...
async fn claim_startup_jobs(
...
) -> Option<Vec<praxis_state::Stage1JobClaim>> {
    ...
    match state_db
        .claim_stage1_jobs_for_startup(
            ...
            praxis_state::Stage1StartupClaimParams {
...
async fn run_jobs(
    session: &Arc<Session>,
    claimed_candidates: Vec<praxis_state::Stage1JobClaim>,
...
mod job {
    ...
    pub(in crate::memories) async fn run(
        session: &Session,
        claim: praxis_state::Stage1JobClaim,
        stage_one_context: &RequestContext,
    ) -> JobResult {
    ...
    async fn sample(
        ...
        let mut client_session = session
            .services
            .model_runtime
            .new_session_for(&stage_one_context.provider_id, &stage_one_context.provider);
