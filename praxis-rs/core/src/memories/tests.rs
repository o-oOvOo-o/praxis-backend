...
use chrono::TimeZone;
use chrono::Utc;
use praxis_config::types::DEFAULT_MEMORIES_MAX_RAW_MEMORIES_FOR_CONSOLIDATION;
...
keywords: praxis-state, migrations
---
### Task 1: migration-test
task: add-migration-test
task_group: praxis-state
...
- Confirmed no ordering regressions."
    ...
    assert!(raw_memories.contains("task_group: praxis-state"));
...
mod phase2 {
    ...
    use praxis_state::Phase2JobClaimOutcome;
    use praxis_state::Stage1Output;
    ...
    struct DispatchHarness {
        ...
        state_db: Arc<praxis_state::StateRuntime>,
    }
    ...
    impl DispatchHarness {
        async fn new() -> Self {
            ...
            let state_db = praxis_state::StateRuntime::init(
        ...
        async fn seed_stage1_output(&self, source_updated_at: i64) {
            ...
            let ownership_token = match claim {
                praxis_state::Stage1JobClaimOutcome::Claimed { ownership_token } => ownership_token,
    ...
    async fn dispatch_reclaims_stale_global_lock_and_starts_consolidation() {
        ...
        pretty_assertions::assert_eq!(
            config_snapshot.cwd,
            memory_root(&harness.config.praxis_home)
        );
        match config_snapshot.sandbox_policy {
            SandboxPolicy::WorkspaceWrite { writable_roots, .. } => {
                assert!(
                    writable_roots
                        .iter()
                        .any(|root| root.as_path() == harness.config.praxis_home.as_path()),
                    "consolidation subagent should have praxis_home as writable root"
                );
            }
            other => panic!("unexpected sandbox policy: {other:?}"),
        }
        subagent.praxis.session.ensure_rollout_materialized().await;
        subagent.praxis.session.flush_rollout().await;
        let rollout_path = subagent
            .rollout_path()
            .expect("consolidation thread should have a rollout path");
        praxis_rollout::state_db::read_repair_rollout_path(
    ...
    async fn dispatch_with_empty_stage1_outputs_rebuilds_local_artifacts() {
        let harness = DispatchHarness::new().await;
        let root = memory_root(&harness.config.praxis_home);
