...
impl ShellSnapshot {
    ...
    async fn try_new(
        ...
        let nonce = SystemTime::now()
            ...
            .unwrap_or(0);
        let path = praxis_home
            .join(SNAPSHOT_DIR)
            .join(format!("{session_id}.{nonce}.{extension}"));
        let temp_path = praxis_home
            .join(SNAPSHOT_DIR)
        ...
        tokio::spawn(async move {
            if let Err(err) = cleanup_stale_snapshots(&praxis_home, cleanup_session_id).await {
...
async fn run_script_with_timeout(
    ...
    unsafe {
        handler.pre_exec(|| {
            praxis_utils_pty::process_group::detach_from_tty()?;
            Ok(())
        });
    }
