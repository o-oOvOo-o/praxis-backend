...
#[test]
fn test_deserialize_provider_compat() {
    let provider_toml = r#"
name = "GLM Common"
...
"#;
    ...
    assert_eq!(
        ...
        Some(ModelProviderMaxTokensField::MaxTokens)
    );
    assert_eq!(compat.requires_tool_result_name, Some(true));
    assert_eq!(compat.requires_assistant_after_tool_result, Some(true));
    assert_eq!(
        compat.thinking_format,
        Some(ModelProviderThinkingFormat::Zai)
    );
    assert_eq!(
        compat.reasoning_effort_map.and_then(|map| map.high),
        Some("deep".to_string())
    );
}
