...
pub use praxis_login::auth::RefreshTokenFailedError;
pub use praxis_login::auth::RefreshTokenFailedReason;
...
pub type Result<T> = std::result::Result<T, PraxisErr>;
...
pub enum PraxisErr {
    ...
    /// could not be captured). Analogous to the previous `PraxisError::Spawn` variant.
    ...
    #[error(
        "To use Praxis with your ChatGPT plan, upgrade to Plus: https://chatgpt.com/explore/plus."
    )]
    UsageNotIncluded,
    ...
    #[error("praxis-linux-sandbox was required but not provided")]
...
impl From<CancelErr> for PraxisErr {
...
impl PraxisErr {
    pub fn is_retryable(&self) -> bool {
        match self {
            PraxisErr::TurnAborted
            | PraxisErr::Interrupted
            | PraxisErr::EnvVar(_)
            | PraxisErr::Fatal(_)
            | PraxisErr::UsageNotIncluded
            | PraxisErr::QuotaExceeded
            | PraxisErr::InvalidImageRequest()
            | PraxisErr::InvalidRequest(_)
            | PraxisErr::RefreshTokenFailed(_)
            ...
            | PraxisErr::ConnectionFailed(_)
            | PraxisErr::InternalServerError
            | PraxisErr::InternalAgentDied
            | PraxisErr::Io(_)
            | PraxisErr::Json(_)
            | PraxisErr::TokioJoin(_) => true,
            #[cfg(target_os = "linux")]
            PraxisErr::LandlockRuleset(_) | PraxisErr::LandlockPathFd(_) => false,
        }
    }
}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        let message = match self.plan_type.as_ref() {
            ...
            Some(PlanType::Known(KnownPlan::Free)) | Some(PlanType::Known(KnownPlan::Go)) => {
                format!(
                    "You've hit your usage limit. Upgrade to Plus to continue using Praxis (https://chatgpt.com/explore/plus),{}",
                    retry_suffix_after_or(self.resets_at.as_ref())
                )
            }
...
impl PraxisErr {
    /// Minimal shim so that existing `e.downcast_ref::<PraxisErr>()` checks continue to compile
    ...
    pub fn to_error_event(&self, message_prefix: Option<String>) -> ErrorEvent {
        ...
        ErrorEvent {
            message,
            praxis_error_info: Some(self.to_praxis_protocol_error()),
        }
    }
...
pub fn get_error_message_ui(e: &PraxisErr) -> String {
    let message = match e {
        PraxisErr::Sandbox(SandboxErr::Denied { output, .. }) => {
        ...
        PraxisErr::Sandbox(SandboxErr::Timeout { output }) => {
            format!(
