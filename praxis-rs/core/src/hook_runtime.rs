...
use praxis_hooks::PostToolUseOutcome;
use praxis_hooks::PostToolUseRequest;
use praxis_hooks::PreToolUseOutcome;
use praxis_hooks::PreToolUseRequest;
use praxis_hooks::SessionStartOutcome;
use praxis_hooks::UserPromptSubmitOutcome;
...
pub(crate) async fn run_pending_session_start_hooks(
    ...
    let request = praxis_hooks::SessionStartRequest {
        ...
        assert_eq!(
            messages
                ...
                .map(|message| match message {
                    praxis_protocol::models::ResponseItem::Message { role, content, .. } => {
                        let text = content
                            .iter()
                            .map(|item| match item {
