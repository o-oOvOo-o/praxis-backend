...
use praxis_config::types::OtelExporterKind as Kind;
use praxis_config::types::OtelHttpProtocol as Protocol;
use praxis_features::Feature;
use praxis_login::default_client::originator;
use praxis_otel::OtelProvider;
use praxis_otel::config::OtelExporter;
use praxis_otel::config::OtelHttpProtocol;
use praxis_otel::config::OtelSettings;
use praxis_otel::config::OtelTlsConfig as OtelTlsSettings;
use std::error::Error;
...
/// Filter predicate for exporting only Praxis-owned events via OTEL.
/// Keeps events that originated from praxis_otel module
pub fn praxis_export_filter(meta: &tracing::Metadata<'_>) -> bool {
    meta.target().starts_with("praxis_otel")
}
