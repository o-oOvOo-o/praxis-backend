...
async fn new_uses_current_model_provider_for_model_refresh() {
    ...
    current_provider.base_url = Some(server.uri());
    current_provider.requires_openai_auth = false;
    config.model_provider_id = "my_provider".to_string();
    config.model_provider = current_provider.clone();
    config
        .model_providers
