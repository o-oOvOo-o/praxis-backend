...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    assert_eq!(project_layers[0].as_path(), nested.join(".praxis").as_path());
    assert_eq!(
...
"#;
    tokio::fs::write(project_root.join(".praxis").join(CONFIG_TOML_FILE), root_cfg).await?;
...
async fn praxis_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    ...
    tokio::fs::write(nested_dot_praxis.join(CONFIG_TOML_FILE), "foo = \"child\"\n").await?;
