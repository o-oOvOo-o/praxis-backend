...
pub use praxis_config::ConfigLayerStackOrdering;
pub use praxis_config::ConfigLoadError;
pub use praxis_config::ConfigRequirements;
pub use praxis_config::ConfigRequirementsToml;
pub use praxis_config::ConstrainedWithSource;
pub use praxis_config::FeatureRequirementsToml;
pub use praxis_config::LoaderOverrides;
pub use praxis_config::McpServerIdentity;
pub use praxis_config::McpServerRequirement;
pub use praxis_config::NetworkConstraints;
pub use praxis_config::NetworkDomainPermissionToml;
pub use praxis_config::NetworkDomainPermissionsToml;
pub use praxis_config::NetworkRequirementsToml;
pub use praxis_config::NetworkUnixSocketPermissionToml;
pub use praxis_config::NetworkUnixSocketPermissionsToml;
pub use praxis_config::RequirementSource;
pub use praxis_config::ResidencyRequirement;
pub use praxis_config::SandboxModeRequirement;
pub use praxis_config::Sourced;
pub use praxis_config::TextPosition;
pub use praxis_config::TextRange;
pub use praxis_config::WebSearchModeRequirement;
pub(crate) use praxis_config::build_cli_overrides_layer;
pub(crate) use praxis_config::config_error_from_toml;
pub use praxis_config::default_project_root_markers;
pub use praxis_config::format_config_error;
pub use praxis_config::format_config_error_with_source;
pub(crate) use praxis_config::io_error_from_config_error;
pub use praxis_config::merge_toml_values;
pub use praxis_config::project_root_markers_from_config;
#[cfg(test)]
...
pub(crate) async fn first_layer_config_error(layers: &ConfigLayerStack) -> Option<ConfigError> {
    praxis_config::first_layer_config_error::<ConfigToml>(layers, CONFIG_TOML_FILE).await
}

pub(crate) async fn first_layer_config_error_from_entries(
    layers: &[ConfigLayerEntry],
) -> Option<ConfigError> {
    praxis_config::first_layer_config_error_from_entries::<ConfigToml>(layers, CONFIG_TOML_FILE)
        .await
}
...
async fn resolve_user_config_toml_file(praxis_home: &Path) -> io::Result<AbsolutePathBuf> {
    let shared_praxis_home = praxis_shared_praxis_home(praxis_home)?;
    resolve_user_config_toml_file_with_shared_home(praxis_home, shared_praxis_home.as_deref()).await
}

async fn resolve_user_config_toml_file_with_shared_home(
    praxis_home: &Path,
    shared_praxis_home: Option<&Path>,
) -> io::Result<AbsolutePathBuf> {
    let user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, praxis_home)?;
    match tokio::fs::metadata(user_file.as_path()).await {
        Ok(_) => return Ok(user_file),
        Err(err) if err.kind() != io::ErrorKind::NotFound => return Ok(user_file),
        Err(_) => {}
    }

    let Some(shared_praxis_home) = shared_praxis_home else {
        return Ok(user_file);
    };
    let shared_user_file =
        AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, shared_praxis_home)?;
    match tokio::fs::metadata(shared_user_file.as_path()).await {
        Ok(_) => Ok(shared_user_file),
        Err(_) => Ok(user_file),
    }
}
...
#[cfg(windows)]
fn windows_praxis_system_dir() -> PathBuf {
    ...
    program_data.join("OpenAI").join("Praxis")
}

#[cfg(windows)]
fn windows_system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    let requirements_toml_file = windows_praxis_system_dir().join("requirements.toml");
    AbsolutePathBuf::try_from(requirements_toml_file)
}

#[cfg(windows)]
fn windows_system_config_toml_file() -> io::Result<AbsolutePathBuf> {
    let config_toml_file = windows_praxis_system_dir().join("config.toml");
...
fn project_layer_entry(
    ...
    let source = ConfigLayerSource::Project {
        dot_praxis_folder: dot_praxis_folder.clone(),
    };
...
async fn load_project_layers(
    ...
    let praxis_home_abs = AbsolutePathBuf::from_absolute_path(praxis_home)?;
    let praxis_home_normalized =
        normalize_path(praxis_home_abs.as_path()).unwrap_or_else(|_| praxis_home_abs.to_path_buf());
    let mut dirs = cwd
        .as_path()
        .ancestors()
        .scan(false, |done, a| {
    ...
    for dir in dirs {
        ...
        let config_file = dot_praxis_abs.join(CONFIG_TOML_FILE)?;
        match tokio::fs::read_to_string(&config_file).await {
            Ok(contents) => {
                ...
                let config =
                    resolve_relative_paths_in_config_toml(config, dot_praxis_abs.as_path())?;
    ...
    fn windows_system_requirements_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_requirements_toml_file()
                .expect("requirements.toml path")
                .as_path()
                .ends_with(Path::new("OpenAI").join("Praxis").join("requirements.toml"))
        );
    }
    ...
    fn windows_system_config_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_config_toml_file()
                .expect("config.toml path")
                .as_path()
                .ends_with(Path::new("OpenAI").join("Praxis").join("config.toml"))
        );
    }
}
