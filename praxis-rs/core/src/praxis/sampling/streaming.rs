...
fn agent_message_text(item: &praxis_protocol::items::AgentMessageItem) -> String {
    item.content
        .iter()
...
pub(in crate::praxis) fn realtime_text_for_event(msg: &EventMsg) -> Option<String> {
    match msg {
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: praxis_protocol::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(praxis_protocol::items::AgentMessageItem {
