...
impl Praxis {
    ...
    async fn spawn_internal(args: PraxisSpawnArgs) -> PraxisResult<PraxisSpawnOk> {
        ...
        let dynamic_tools = resolve_dynamic_tools_for_session(
            config.as_ref(),
            &conversation_history,
            dynamic_tools,
        )
...
async fn resolve_dynamic_tools_for_session(
    config: &Config,
    ...
    persisted_dynamic_tools_for_history(config, conversation_history)
        .await
        .or_else(|| conversation_history.get_dynamic_tools())
        .unwrap_or_default()
}

async fn persisted_dynamic_tools_for_history(
    config: &Config,
    conversation_history: &InitialHistory,
