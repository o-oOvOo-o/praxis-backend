...
//!
//! The history is stored at `~/.praxis/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.praxis`.
...
fn history_filepath(config: &Config) -> PathBuf {
    let mut path = config.praxis_home.clone();
...
pub async fn append_entry(text: &str, conversation_id: &ThreadId, config: &Config) -> Result<()> {
    ...
    // Resolve `~/.praxis/history.jsonl` and ensure the parent directory exists.
