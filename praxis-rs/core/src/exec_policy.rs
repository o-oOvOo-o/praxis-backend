...
use praxis_execpolicy::AmendError;
use praxis_execpolicy::Decision;
use praxis_execpolicy::Error as ExecPolicyRuleError;
...
use praxis_execpolicy::PolicyParser;
use praxis_execpolicy::RuleMatch;
use praxis_execpolicy::blocking_append_allow_prefix_rule;
use praxis_execpolicy::blocking_append_network_rule;
...
use praxis_shell_command::is_dangerous_command::command_might_be_dangerous;
...
pub(crate) fn child_uses_parent_exec_policy(parent_config: &Config, child_config: &Config) -> bool {
    fn exec_policy_config_folders(config: &Config) -> Vec<AbsolutePathBuf> {
        config
            ...
            .into_iter()
            .filter_map(praxis_config::ConfigLayerEntry::config_folder)
            .collect()
    }
...
pub enum ExecPolicyError {
    ...
    ParsePolicy {
        path: String,
        source: praxis_execpolicy::Error,
    },
}
...
impl ExecPolicyManager {
    ...
    pub(crate) async fn append_amendment_and_update(
        ...
        let policy_path = default_policy_path(praxis_home);
        spawn_blocking({
...
fn exec_policy_message_for_display(source: &praxis_execpolicy::Error) -> String {
...
fn default_policy_path(praxis_home: &Path) -> PathBuf {
    praxis_home.join(RULES_DIR_NAME).join(DEFAULT_POLICY_FILE)
}
...
/// - Note: we only use this amendment when the command fails to run in sandbox and Praxis prompts the user to run outside the sandbox
