...
use praxis_api::RealtimeAudioFrame;
use praxis_api::RealtimeEvent;
use praxis_api::RealtimeEventParser;
use praxis_api::RealtimeSessionConfig;
use praxis_api::RealtimeSessionMode;
use praxis_api::RealtimeWebsocketClient;
...
use praxis_protocol::protocol::RealtimeHandoffRequested;
...
impl RealtimeConversationManager {
    ...
    ) -> PraxisResult<(Receiver<RealtimeEvent>, Arc<AtomicBool>)> {
        let previous_state = {
    ...
    pub(crate) async fn audio_in(&self, frame: RealtimeAudioFrame) -> PraxisResult<()> {
        let sender = {
        ...
        match sender.try_send(frame) {
            ...
            Err(TrySendError::Closed(_)) => Err(PraxisErr::InvalidRequest(
                "conversation is not running".to_string(),
            )),
        }
    }

    pub(crate) async fn text_in(&self, text: String) -> PraxisResult<()> {
        let sender = {
            let guard = self.state.lock().await;
        ...
        sender
            .send(text)
            .await
            .map_err(|_| PraxisErr::InvalidRequest("conversation is not running".to_string()))?;
        Ok(())
    }

    pub(crate) async fn handoff_out(&self, output_text: String) -> PraxisResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
        ...
        *handoff.last_output_text.lock().await = Some(output_text.clone());
        if matches!(handoff.session_kind, RealtimeSessionKind::V1) {
            handoff
                ...
                .await
                .map_err(|_| {
                    PraxisErr::InvalidRequest("conversation is not running".to_string())
                })?;
        }
        Ok(())
    }

    pub(crate) async fn handoff_complete(&self) -> PraxisResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
    ...
    pub(crate) async fn shutdown(&self) -> PraxisResult<()> {
        let state = {
            let mut guard = self.state.lock().await;
            guard.take()
        };
...
async fn prepare_realtime_start(
    sess: &Arc<Session>,
    params: ConversationStartParams,
) -> PraxisResult<PreparedRealtimeConversationStart> {
    let config = sess.get_config().await;
    let provider = config.model_provider.clone();
    let model_client = sess
        .services
        .model_runtime
        .client_for(&config.model_provider_id, &provider);
    ...
    let extra_headers = realtime_request_headers(requested_session_id.as_deref(), &setup.api_auth)?;
...
fn realtime_request_headers(
    ...
    api_auth: &crate::api_bridge::CoreAuthProvider,
) -> PraxisResult<Option<HeaderMap>> {
    ...
    let api_key = api_auth.bearer_token_value().ok_or_else(|| {
        PraxisErr::InvalidRequest("realtime conversation requires API key auth".to_string())
    })?;
    let auth_value = HeaderValue::from_str(&format!("Bearer {api_key}")).map_err(|err| {
        PraxisErr::InvalidRequest(format!("invalid realtime api key header: {err}"))
    })?;
