...
use praxis_execpolicy::MatchOptions;
...
use praxis_protocol::protocol::GuardianCommandSource;
...
use praxis_shell_command::bash::parse_shell_lc_plain_commands;
use praxis_shell_command::bash::parse_shell_lc_single_command_prefix;
use praxis_shell_escalation::EscalateServer;
use praxis_shell_escalation::EscalationDecision;
use praxis_shell_escalation::EscalationExecution;
use praxis_shell_escalation::EscalationPermissions;
use praxis_shell_escalation::EscalationPolicy;
use praxis_shell_escalation::EscalationSession;
use praxis_shell_escalation::ExecParams;
use praxis_shell_escalation::ExecResult;
use praxis_shell_escalation::Permissions as EscalatedPermissions;
use praxis_shell_escalation::PreparedExec;
use praxis_shell_escalation::ShellCommandExecutor;
use praxis_shell_escalation::Stopwatch;
...
fn evaluate_intercepted_exec_policy(
    ...
    let CandidateCommands {
        ...
        // In this praxisath, the first argument in `commands` could be a bare
        ...
        // In this praxisath, `commands` has a single entry where the program
...
struct CoreShellCommandExecutor {
    ...
    network: Option<praxis_network_proxy::NetworkProxy>,
...
impl CoreShellCommandExecutor {
    ...
    fn prepare_sandboxed_exec(
        ...
        let exec_request = sandbox_manager.transform(SandboxTransformRequest {
            ...
            praxis_linux_sandbox_exe: self.praxis_linux_sandbox_exe.as_ref(),
...
fn map_exec_result(
    ...
    if result.timed_out {
        return Err(ToolError::Praxis(PraxisErr::Sandbox(SandboxErr::Timeout {
