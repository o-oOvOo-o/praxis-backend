...
use praxis_tools::DiscoverableToolType;
use praxis_tools::TOOL_SUGGEST_TOOL_NAME;
use praxis_tools::ToolSuggestArgs;
use praxis_tools::ToolSuggestResult;
use praxis_tools::all_suggested_connectors_picked_up;
use praxis_tools::build_tool_suggestion_elicitation_request;
use praxis_tools::filter_tool_suggest_discoverable_tools_for_client;
use praxis_tools::verified_connector_suggestion_completed;
...
async fn refresh_missing_suggested_connectors(
    ...
    match manager.hard_refresh_praxis_apps_tools_cache().await {
        Ok(mcp_tools) => {
            let accessible_connectors = connectors::with_app_enabled_state(
                connectors::accessible_connectors_from_mcp_tools(&mcp_tools),
                &turn.config,
