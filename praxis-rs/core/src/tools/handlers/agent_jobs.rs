...
#[cfg(test)]
use csv_io::csv_escape;
use csv_io::default_output_csv_path;
use csv_io::ensure_unique_headers;
use csv_io::is_item_stale;
use csv_io::job_runtime_timeout;
#[cfg(test)]
use csv_io::parse_csv;
use csv_io::parse_csv_file;
use csv_io::render_job_csv;
use csv_io::started_at_from_item;
...
impl JobProgressEmitter {
    ...
    async fn maybe_emit(
        ...
        turn: &TurnContext,
        job_id: &str,
        progress: &praxis_state::AgentJobProgress,
        force: bool,
    ) -> anyhow::Result<()> {
...
mod spawn_agents_on_csv {
    ...
    pub async fn handle(
        ...
        for (idx, row) in rows.into_iter().enumerate() {
            ...
            items.push(praxis_state::AgentJobItemCreateParams {
                item_id,
        ...
        let _job = db
            .create_agent_job(
                &praxis_state::AgentJobCreateParams {
                    id: job_id.clone(),
                    name: job_name,
        ...
        let failed_item_errors = if progress.failed_items > 0 {
            let items = db
                .list_agent_job_items(
                    job_id.as_str(),
                    Some(praxis_state::AgentJobItemStatus::Failed),
                    Some(5),
                )
                .await
                .unwrap_or_default();
    ...
    loop {
        ...
        if !cancel_requested && active_items.len() < options.max_concurrency {
            ...
            let pending_items = db
                .list_agent_job_items(
                    job_id.as_str(),
                    Some(praxis_state::AgentJobItemStatus::Pending),
                    Some(slots),
                )
                .await?;
            for item in pending_items {
                    ...
                    .await
                {
                    ...
                    Err(PraxisErr::AgentLimitReached { .. }) => {
...
async fn recover_running_items(
    ...
    let running_items = db
        .list_agent_job_items(
            job_id,
            Some(praxis_state::AgentJobItemStatus::Running),
            /*limit*/ None,
        )
        .await?;
...
async fn finalize_finished_item(
    ...
    if matches!(item.status, praxis_state::AgentJobItemStatus::Running) {
...
fn build_worker_prompt(
    job: &praxis_state::AgentJob,
    item: &praxis_state::AgentJobItem,
