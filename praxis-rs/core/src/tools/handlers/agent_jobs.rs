...
fn job_runtime_timeout(job: &praxis_state::AgentJob) -> Duration {
    job.max_runtime_seconds
        .map(Duration::from_secs)
        .unwrap_or(DEFAULT_AGENT_JOB_ITEM_TIMEOUT)
}

fn started_at_from_item(item: &praxis_state::AgentJobItem) -> Instant {
...
fn is_item_stale(item: &praxis_state::AgentJobItem, runtime_timeout: Duration) -> bool {
    let now = chrono::Utc::now();
