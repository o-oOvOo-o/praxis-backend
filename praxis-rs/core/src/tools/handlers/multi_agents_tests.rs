...
async fn multi_agent_v2_list_agents_returns_completed_status_and_last_task_message() {
    ...
    let child_thread = manager
        ...
        .await
        .expect("child thread should exist");
    let child_turn = child_thread.praxis.session.new_default_turn().await;
    child_thread
        .codex
        .session
        .send_event(
...
async fn multi_agent_v2_assign_task_interrupts_busy_child_without_losing_message() {
    ...
    let active_turn = thread.praxis.session.new_default_turn().await;
    thread
        .codex
        .session
        .spawn_task(
...
async fn multi_agent_v2_assign_task_completion_notifies_parent_on_every_turn() {
    ...
    let first_turn = thread.praxis.session.new_default_turn().await;
    thread
        .codex
        .session
        .send_event(
    ...
    let second_turn = thread.praxis.session.new_default_turn().await;
    thread
        .codex
        .session
        .send_event(
...
async fn multi_agent_v2_interrupted_turn_does_not_notify_parent() {
    ...
    let aborted_turn = thread.praxis.session.new_default_turn().await;
    thread
        .codex
        .session
        .send_event(
...
async fn tool_handlers_cascade_close_and_resume_and_keep_explicitly_closed_subtrees_closed() {
    ...
    let parent_session = parent.thread.praxis.session.clone();
    ...
    let child_thread = manager
        ...
        .await
        .expect("child thread should exist");
    let child_session = child_thread.praxis.session.clone();
    ...
    let operator_session = operator.thread.praxis.session.clone();
    let _ = manager
        .agent_control()
    ...
    let parent_resume_output = ResumeAgentHandler
        .handle(invocation(
            operator_session,
            operator.thread.praxis.session.new_default_turn().await,
            "resume_agent",
...
async fn build_agent_spawn_config_uses_turn_context_values() {
    ...
    turn.praxis_linux_sandbox_exe = Some(PathBuf::from("/bin/echo"));
    ...
    expected.praxis_linux_sandbox_exe = turn.praxis_linux_sandbox_exe.clone();
