...
async fn lists_directory_entries() {
    ...
    let entries = list_directory_slice(
...
async fn respects_depth_parameter() {
    ...
    let entries_depth_one = list_directory_slice(
    ...
    let entries_depth_two = list_directory_slice(
    ...
    let entries_depth_three = list_directory_slice(
...
async fn paginates_in_sorted_order() {
    ...
    let first_page = list_directory_slice(
    ...
    let second_page = list_directory_slice(
...
async fn handles_large_limit_without_overflow() {
    ...
    let entries = list_directory_slice(dir_path, /*offset*/ 2, usize::MAX, /*depth*/ 1)
        .await
...
async fn respects_gitignore_by_default() {
    let temp = tempdir().expect("create tempdir");
    let dir_path = temp.path();
    tokio::fs::create_dir(dir_path.join(".git"))
        .await
    ...
    tokio::fs::create_dir(dir_path.join("ignored_dir"))
        .await
        .expect("create ignored dir");
    tokio::fs::write(dir_path.join("visible.txt"), b"visible")
        .await
        .expect("write visible");

    let entries = list_directory_slice_with_options(
        dir_path,
        /*offset*/ 1,
        /*limit*/ 20,
        /*depth*/ 2,
    ...
    .await
    .expect("list with ignore");
    assert_eq!(entries, vec!["visible.txt".to_string()]);

    let entries_without_ignore = list_directory_slice_with_options(
        dir_path,
        /*offset*/ 1,
        /*limit*/ 20,
        /*depth*/ 2,
    ...
    .await
    .expect("list without ignore");
    assert_eq!(
        entries_without_ignore,
        vec![
...
#[tokio::test]
async fn hidden_entries_require_explicit_opt_in() {
    let temp = tempdir().expect("create tempdir");
    let dir_path = temp.path();
    tokio::fs::write(dir_path.join(".secret"), b"hidden")
        .await
        .expect("write hidden");
    tokio::fs::write(dir_path.join("visible.txt"), b"visible")
        .await
    ...
    let default_entries = list_directory_slice_with_options(
        dir_path,
        /*offset*/ 1,
        /*limit*/ 20,
        /*depth*/ 1,
    ...
    .await
    .expect("list default hidden policy");
    assert_eq!(default_entries, vec!["visible.txt".to_string()]);

    let hidden_entries = list_directory_slice_with_options(
        dir_path,
        /*offset*/ 1,
        /*limit*/ 20,
        /*depth*/ 1,
...
#[tokio::test]
async fn filters_by_entry_kind() {
    let temp = tempdir().expect("create tempdir");
    let dir_path = temp.path();
    tokio::fs::create_dir(dir_path.join("nested"))
        .await
        .expect("create dir");
    tokio::fs::write(dir_path.join("file.txt"), b"file")
        .await
        .expect("write file");
    tokio::fs::write(dir_path.join("nested").join("child.txt"), b"child")
        .await
        .expect("write child");

    let directories = list_directory_slice_with_options(
        dir_path,
        /*offset*/ 1,
        /*limit*/ 20,
        /*depth*/ 2,
        DirectoryEntryFilter::Directories,
        /*respect_ignore*/ false,
        /*include_hidden*/ false,
    )
    .await
    .expect("list directories");
    assert_eq!(directories, vec!["nested/".to_string()]);

    let files = list_directory_slice_with_options(
        dir_path,
        /*offset*/ 1,
        /*limit*/ 20,
        /*depth*/ 2,
        DirectoryEntryFilter::Files,
        /*respect_ignore*/ false,
        /*include_hidden*/ false,
    )
    .await
    ...
    assert_eq!(
        files,
        vec!["file.txt".to_string(), "  child.txt".to_string()]
    );
}
