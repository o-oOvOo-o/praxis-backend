...
//! requirements cannot be loaded for those accounts, Praxis fails configuration loading rather than
...
impl CloudRequirementsService {
    fn new(
        ...
        Self {
            auth_manager,
            fetcher,
            cache_path: praxis_home.join(CLOUD_REQUIREMENTS_CACHE_FILENAME),
            timeout,
        }
    }
...
mod tests {
    ...
    fn write_auth_json(praxis_home: &Path, value: serde_json::Value) -> std::io::Result<()> {
        std::fs::write(
