...
async fn websocket_transport_routes_per_connection_handshake_and_responses() -> Result<()> {
    ...
    let (mut process, bind_addr) = spawn_websocket_server(praxis_home.path()).await?;
    ...
    process
        .kill()
        .await
        .context("failed to stop websocket app-gateway process")?;
    Ok(())
}
...
async fn websocket_transport_rejects_missing_and_invalid_capability_tokens() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let praxis_home = TempDir::new()?;
    let token_file = praxis_home.path().join("app-gateway-token");
    ...
    let (mut process, bind_addr) =
        spawn_websocket_server_with_args(praxis_home.path(), "ws://127.0.0.1:0", &auth_args)
            .await?;
...
async fn websocket_transport_verifies_signed_short_lived_bearer_tokens() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let praxis_home = TempDir::new()?;
    let shared_secret_file = praxis_home.path().join("app-gateway-signing-secret");
    ...
    let auth_args = vec![
        ...
        "--ws-issuer".to_string(),
        "praxis-enroller".to_string(),
        "--ws-audience".to_string(),
        "praxis-app-gateway-runtime".to_string(),
    ...
    let expired_token = signed_bearer_token(
        ...
        json!({
            ...
            "iss": "praxis-enroller",
            "aud": "praxis-app-gateway-runtime",
        }),
    )?;
...
async fn websocket_transport_allows_unauthenticated_non_loopback_startup_by_default() -> Result<()>
{
    ...
    let (mut process, bind_addr) =
        spawn_websocket_server_with_args(praxis_home.path(), "ws://0.0.0.0:0", &[]).await?;
...
pub(super) async fn spawn_websocket_server(praxis_home: &Path) -> Result<(Child, SocketAddr)> {
    spawn_websocket_server_with_args(praxis_home, "ws://127.0.0.1:0", &[]).await
}
...
pub(super) async fn spawn_websocket_server_with_args(
    ...
    let program = praxis_utils_cargo_bin::cargo_bin("praxis-app-gateway-runtime")
        .context("should find app-gateway binary")?;
    ...
    cmd.arg("--listen")
        ...
        .stdout(Stdio::null())
        .stderr(Stdio::piped())
        .env("CODEX_HOME", praxis_home)
        .env("RUST_LOG", "debug");
    let mut process = cmd
        .kill_on_drop(true)
        .spawn()
        .context("failed to spawn websocket app-gateway process")?;

    let stderr = process
        .stderr
        .take()
        .context("failed to capture websocket app-gateway stderr")?;
    ...
    let bind_addr = loop {
        ...
        .await
        .context("timed out waiting for websocket app-gateway to report bound websocket address")?
        .context("failed to read websocket app-gateway stderr")?
        .context("websocket app-gateway exited before reporting bound websocket address")?;
        eprintln!("[websocket app-gateway stderr] {line}");
...
async fn run_websocket_server_to_completion_with_args(
    ...
    timeout(Duration::from_secs(10), cmd.output())
        .await
        .context("timed out waiting for websocket app-gateway to exit")?
        .context("failed to run websocket app-gateway")
}
