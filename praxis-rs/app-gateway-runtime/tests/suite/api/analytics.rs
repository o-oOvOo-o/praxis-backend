...
use praxis_config::types::OtelHttpProtocol;
...
fn set_metrics_exporter(config: &mut praxis_core::config::Config) {
...
#[tokio::test]
async fn app_gateway_default_analytics_disabled_without_flag() -> Result<()> {
    ...
    let provider = praxis_core::otel_init::build_provider(
        &config,
        SERVICE_VERSION,
...
#[tokio::test]
async fn app_gateway_default_analytics_enabled_with_flag() -> Result<()> {
    let praxis_home = TempDir::new()?;
    let mut config = ConfigBuilder::default()
        .praxis_home(praxis_home.path().to_path_buf())
        .build()
...
pub(crate) fn thread_initialized_event(payload: &Value) -> Result<&Value> {
    ...
    events
        .iter()
        .find(|event| event["event_type"] == "praxis_thread_initialized")
        .ok_or_else(|| anyhow::anyhow!("praxis_thread_initialized event should be present"))
}
...
pub(crate) fn assert_basic_thread_initialized_event(
    ...
    assert_eq!(
        event["event_params"]["app_gateway_client"]["product_client_id"],
        DEFAULT_CLIENT_NAME
    );
    assert_eq!(
        event["event_params"]["app_gateway_client"]["client_name"],
        DEFAULT_CLIENT_NAME
    );
    assert_eq!(
        event["event_params"]["app_gateway_client"]["rpc_transport"],
        "stdio"
    );
