...
fn init_test_tracing() -> &'static TestTracing {
    ...
    TEST_TRACING.get_or_init(|| {
        ...
        let provider = SdkTracerProvider::builder()
            ...
            .build();
        let tracer = provider.tracer("praxis-app-gateway-runtime-message-processor-tests");
...
impl TracingHarness {
    async fn new() -> Result<Self> {
        ...
        let config = Arc::new(build_test_config(praxis_home.path(), &server.uri()).await?);
                ...
                ClientRequest::Initialize {
                    ...
                    params: InitializeParams {
                        client_info: ClientInfo {
                            name: "praxis-app-gateway-runtime-tests".to_string(),
                            title: None,
...
async fn build_test_config(praxis_home: &Path, server_uri: &str) -> Result<Config> {
    ...
    loop {
        ...
        match envelope {
            crate::outgoing_message::OutgoingEnvelope::ToConnection {
                ...
                let crate::outgoing_message::OutgoingMessage::AppGatewayNotification(notification) =
                    message
                else {
                    continue;
                };
                if matches!(
                    notification,
                    praxis_app_gateway_protocol::ServerNotification::ThreadStarted(_)
                ) {
                    return;
                }
            }
...
async fn thread_start_jsonrpc_span_exports_server_span_and_parents_children() -> Result<()> {
    ...
    let spans = wait_for_new_exported_spans(harness.tracing, baseline_len, |spans| {
        spans.iter().any(|span| {
            ...
            span.name.as_ref() == "app_gateway.thread_start.notify_started"
                && span.span_context.trace_id() == remote_trace_id
        })
    })
    .await;
