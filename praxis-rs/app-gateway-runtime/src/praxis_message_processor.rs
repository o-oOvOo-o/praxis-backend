...
pub(crate) struct RolloutSummary {
    ...
    git_info: Option<RolloutGitInfo>,
}

#[derive(Debug, Clone, PartialEq)]
struct RolloutGitInfo {
    sha: Option<String>,
...
impl PraxisMessageProcessor {
    ...
    async fn login_account(&mut self, request_id: ConnectionRequestId, params: LoginAccountParams) {
        match params {
    ...
    async fn login_api_key(&mut self, request_id: ConnectionRequestId, api_key: String) {
        match self.login_api_key_common(&api_key).await {
            Ok(()) => {
                let response = praxis_app_gateway_protocol::LoginAccountResponse::ApiKey {};
        ...
        match self.login_chatgpt_common().await {
            Ok(opts) => match run_login_server(opts) {
                Ok(server) => {
                    ...
                    let response = praxis_app_gateway_protocol::LoginAccountResponse::Chatgpt {
        ...
        match self.login_chatgpt_common().await {
            Ok(opts) => match request_device_code(&opts).await {
                Ok(device_code) => {
                    ...
                    let response =
                        praxis_app_gateway_protocol::LoginAccountResponse::ChatgptDeviceCode {
                            login_id: login_id.to_string(),
                            verification_url,
                            user_code,
                        };
    ...
    async fn logout_account(&mut self, request_id: ConnectionRequestId) {
            ...
            && let Ok(existing_thread) = self.thread_manager.get_thread(existing_thread_id).await
        {
            ...
            let rollout_path = if let Some(path) = existing_thread.rollout_path() {
                if path.exists() {
                    ...
                    match directory.find_rollout_path(existing_thread_id, None).await {
                        Ok(Some(path)) => path,
                        Ok(None) => {
                            self.send_invalid_request_error(
                                request_id,
        ...
        for cwd in cwds {
            ...
            let cwd_abs = match AbsolutePathBuf::try_from(cwd.as_path()) {
                ...
                Err(err) => {
                    ...
                    data.push(praxis_app_gateway_protocol::SkillsListEntry {
                        cwd,
                        skills: Vec::new(),
        ...
        let (data, marketplace_load_errors) = match tokio::task::spawn_blocking(move || {
            ...
            Ok::<
                (
                    ...
                    Vec<praxis_app_gateway_protocol::MarketplaceLoadErrorInfo>,
                ),
                MarketplaceError,
            >((
                outcome
                ...
                outcome
                    ...
                    .into_iter()
                    .map(
                        |err| praxis_app_gateway_protocol::MarketplaceLoadErrorInfo {
...
) -> Vec<praxis_app_gateway_protocol::SkillMetadata> {
    skills
        .iter()
        .map(|skill| {
            let enabled = !disabled_paths.contains(&skill.path_to_skills_md);
            praxis_app_gateway_protocol::SkillMetadata {
                name: skill.name.clone(),
                description: skill.description.clone(),
                short_description: skill.short_description.clone(),
                interface: skill.interface.clone().map(|interface| {
                    praxis_app_gateway_protocol::SkillInterface {
                        display_name: interface.display_name,
                ...
                dependencies: skill.dependencies.clone().map(|dependencies| {
                    praxis_app_gateway_protocol::SkillDependencies {
                        tools: dependencies
                            .tools
                            .into_iter()
                            .map(|tool| praxis_app_gateway_protocol::SkillToolDependency {
                                r#type: tool.r#type,
                                value: tool.value,
                                description: tool.description,
...
) -> Vec<praxis_app_gateway_protocol::SkillErrorInfo> {
    errors
        .iter()
        .map(|err| praxis_app_gateway_protocol::SkillErrorInfo {
            path: err.path.clone(),
...
fn thread_summary_to_rollout_summary(summary: praxis_rollout::ThreadSummary) -> RolloutSummary {
    RolloutSummary {
