//! Tracing helpers shared by socket and in-process app-gateway entry points.
//!
...
pub(crate) fn request_span(
    ...
    let span = app_gateway_request_span_template(
        method,
...
pub(crate) fn typed_request_span(
    ...
    let span =
        app_gateway_request_span_template(&method, "in-process", request.id(), connection_id);
    ...
    record_client_info(
        ...
        client_info
            ...
            .or(session.app_gateway_client_name.as_deref()),
        client_info
...
fn transport_name(transport: AppGatewayRuntimeTransport) -> &'static str {
    match transport {
        AppGatewayRuntimeTransport::Stdio => "stdio",
        AppGatewayRuntimeTransport::WebSocket { .. } => "websocket",
    }
}

fn app_gateway_request_span_template(
    method: &str,
    ...
    info_span!(
        ...
        app_gateway.connection_id = %connection_id,
        app_gateway.protocol = "app-gateway",
        app_gateway.client_name = field::Empty,
        app_gateway.client_version = field::Empty,
        turn.id = field::Empty,
    )
}
...
fn record_client_info(span: &Span, client_name: Option<&str>, client_version: Option<&str>) {
    if let Some(client_name) = client_name {
        span.record("app_gateway.client_name", client_name);
    }
    if let Some(client_version) = client_version {
        span.record("app_gateway.client_version", client_version);
    }
}
...
fn client_name<'a>(
    ...
    session.app_gateway_client_name.as_deref()
}
