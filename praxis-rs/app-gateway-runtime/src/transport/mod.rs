...
pub enum AppGatewayRuntimeTransport {
    Stdio,
    WebSocket { bind_address: SocketAddr },
}

#[derive(Debug, Clone, Eq, PartialEq)]
pub enum AppGatewayRuntimeTransportParseError {
    UnsupportedListenUrl(String),
    InvalidWebSocketListenUrl(String),
}

impl std::fmt::Display for AppGatewayRuntimeTransportParseError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            AppGatewayRuntimeTransportParseError::UnsupportedListenUrl(listen_url) => write!(
                f,
                "unsupported --listen URL `{listen_url}`; expected `stdio://` or `ws://IP:PORT`"
            ),
            AppGatewayRuntimeTransportParseError::InvalidWebSocketListenUrl(listen_url) => write!(
                f,
...
impl std::error::Error for AppGatewayRuntimeTransportParseError {}

impl AppGatewayRuntimeTransport {
    pub const DEFAULT_LISTEN_URL: &'static str = "stdio://";

    pub fn from_listen_url(listen_url: &str) -> Result<Self, AppGatewayRuntimeTransportParseError> {
        ...
        Err(AppGatewayRuntimeTransportParseError::UnsupportedListenUrl(
            listen_url.to_string(),
        ))
    }
}

impl FromStr for AppGatewayRuntimeTransport {
    type Err = AppGatewayRuntimeTransportParseError;
...
mod tests {
    ...
    fn app_gateway_transport_parses_websocket_listen_url() {
        let transport = AppGatewayRuntimeTransport::from_listen_url("ws://127.0.0.1:1234")
            .expect("websocket listen URL should parse");
        assert_eq!(
            transport,
            AppGatewayRuntimeTransport::WebSocket {
    ...
    fn app_gateway_transport_rejects_invalid_websocket_listen_url() {
        let err = AppGatewayRuntimeTransport::from_listen_url("ws://localhost:1234")
    ...
    fn app_gateway_transport_rejects_unsupported_listen_url() {
        let err = AppGatewayRuntimeTransport::from_listen_url("http://127.0.0.1:1234")
