...
pub struct AppGatewayWebsocketAuthArgs {
...
pub struct AppGatewayWebsocketAuthSettings {
    pub config: Option<AppGatewayWebsocketAuthConfig>,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub enum AppGatewayWebsocketAuthConfig {
    CapabilityToken {
...
impl AppGatewayWebsocketAuthArgs {
    pub fn try_into_settings(self) -> anyhow::Result<AppGatewayWebsocketAuthSettings> {
        ...
        let config = match self.ws_auth {
            Some(WebsocketAuthCliMode::CapabilityToken) => {
                ...
                Some(AppGatewayWebsocketAuthConfig::CapabilityToken {
            ...
            Some(WebsocketAuthCliMode::SignedBearerToken) => {
                ...
                Some(AppGatewayWebsocketAuthConfig::SignedBearerToken {
        ...
        Ok(AppGatewayWebsocketAuthSettings { config })
    }
}

pub(crate) fn policy_from_settings(
    settings: &AppGatewayWebsocketAuthSettings,
) -> io::Result<WebsocketAuthPolicy> {
    let mode = match settings.config.as_ref() {
        Some(AppGatewayWebsocketAuthConfig::CapabilityToken { token_file }) => {
...
mod tests {
    ...
    fn capability_token_args_require_token_file() {
        let err = AppGatewayWebsocketAuthArgs {
    ...
    fn signed_bearer_args_default_clock_skew_and_trim_optional_claims() {
        let settings = AppGatewayWebsocketAuthArgs {
        ...
        assert_eq!(
            settings,
            AppGatewayWebsocketAuthSettings {
                config: Some(AppGatewayWebsocketAuthConfig::SignedBearerToken {
