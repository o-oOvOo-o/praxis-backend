...
work_root="$(mktemp -d "${temp_root%/}/praxis-zsh-release.XXXXXX")"
trap 'rm -rf "$work_root"' EXIT

source_root="${work_root}/zsh"
package_root="${work_root}/praxis-zsh"
...
#!/usr/bin/env bash
set -euo pipefail
: "${PRAXIS_WRAPPER_LOG:?missing PRAXIS_WRAPPER_LOG}"
printf '%s\n' "$@" > "$PRAXIS_WRAPPER_LOG"
file="$1"
shift
if [[ "$#" -eq 0 ]]; then
...
PRAXIS_WRAPPER_LOG="$wrapper_log_path" \
...
(cd "$work_root" && tar -czf "${workspace}/${archive_path}" praxis-zsh)
