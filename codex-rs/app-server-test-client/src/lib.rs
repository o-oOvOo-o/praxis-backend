...
struct Cli {
    /// Path to the `codep` CLI binary. When set, requests use stdio by
    /// spawning `codep app-server` as a child process.
    ...
    /// Forwarded to the `codep` CLI as `--config key=value`. Repeatable.
    ///
    /// Example:
...
#[derive(Subcommand)]
enum CliCommand {
    /// Start `codep app-server` on a websocket endpoint in the background.
    ///
    /// Logs are written to:
    ///   `/tmp/codex-app-server-test-client/`
    Serve {
        /// WebSocket listen URL passed to `codep app-server --listen`.
...
pub async fn run() -> Result<()> {
    ...
    match command {
        CliCommand::Serve { listen, kill } => {
            ...
            let codex_bin = codex_bin.unwrap_or_else(|| PathBuf::from(PRIMARY_CLI_COMMAND));
...
impl CodexClient {
    ...
    fn spawn_stdio(codex_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        ...
        let stdin = codex_app_server
            .stdin
            .take()
            .context("codep app-server stdin unavailable")?;
        let stdout = codex_app_server
            .stdout
            .take()
            .context("codep app-server stdout unavailable")?;
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdin, .. } => {
                if let Some(stdin) = stdin.as_mut() {
                    ...
                    stdin
                        .flush()
                        .context("failed to flush payload to codep app-server")?;
                    return Ok(());
                }
                bail!("codep app-server stdin closed")
            }
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdout, .. } => {
                ...
                let bytes = stdout
                    ...
                    .context("failed to read from codep app-server")?;
                if bytes == 0 {
                    bail!("codep app-server closed stdout");
                }
                Ok(response_line)
            }
...
impl Drop for CodexClient {
    fn drop(&mut self) {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            println!("[codep app-server exited: {status}]");
            return;
        }
