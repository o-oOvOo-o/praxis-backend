...
fn create_auth_storage_with_keyring_store(
    ...
    let codex_home = resolve_auth_storage_home(codex_home);
    create_auth_storage_with_resolved_home(codex_home, mode, keyring_store)
}
...
fn resolve_auth_storage_home(codex_home: PathBuf) -> PathBuf {
    match codep_shared_codex_home(&codex_home) {
        Ok(Some(shared_home)) => shared_home,
        Ok(None) | Err(_) => codex_home,
    }
}
