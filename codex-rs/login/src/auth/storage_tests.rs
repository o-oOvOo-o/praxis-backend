...
#[test]
fn create_auth_storage_with_resolved_home_uses_shared_codex_home() -> anyhow::Result<()> {
    let codep_home = tempdir()?;
    let shared_codex_home = tempdir()?;

    let storage = create_auth_storage_with_resolved_home(
        shared_codex_home.path().to_path_buf(),
    ...
    assert!(
        !codep_home.path().join("auth.json").exists(),
        "original codep home should remain untouched when auth storage is bridged"
    );
    let shared_auth_file = shared_codex_home.path().join("auth.json");
    assert!(
        shared_auth_file.exists(),
        "shared codex home should receive the auth.json"
    );

    let persisted = FileAuthStorage::new(shared_codex_home.path().to_path_buf()).load()?;
