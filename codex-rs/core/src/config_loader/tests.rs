...
#[tokio::test]
async fn falls_back_to_shared_codex_user_config_for_default_codep_home() -> std::io::Result<()> {
    let codep_home = tempdir()?;
    let shared_codex_home = tempdir()?;
    tokio::fs::write(
        shared_codex_home.path().join(CONFIG_TOML_FILE),
        "model = \"shared-codex-model\"\n",
    )
    .await?;

    let resolved = super::resolve_user_config_toml_file_with_shared_home(
        codep_home.path(),
        Some(shared_codex_home.path()),
    )
    .await?;

    assert_eq!(
        resolved,
        AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, shared_codex_home.path())?
    );
    Ok(())
}
