...
use codex_utils_home_dir::codep_shared_codex_home;
...
pub async fn load_config_layers_state(
    ...
    // user.
    let user_file = resolve_user_config_toml_file(codex_home).await?;
...
async fn resolve_user_config_toml_file(codex_home: &Path) -> io::Result<AbsolutePathBuf> {
    let shared_codex_home = codep_shared_codex_home(codex_home)?;
    resolve_user_config_toml_file_with_shared_home(codex_home, shared_codex_home.as_deref()).await
}
...
async fn resolve_user_config_toml_file_with_shared_home(
    ...
    let Some(shared_codex_home) = shared_codex_home else {
        return Ok(user_file);
    };
    let shared_user_file =
        AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, shared_codex_home)?;
