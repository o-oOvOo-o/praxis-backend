...
impl Session {
    ...
    async fn new(
        ...
        {
            thread_name =
                match session_index::find_thread_name_by_id(&config.codex_home, &source_thread_id)
                    .await
                {
                    Ok(name) => {
        ...
        if inherited_thread_name_from_fork
            ...
            && let Err(err) =
                session_index::append_thread_name(&config.codex_home, conversation_id, name).await
        {
