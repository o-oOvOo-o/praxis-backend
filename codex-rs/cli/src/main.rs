...
use codex_tui::SessionLookupSource;
...
use codex_core::util::PRIMARY_CLI_COMMAND;
...
#[clap(
    ...
    // `codep-x86_64-unknown-linux-musl`, but the help output should always use
    // the generic `codep` command name that users run.
    bin_name = "codep",
    override_usage = "codep [OPTIONS] [PROMPT]\n       codep [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
...
enum Subcommand {
    /// Run Code Puppy non-interactively.
    ...
    /// Manage external MCP servers for Code Puppy.
    Mcp(McpCli),

    /// Start Code Puppy as an MCP server (stdio).
    McpServer,

    /// [experimental] Run the app server or related tooling.
    AppServer(AppServerCommand),

    /// Launch the Code Puppy desktop app (downloads the macOS installer if missing).
    #[cfg(target_os = "macos")]
    ...
    /// Run commands within a Code Puppy-provided sandbox.
    Sandbox(SandboxArgs),
    ...
    /// Apply the latest diff produced by Code Puppy agent as a `git apply` to your local working tree.
...
struct ResumeCommand {
    /// Optional source namespace (`codep` or `codex`) followed by a conversation/session id
    /// (UUID) or thread name. If omitted, defaults to CodeP sessions.
...
struct LoginCommand {
    ...
    #[arg(
        long = "with-api-key",
        help = "Read the API key from stdin (e.g. `printenv OPENAI_API_KEY | codep login --with-api-key`)"
    )]
    with_api_key: bool,
...
fn run_update_action(action: UpdateAction) -> anyhow::Result<()> {
    ...
    println!("Updating Code Puppy via `{cmd_str}`...");
    ...
    println!("\n🎉 Update ran successfully! Please restart Code Puppy.");
    Ok(())
}
...
impl SessionTargetSource {
    ...
    fn lookup_source(self) -> SessionLookupSource {
        match self {
            Self::Codep => SessionLookupSource::Codep,
...
fn parse_session_target_args(
    targets: Vec<String>,
    action: &str,
) -> anyhow::Result<ParsedSessionTarget> {
    match targets.as_slice() {
        [] => Ok(ParsedSessionTarget {
            source: SessionTargetSource::Codep,
            session_id: None,
        }),
        [single] => {
        ...
        [source, session_id] => {
            let Some(source) = SessionTargetSource::from_keyword(source) else {
                anyhow::bail!(
                    "`codep {action}` accepts two positional arguments only when the first is `codep` or `codex`."
                );
            };
            Ok(ParsedSessionTarget {
                source,
...
fn validate_session_target_with_last(
    ...
    if last && parsed_target.session_id.is_some() {
        anyhow::bail!(
            "`codep {action} --last` cannot be combined with an explicit session id or thread name."
        );
    }
    Ok(())
}
            ...
            match login_cli.action {
                ...
                None => {
                    if login_cli.use_device_code {
                        ...
                        eprintln!(
                            "The --api-key flag is no longer supported. Pipe the key instead, e.g. `printenv OPENAI_API_KEY | codep login --with-api-key`."
                        );
                        std::process::exit(1);
...
async fn run_interactive_tui(
    ...
    if terminal_info.name == TerminalName::Dumb {
        ...
        eprintln!(
            "WARNING: TERM is set to \"dumb\". Code Puppy's interactive TUI may not work in this terminal."
        );
...
/// Build the final `TuiCli` for a `codep resume` invocation.
fn finalize_resume_interactive(
    ...
    // configuration surface area as `codep` without additional flags.
...
/// Build the final `TuiCli` for a `codep fork` invocation.
...
/// Merge flags provided to `codep resume`/`codep fork` so they take precedence over any
...
mod tests {
    ...
    fn format_exit_messages_includes_resume_hint_without_color() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "To continue this session, run codep resume 123e4567-e89b-12d3-a456-426614174000"
                    .to_string(),
            ]
        );
    }
    ...
    fn format_exit_messages_prefers_thread_name() {
        ...
        assert_eq!(
            ...
            vec![
                "Token usage: total=2 input=0 output=2".to_string(),
                "To continue this session, run codep resume my-thread".to_string(),
            ]
        );
    }
    ...
    fn resume_model_flag_applies_when_no_root_flags() {
        ...
        assert_eq!(interactive.resume_source, SessionLookupSource::Codep);
    }
    ...
    fn fork_picker_logic_none_and_not_last() {
        ...
        assert_eq!(interactive.fork_source, SessionLookupSource::Codep);
    }
