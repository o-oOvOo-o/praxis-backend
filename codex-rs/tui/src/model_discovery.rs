...
use codex_core::model_provider_info::ModelProviderCompatInfo;
use codex_core::model_provider_info::ModelProviderInfo;
use codex_core::model_provider_info::ModelProviderMaxTokensField;
use codex_core::model_provider_info::ModelProviderReasoningEffortMap;
use codex_core::model_provider_info::ModelProviderThinkingFormat;
...
struct CodepConfigToml {
    model: Option<String>,
    review_model: Option<String>,
    model_provider: Option<String>,
    profile: Option<String>,
    #[serde(default)]
    profiles: BTreeMap<String, CodepConfigProfileToml>,
    #[serde(default)]
    model_providers: BTreeMap<String, toml::Table>,
}

#[derive(Debug, Clone, Default, serde::Deserialize)]
struct CodepConfigProfileToml {
    model: Option<String>,
    model_provider: Option<String>,
}
...
fn collect_discovered_models(config: &Config) -> Vec<DiscoveredModel> {
    let mut models = Vec::new();
    models.extend(collect_codep_config_models(config));
    models.extend(collect_claude_code_models());
    models.extend(collect_zed_models());
    models
}

fn collect_codep_config_models(config: &Config) -> Vec<DiscoveredModel> {
    let mut models = Vec::new();
    let current_config_path = config.codex_home.join("config.toml");
    for path in discover_codep_config_paths() {
        let Ok(codep_config) = parse_toml_file::<CodepConfigToml>(&path) else {
            continue;
        };
        models.extend(build_codep_config_models(
            config,
            &codep_config,
            &path,
            &current_config_path,
...
fn build_codep_config_models(
    config: &Config,
    codep_config: &CodepConfigToml,
    path: &Path,
    current_config_path: &Path,
) -> Vec<DiscoveredModel> {
    let mut provider_models = BTreeMap::<String, Vec<(String, bool)>>::new();

    if let Some((provider_id, model)) = codep_effective_selection(codep_config, None) {
        insert_provider_model_candidate(&mut provider_models, &provider_id, model, false);
    }

    let active_profile = codep_config
        .profile
        .as_deref()
        .and_then(|name| codep_config.profiles.get(name));
    if let Some((provider_id, model)) = codep_effective_selection(codep_config, active_profile) {
        insert_provider_model_candidate(&mut provider_models, &provider_id, model, true);
    }

    if let Some(review_model) = codep_config_string(codep_config.review_model.as_deref())
        && let Some(provider_id) = active_profile
            .and_then(|profile| codep_config_string(profile.model_provider.as_deref()))
            .or_else(|| codep_config_string(codep_config.model_provider.as_deref()))
    {
        insert_provider_model_candidate(&mut provider_models, &provider_id, review_model, false);
    }

    for profile in codep_config.profiles.values() {
        if let Some((provider_id, model)) = codep_effective_selection(codep_config, Some(profile)) {
            insert_provider_model_candidate(&mut provider_models, &provider_id, model, false);
        }
    }
    ...
    let subtitle = if normalize_path(path).ends_with("/.codex/config.toml") {
        "Legacy Codex config"
    } else if normalize_path(path).ends_with("/.codep/config.toml") {
        "CodeP config"
    } else {
        "Discovered config"
    };
    ...
    for original_provider_id in provider_ids {
        let parsed_provider = codep_config
            .model_providers
            .get(&original_provider_id)
            .and_then(parse_codep_config_provider_table);
        let Some((provider_id, provider_name, provider)) = resolve_codep_provider(
            config,
            path,
            current_config_path,
    ...
    match (path_is_current, existing_provider, parsed_provider.cloned()) {
        ...
        (_, _, Some(parsed)) => {
            let provider_id = if path_is_current {
                ...
                imported_codep_provider_id(path, original_provider_id)
            };
...
fn imported_codep_provider_id(path: &Path, provider_id: &str) -> String {
...
fn codep_effective_selection(
    config: &CodepConfigToml,
    profile: Option<&CodepConfigProfileToml>,
) -> Option<(String, String)> {
    let model = profile
        .and_then(|profile| codep_config_string(profile.model.as_deref()))
        .or_else(|| codep_config_string(config.model.as_deref()))?;
    let provider_id = profile
        .and_then(|profile| codep_config_string(profile.model_provider.as_deref()))
        .or_else(|| codep_config_string(config.model_provider.as_deref()))?;
...
fn insert_provider_model_candidate(
    ...
    if let Some(existing) = entry.iter_mut().find(|(existing_model, _)| existing_model == model) {
...
fn parse_codep_config_provider_table(table: &toml::Table) -> Option<ModelProviderInfo> {
...
fn collect_claude_code_models() -> Vec<DiscoveredModel> {
    ...
    if provider_kind != ClaudeCodeProviderKind::FirstParty {
        return Vec::new();
    }
...
fn insert_zed_preference(
    ...
    if !entry.preferred_models.iter().any(|existing| existing == model) {
...
fn discover_codep_config_paths() -> Vec<PathBuf> {
    ...
    for candidate in [
        home.join(".codep").join("config.toml"),
...
fn detect_claude_code_provider() -> ClaudeCodeProviderKind {
    if env_truthy("CLAUDE_CODE_USE_BEDROCK") {
        ClaudeCodeProviderKind::Bedrock
    } else if env_truthy("CLAUDE_CODE_USE_VERTEX") {
        ClaudeCodeProviderKind::Vertex
    } else if env_truthy("CLAUDE_CODE_USE_FOUNDRY") {
        ClaudeCodeProviderKind::Foundry
    } else {
        ClaudeCodeProviderKind::FirstParty
    }
}
...
fn codep_config_string(value: Option<&str>) -> Option<String> {
    value
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .map(str::to_owned)
