...
use super::frame_rate_limiter::FramePacing;
...
pub struct FrameRequester {
    ...
    shared_state: Arc<Mutex<SharedFrameState>>,
}
...
impl FrameRequester {
    ...
    pub fn new(draw_tx: broadcast::Sender<()>) -> Self {
        ...
        let shared_state = Arc::new(Mutex::new(SharedFrameState::default()));
        let scheduler = FrameScheduler::new(rx, draw_tx, shared_state.clone());
    ...
    pub fn schedule_frame(&self) {
        self.schedule_request(Instant::now(), FramePacing::Immediate);
    }

    /// Schedule a frame draw to occur after the specified duration.
    pub fn schedule_frame_in(&self, dur: Duration) {
        self.schedule_request(Instant::now() + dur, FramePacing::Immediate);
    }

    /// Schedule a frame draw for user-driven transcript scrolling.
    ///
    /// Scroll input can arrive in bursts while the user is holding a key or wheel.
    /// We still redraw promptly, but clamp those redraws to a 60 FPS lane so the
    /// transcript advances smoothly without competing with other frame work.
    pub fn schedule_scroll_frame(&self) {
        self.schedule_request(Instant::now(), FramePacing::Scroll);
    }

    fn schedule_request(&self, requested_at: Instant, pacing: FramePacing) {
        let should_wake = {
            let mut shared_state = self
                .shared_state
                .lock()
                .unwrap_or_else(std::sync::PoisonError::into_inner);
            shared_state.schedule(requested_at, pacing)
        };
        if should_wake {
                ...
                _ = tokio::time::sleep_until(next_deadline.into()) => {
                    let emitted = {
                        let mut shared_state = self
                            ...
                            .lock()
                            .unwrap_or_else(std::sync::PoisonError::into_inner);
                        shared_state.take_due_deadline(next_deadline)
                    };
                    if emitted.is_some() {
...
impl SharedFrameState {
    fn schedule(&mut self, requested_at: Instant, pacing: FramePacing) -> bool {
        let deadline = self.rate_limiter.clamp_deadline(requested_at, pacing);
    ...
    fn clear_wake_queued(&mut self) {
        self.wake_queued = false;
    }

    fn take_due_deadline(&mut self, observed_deadline: Instant) -> Option<Instant> {
        let pending_deadline = self.pending_deadline?;
        if pending_deadline > observed_deadline {
            return None;
        }
        self.pending_deadline = None;
        self.rate_limiter.mark_emitted(pending_deadline);
...
mod tests {
    ...
    use super::super::frame_rate_limiter::SCROLL_FRAME_INTERVAL;
    use super::*;
    use tokio::time;
    ...
    #[tokio::test(flavor = "current_thread", start_paused = true)]
    async fn test_scroll_frames_use_scroll_pacing() {
        let (draw_tx, mut draw_rx) = broadcast::channel(16);
        let requester = FrameRequester::new(draw_tx);

        requester.schedule_scroll_frame();
        time::advance(Duration::from_millis(1)).await;
        let first = draw_rx
            .recv()
            .timeout(Duration::from_millis(50))
            .await
            .expect("timed out waiting for first scroll draw");
        assert!(first.is_ok(), "broadcast closed unexpectedly");
        ...
        assert!(
            too_early.is_err(),
            "scroll draw fired too early; expected min interval {SCROLL_FRAME_INTERVAL:?}"
        );

        time::advance(SCROLL_FRAME_INTERVAL).await;
        let second = draw_rx
            .recv()
            .timeout(Duration::from_millis(50))
            .await
            .expect("timed out waiting for scroll-paced draw");
        assert!(second.is_ok(), "broadcast closed unexpectedly");
    }

    #[tokio::test(flavor = "current_thread", start_paused = true)]
    async fn test_immediate_draw_preempts_pending_scroll_draw() {
        let (draw_tx, mut draw_rx) = broadcast::channel(16);
        let requester = FrameRequester::new(draw_tx);
        ...
        let extra = draw_rx.recv().timeout(SCROLL_FRAME_INTERVAL).await;
        assert!(extra.is_err(), "unexpected trailing scroll draw received");
    }
}
