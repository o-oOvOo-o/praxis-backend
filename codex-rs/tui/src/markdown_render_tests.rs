...
fn bare_relative_file_link_hides_destination() {
    let text = render_markdown_text_for_cwd(
        "[markdown_render.rs](codex-rs/tui/src/markdown_render.rs:74)",
...
fn relative_local_link_hyperlink_target_resolves_against_cwd() {
    ...
    assert_eq!(
        target,
        Some("file:///Users/example/code/codex-rs/tui/src/markdown_render.rs#L74C3".to_string())
    );
}
...
fn wrapped_relative_local_file_link_token_stays_whole() {
    let text = render_markdown_text_with_width_and_cwd(
        "See [markdown_render.rs](codex-rs/tui/src/markdown_render.rs:74) for details.",
        Some(18),
        Some(Path::new("/Users/example/code")),
    );
    let rendered = text
    ...
    assert_eq!(
        rendered
            .iter()
            .filter(|line| line.contains("codex-rs/tui/src/markdown_render.rs:74"))
            .count(),
        1,
