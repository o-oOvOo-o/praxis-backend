...
use codex_rollout::session_index::SessionIndexEntry;
...
impl ReasoningSummaryCell {
    ...
    fn expanded_lines(&self, width: u16) -> Vec<Line<'static>> {
...
pub(crate) fn new_session_info(
    ...
    header.apply_startup_context(&config.codex_home, Some(session_id), auth_plan, 3);
...
impl SessionHeaderHistoryCell {
    ...
    pub(crate) fn apply_startup_context(
        ...
        self.recent_activity =
            Self::load_recent_activity(codex_home, current_session_id, recent_activity_limit);
    }
    ...
    fn home_directory_warning_line() -> Line<'static> {
        Line::from(vec![
            ...
            Span::from(
                " codep is running in your home directory. Launch it in a project directory for the best experience.",
            ),
        ])
    }
    ...
    fn border_title_spans(&self) -> Vec<Span<'static>> {
        vec![
            Span::styled(" Code Puppy CLI ", Self::section_title_style()),
            Span::styled(
...
///   register it as an MCP server via `codep mcp add`.
...
mod tests {
    ...
    async fn session_info_availability_nux_tooltip_snapshot() {
        ...
        let rendered = render_transcript(&cell).join("\n");
        assert!(rendered.contains("Code Puppy CLI"));
    ...
    fn session_header_wide_snapshot_matches_claude_style_layout() {
        ...
        cell.set_startup_notice(Some(
            "Workspace-aware resume keeps Code Puppy threads attached to this project.".to_string(),
        ));
        cell.recent_activity = vec![
            StartupRecentActivity {
                thread_id: ThreadId::new(),
        ...
        assert!(
            rendered.contains(
                "Workspace-aware resume keeps Code Puppy threads attached to this project."
            )
        );
        assert!(rendered.contains("Recent activity"));
        assert!(rendered.contains("/resume for more"));
