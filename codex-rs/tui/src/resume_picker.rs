...
impl SourceSwitcher {
    fn from_sources(
        ...
        match (primary_source, alternate_source) {
            (SessionLookupSource::Codep, SessionLookupSource::Codex) => Self {
                codep: primary,
                codex: alternate,
            },
            (SessionLookupSource::Codex, SessionLookupSource::Codep) => Self {
                codep: alternate,
                codex: primary,
            },
            _ => unreachable!("source switcher requires CodeP and Codex sources"),
        }
    }

    fn config(&self, source: SessionLookupSource) -> &PickerSourceConfig {
        match source {
            SessionLookupSource::Codep => &self.codep,
...
async fn run_session_picker_with_loader(
    ...
    let filter_cwd = if show_all || is_remote {
        ...
        Some(project_scope_root(config.cwd.as_path()))
    };
...
impl PickerState {
    fn new(
        ...
        Self {
            ...
            filter_cwd,
            action,
            active_source: SessionLookupSource::Codep,
    ...
    fn apply_source(&mut self, source: SessionLookupSource) {
        let Some((codex_home, page_loader)) = self.source_switcher.as_ref().map(|switcher| {
            let source_config = switcher.config(source);
            (
                source_config.codex_home.clone(),
    ...
    async fn handle_key(&mut self, key: KeyEvent) -> Result<Option<SessionSelection>> {
        ...
        match key.code {
            ...
            KeyCode::Left => {
                self.switch_source(SessionLookupSource::Codep);
                self.request_frame();
            }
            KeyCode::Right => {
    ...
    fn row_matches_filter(&self, row: &Row) -> bool {
        ...
        paths_match(project_scope_root(row_cwd).as_path(), filter_cwd)
    }
...
fn project_scope_root(path: &Path) -> PathBuf {
    codex_git_utils::resolve_root_git_project_for_trust(path).unwrap_or_else(|| path.to_path_buf())
}
...
fn source_display_name(source: SessionLookupSource) -> &'static str {
    match source {
        SessionLookupSource::Codep => "CodeP",
...
mod tests {
    ...
    fn picker_header_and_hint_show_source_switcher() {
        let codep_loader: PageLoader = Arc::new(|_| {});
        let codex_loader: PageLoader = Arc::new(|_| {});
        let mut state = PickerState::new(
            PathBuf::from("/tmp/codep"),
            FrameRequester::test_dummy(),
            codep_loader.clone(),
            ProviderFilter::Any,
            /*show_all*/ true,
        ...
        state.configure_source_switcher(
            ...
            SourceSwitcher::from_sources(
                SessionLookupSource::Codep,
                PickerSourceConfig {
                    codex_home: PathBuf::from("/tmp/codep"),
                    page_loader: codep_loader,
                },
                SessionLookupSource::Codex,
                PickerSourceConfig {
    ...
    async fn switching_source_reloads_other_loader_and_codex_resume_forks() {
        let codep_requests: Arc<Mutex<Vec<PageLoadRequest>>> = Arc::new(Mutex::new(Vec::new()));
        let codex_requests: Arc<Mutex<Vec<PageLoadRequest>>> = Arc::new(Mutex::new(Vec::new()));
        let codep_sink = codep_requests.clone();
        let codex_sink = codex_requests.clone();
        let codep_loader: PageLoader = Arc::new(move |req: PageLoadRequest| {
            codep_sink.lock().unwrap().push(req);
        });
        ...
        state.start_initial_load();
        assert_eq!(codep_requests.lock().unwrap().len(), 1);
