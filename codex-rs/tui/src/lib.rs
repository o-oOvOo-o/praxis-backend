...
use codex_core::config::CodexHomeNamespace;
use codex_core::config::Config;
use codex_core::config::ConfigBuilder;
use codex_core::config::ConfigOverrides;
use codex_core::config::current_codex_home_namespace;
use codex_core::config::default_codex_home_for_namespace;
...
fn session_lookup_command_hint(action: &str, source: SessionLookupSource) -> String {
    match source {
        SessionLookupSource::Codep => format!("codep {action}"),
        SessionLookupSource::Codex => format!("codep {action} codex"),
    }
}
...
pub(crate) async fn build_codex_bridge_lookup_config(
    ...
    let codex_home = default_codex_home_for_namespace(CodexHomeNamespace::Codex)?;
...
pub(crate) fn picker_source_switch_enabled(app_server_target: &AppServerTarget) -> bool {
    matches!(current_codex_home_namespace(), CodexHomeNamespace::Codep)
...
async fn start_session_lookup_context(
    ...
    let lookup_config = match source {
        SessionLookupSource::Codep => primary_config.clone(),
        SessionLookupSource::Codex => build_codex_bridge_lookup_config(primary_config)
            .await
        ...
        if let Some(id_str) = cli.fork_session_id.as_deref() {
            ...
            let alternate_source = if picker_source_switch_enabled(&app_server_target) {
                let alternate_lookup = start_session_lookup_context(
                    match source {
                        SessionLookupSource::Codep => SessionLookupSource::Codex,
                        SessionLookupSource::Codex => SessionLookupSource::Codep,
                    },
                    &config,
                    &app_server_target,
                    arg0_paths.clone(),
