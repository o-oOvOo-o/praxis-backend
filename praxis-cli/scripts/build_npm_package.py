...
def prepare_staging_dir(staging_dir: Path | None) -> tuple[Path, bool]:
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="praxis-npm-stage-"))
...
def stage_sources(staging_dir: Path, version: str, package: str) -> None:
    ...
    if package == "praxis":
        ...
        package_json = {
            ...
            "files": ["vendor"],
            "repository": praxis_package_json.get("repository"),
        }

        engines = praxis_package_json.get("engines")
        if isinstance(engines, dict):
            package_json["engines"] = engines

        package_manager = praxis_package_json.get("packageManager")
        if isinstance(package_manager, str):
            package_json["packageManager"] = package_manager
    elif package == "praxis-responses-api-proxy":
        bin_dir = staging_dir / "bin"
        bin_dir.mkdir(parents=True, exist_ok=True)
        launcher_src = RESPONSES_API_PROXY_NPM_ROOT / "bin" / "praxis-responses-api-proxy.js"
        shutil.copy2(launcher_src, bin_dir / "praxis-responses-api-proxy.js")
...
def stage_praxis_sdk_sources(staging_dir: Path) -> None:
    ...
    if not dist_src.exists():
        raise RuntimeError("praxis-sdk build did not produce a dist directory.")
...
def run_npm_pack(staging_dir: Path, output_path: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="praxis-npm-pack-") as pack_dir_str:
