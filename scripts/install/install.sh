...
VERSION="${1:-latest}"
INSTALL_DIR="${PRAXIS_INSTALL_DIR:-$HOME/.local/bin}"
path_action="already"
path_profile=""
...
download_file() {
  ...
  echo "curl or wget is required to install Praxis." >&2
  exit 1
}
...
release_url_for_asset() {
  ...
  printf 'https://github.com/o-oOvOo-o/praxis-backend/releases/download/rust-v%s/%s\n' "$resolved_version" "$asset"
}

require_command() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "$1 is required to install Praxis." >&2
    exit 1
  fi
}
...
resolve_version() {
  ...
  release_json="$(download_text "https://api.github.com/repos/o-oOvOo-o/praxis-backend/releases/latest")"
  resolved="$(printf '%s\n' "$release_json" | sed -n 's/.*"tag_name":[[:space:]]*"rust-v\([^"]*\)".*/\1/p' | head -n 1)"

  if [ -z "$resolved" ]; then
    echo "Failed to resolve the latest Praxis release version." >&2
    exit 1
  fi
...
if [ -x "$INSTALL_DIR/praxis" ]; then
  install_mode="Updating"
else
  install_mode="Installing"
fi

step "$install_mode Praxis CLI"
...
case "$path_action" in
  added)
    ...
    step "Run now: export PATH=\"$INSTALL_DIR:\$PATH\" && praxis"
    step "Or open a new terminal and run: praxis"
    ;;
  configured)
...
printf 'Praxis CLI %s installed successfully.\n' "$resolved_version"
