...
- `praxis-rs/tui/src/bottom_pane/chat_composer.rs`

Paste-burst detector:

- `praxis-rs/tui/src/bottom_pane/paste_burst.rs`
...
- **Persistent history** (cross-session, fetched from the Praxis history store): text-only. It
...
(`praxis-rs/tui/src/bottom_pane/chat_composer.rs`). The default config preserves current chat
behavior.
...
`praxis-rs/tui/src/bottom_pane/slash_commands.rs` and reused by both the composer and the command
