...
`praxis-rs/tui/src/streaming/chunking.rs`, and baseline commit cadence in
`praxis-rs/tui/src/app.rs`.
...
- Capture trace logs with:
  - `praxis_tui::streaming::commit_tick`
...
- `cargo test -p praxis-tui` passes.
